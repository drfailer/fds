...
SUBROUTINE VELOCITY_BC_PROCESS_EDGES_KERNEL(M,NM,T,APPLY_TO_ESTIMATED_VARIABLES)
...
! Call preprocessing: OMESH reads for wall boundary velocities
CALL VELOCITY_BC_PREPROCESSING(M,NM,T,APPLY_TO_ESTIMATED_VARIABLES)

! Call edge processing kernel: process all edge boundary conditions

CALL VELOCITY_BC_PROCESS_EDGES_KERNEL(M,NM,T,APPLY_TO_ESTIMATED_VARIABLES)
