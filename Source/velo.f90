...
PUBLIC VELOCITY_PREDICTOR,VELOCITY_CORRECTOR,NO_FLUX,BAROCLINIC_CORRECTION,MATCH_VELOCITY,MATCH_VELOCITY_FLUX,&
       ...
       CHECK_STABILITY,WALL_VELOCITY_NO_GRADH
...
CALL VELOCITY_FLUX_KERNEL(M,T,DT,NM,APPLY_TO_ESTIMATED_VARIABLES,GX,GY,GZ)
...
CALL VELOCITY_PREDICTOR_KERNEL(M,DT)
...
IF (.NOT.FREEZE_VELOCITY) THEN
   ...
   ENDIF
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,ULMAT_FLAG); CALL WALL_VELOCITY_NO_GRADH(M,DT,.FALSE.)
   END SELECT
ENDIF
   ...
   DO K=1,M%KBAR
      DO J=1,M%JBAR
         DO I=0,M%IBAR
            ...
            M%US(I,J,K) = VD2D_MMS_U(XHAT,ZHAT,T)
         ENDDO
      ENDDO
   ENDDO
   DO K=0,M%KBAR
      DO J=1,M%JBAR
         DO I=1,M%IBAR
            ...
            ZHAT =  M%Z(K) - WF_MMS*(T)
            M%WS(I,J,K) = VD2D_MMS_V(XHAT,ZHAT,T)
         ENDDO
      ENDDO
   ENDDO
ENDIF
...
IF (.NOT.FREEZE_VELOCITY) THEN
   ...
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,ULMAT_FLAG)
         CALL WALL_VELOCITY_NO_GRADH(M,DT,.TRUE.)                    ! Store U velocities on OBST surfaces.
   END SELECT
ENDIF

CALL VELOCITY_CORRECTOR_KERNEL(M,DT)
...
IF (.NOT.FREEZE_VELOCITY) THEN
   ...
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,ULMAT_FLAG)
         CALL WALL_VELOCITY_NO_GRADH(M,DT,.FALSE.)
   END SELECT
ENDIF
   ...
   DO K=1,M%KBAR
      DO J=1,M%JBAR
         DO I=0,M%IBAR
            XHAT =  M%X(I) - UF_MMS*T
            ZHAT = M%ZC(K) - WF_MMS*T
            M%U(I,J,K) = VD2D_MMS_U(XHAT,ZHAT,T)
         ENDDO
      ENDDO
   ENDDO
   DO K=0,M%KBAR
      DO J=1,M%JBAR
         DO I=1,M%IBAR
            XHAT = M%XC(I) - UF_MMS*T
            ZHAT =  M%Z(K) - WF_MMS*T
            M%W(I,J,K) = VD2D_MMS_V(XHAT,ZHAT,T)
         ENDDO
      ENDDO
   ENDDO
ENDIF
...
RECURSIVE SUBROUTINE MATCH_VELOCITY_FLUX(NM,M)
...
IF(CC_IBM) THEN
   CALL CC_MATCH_VELOCITY_FLUX(NM,M)
   RETURN
ENDIF
...
IF(CC_IBM) CALL CC_BAROCLINIC_CORRECTION(T,NM,MESHES(NM))
...
SUBROUTINE WALL_VELOCITY_NO_GRADH(M,DT,STORE_UN)
...
STORE_UN_COND : IF ( STORE_UN .AND. CORRECTOR) THEN
   ...
   ALLOCATE( UN_WALLS(1:M%N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS_AUX) )
   UN_WALLS(:) = 0._EB

   STORE_LOOP : DO IW=1,M%N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS_AUX
      ...
      SELECT CASE(IOR)
      CASE( IAXIS)
         UN_WALLS(IW) = M%U(IIG-1,JJG  ,KKG  )
      CASE(-IAXIS)
         UN_WALLS(IW) = M%U(IIG  ,JJG  ,KKG  )
      CASE( JAXIS)
         UN_WALLS(IW) = M%V(IIG  ,JJG-1,KKG  )
      CASE(-JAXIS)
         UN_WALLS(IW) = M%V(IIG  ,JJG  ,KKG  )
      CASE( KAXIS)
         UN_WALLS(IW) = M%W(IIG  ,JJG  ,KKG-1)
      CASE(-KAXIS)
         UN_WALLS(IW) = M%W(IIG  ,JJG  ,KKG  )
      END SELECT
...
PREDICTOR_COND : IF (PREDICTOR) THEN
   ...
   WALL_CELL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS_AUX
      ...
      IF (WC%BOUNDARY_TYPE==NULL_BOUNDARY .AND. .NOT.M%CELL(ICG)%SOLID .AND. .NOT.M%CELL(IC)%SOLID) CYCLE

      IOR = BC%IOR

      SELECT CASE(IOR)
      CASE( IAXIS)
         M%US(IIG-1,JJG  ,KKG  ) = (M%U(IIG-1,JJG  ,KKG  ) - DT*( M%FVX(IIG-1,JJG  ,KKG  ) ))
      CASE(-IAXIS)
         M%US(IIG  ,JJG  ,KKG  ) = (M%U(IIG  ,JJG  ,KKG  ) - DT*( M%FVX(IIG  ,JJG  ,KKG  ) ))
      CASE( JAXIS)
         M%VS(IIG  ,JJG-1,KKG  ) = (M%V(IIG  ,JJG-1,KKG  ) - DT*( M%FVY(IIG  ,JJG-1,KKG  ) ))
      CASE(-JAXIS)
         M%VS(IIG  ,JJG  ,KKG  ) = (M%V(IIG  ,JJG  ,KKG  ) - DT*( M%FVY(IIG  ,JJG  ,KKG  ) ))
      CASE( KAXIS)
         M%WS(IIG  ,JJG  ,KKG-1) = (M%W(IIG  ,JJG  ,KKG-1) - DT*( M%FVZ(IIG  ,JJG  ,KKG-1) ))
      CASE(-KAXIS)
         M%WS(IIG  ,JJG  ,KKG  ) = (M%W(IIG  ,JJG  ,KKG  ) - DT*( M%FVZ(IIG  ,JJG  ,KKG  ) ))
      END SELECT
...
ELSE ! Corrector
  ...
  WALL_CELL_LOOP_2: DO IW=1,M%N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS_AUX
