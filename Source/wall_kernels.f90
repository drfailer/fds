...
PUBLIC CALCULATE_RHO_D_F, CALC_DEPOSITION, PYROLYSIS, &
       ...
       NEAR_SURFACE_GAS_VARIABLES_KERNEL, ASSIGN_GHOST_VALUE_KERNEL
...
!> \brief Thread-safe kernel version of ASSIGN_GHOST_VALUE.
!> Fills ghost cells of RHO/RHOS, ZZ/ZZS, TMP, RSUM from neighboring mesh OMESH data.
!> \param M Mesh data structure
!> \param WALL_INDEX Wall cell index (must be <= N_EXTERNAL_WALL_CELLS)
!> \param BC Boundary coordinate pointer
!> \param B1 Boundary property pointer
!> \param PREDICTOR_FLAG Whether we are in the predictor phase

SUBROUTINE ASSIGN_GHOST_VALUE_KERNEL(M,WALL_INDEX,BC,B1,PREDICTOR_FLAG)
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: OM_RHOP, RHOP
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP
REAL(EB), POINTER, DIMENSION(:,:) :: PBAR_P
...
M%TMP(BC%II,BC%JJ,BC%KK) = PBAR_P(BC%KK,B1%PRESSURE_ZONE)/(M%RSUM(BC%II,BC%JJ,BC%KK)*RHOP(BC%II,BC%JJ,BC%KK))
...
INTERPOLATED_IF: IF (WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY) THEN
   ...
   IF (ABS(EWC%AREA_RATIO-1._EB)<0.01_EB) THEN
      ...
      END SELECT
      IF (CC_IBM) THEN
         IF(M%CCVAR(BC%IIG,BC%JJG,BC%KKG,CC_CGSC)==CC_SOLID .OR. &
            M%CCVAR(BC%II,BC%JJ,BC%KK,CC_CGSC)==CC_SOLID) CC_SOLID_FLAG = .TRUE.
      ENDIF
      IF (.NOT.M%CELL(ICG)%SOLID.AND..NOT.MM%CELL(ICO)%SOLID.AND..NOT.CC_SOLID_FLAG) &
...
IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
   ...
   ZZP(BC%II2,BC%JJ2,BC%KK2,1:N_TOTAL_SCALARS) = &
      MAX(0._EB,MIN(1._EB,RHO_ZZ_OTHER_2(1:N_TOTAL_SCALARS)/RHO_OTHER_2))
   ...
   M%TMP(BC%II2,BC%JJ2,BC%KK2) = PBAR_P_2/(RSUM_TMP*RHOP(BC%II2,BC%JJ2,BC%KK2))
ELSE
   ...
   M%TMP(BC%II2,BC%JJ2,BC%KK2) = M%TMP(BC%II,BC%JJ,BC%KK)
ENDIF

END SUBROUTINE ASSIGN_GHOST_VALUE_KERNEL
