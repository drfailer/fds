...
!> \brief Calculate the surface density RHO_F at a boundary cell
!> \param M Mesh data structure
!> \param BC Pointer to BOUNDARY_COORD derived type variable
!> \param B1 Pointer to BOUNDARY_PROP1 derived type variable
!> \param WALL_INDEX Optional WALL cell index
...
SUBROUTINE CALCULATE_RHO_F_KERNEL(M,BC,B1,WALL_INDEX,CFACE_INDEX)
...
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: B1
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
REAL(EB) :: ZZ_GET(1:MAX_SPECIES),RSUM_F,UN_P
...
IF (PRESENT(WALL_INDEX)) THEN
   BOUNDARY_TYPE = M%WALL(WALL_INDEX)%BOUNDARY_TYPE
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   BOUNDARY_TYPE = M%CFACE(CFACE_INDEX)%BOUNDARY_TYPE
ENDIF

ZZ_GET(1:N_TRACKED_SPECIES) = &
   MAX(0._EB,B1%ZZ_F(1:N_TRACKED_SPECIES))
CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
B1%RHO_F = PBAR_P(BC%KK,B1%PRESSURE_ZONE) / &
   (RSUM_F*B1%TMP_F)
...
IF (BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
   ...
   ENDIF
   IF (UN_P>0._EB) THEN
      B1%ZZ_F(1:N_TRACKED_SPECIES) = &
         B1%ZZ_G(1:N_TRACKED_SPECIES)
      B1%RHO_F = B1%RHO_G
   ENDIF
ENDIF

IF (PRESENT(WALL_INDEX) .AND. &
    BOUNDARY_TYPE/=OPEN_BOUNDARY) THEN
   IF (WALL_INDEX<=M%N_EXTERNAL_WALL_CELLS) THEN
...
END SUBROUTINE CALCULATE_RHO_F_KERNEL


!> \brief Compute near-surface gas-phase variables for boundary cells
!> \param M Mesh data structure
!> \param T Current time (s)
!> \param SF Pointer to SURFACE derived type variable
!> \param BC Pointer to BOUNDARY_COORD derived type variable
!> \param B1 Pointer to BOUNDARY_PROP1 derived type variable
!> \param LP (Optional) Pointer to LAGRANGIAN_PARTICLE
...
SUBROUTINE NEAR_SURFACE_GAS_VARIABLES_KERNEL(M,T,SF,BC,B1, &
   LP,TW,WALL_INDEX,PARTICLE_INDEX,THIN_WALL_INDEX)
...
IF (PREDICTOR) THEN
   UU => M%US; VV => M%VS; WW => M%WS
   RHOP => M%RHOS; ZZP => M%ZZS
ELSE
   UU => M%U; VV => M%V; WW => M%W
   RHOP => M%RHO; ZZP => M%ZZ
ENDIF

IF (PRESENT(WALL_INDEX)) THEN

   IF (ABS(SF%T_IGN-T_BEGIN)<=SPACING(SF%T_IGN) .AND. &
       SF%RAMP(TIME_VELO)%INDEX>=1) THEN
      TSI = T
   ELSE
      TSI = T-SF%T_IGN
   ENDIF
   RAMP_FACTOR = EVALUATE_RAMP(TSI, &
      SF%RAMP(TIME_VELO)%INDEX, &
      TAU=SF%RAMP(TIME_VELO)%TAU)
   SELECT CASE(BC%IOR)
      CASE(1,-1)
         VBAR = 0.5_EB*(VV(BC%IIG,BC%JJG,BC%KKG) + &
            VV(BC%IIG,BC%JJG-1,BC%KKG)) - &
            SF%VEL_T(1)*RAMP_FACTOR
         WBAR = 0.5_EB*(WW(BC%IIG,BC%JJG,BC%KKG) + &
            WW(BC%IIG,BC%JJG,BC%KKG-1)) - &
            SF%VEL_T(2)*RAMP_FACTOR
         B1%U_TANG = SQRT(VBAR**2+WBAR**2)
      CASE(2,-2)
         UBAR = 0.5_EB*(UU(BC%IIG,BC%JJG,BC%KKG) + &
            UU(BC%IIG-1,BC%JJG,BC%KKG)) - &
            SF%VEL_T(1)*RAMP_FACTOR
         WBAR = 0.5_EB*(WW(BC%IIG,BC%JJG,BC%KKG) + &
            WW(BC%IIG,BC%JJG,BC%KKG-1)) - &
            SF%VEL_T(2)*RAMP_FACTOR
   ...
   IF (SF%TMP_GAS_FRONT > 0._EB) THEN
      B1%TMP_G = TMPA + EVALUATE_RAMP(T-T_BEGIN, &
         SF%RAMP(TIME_TGF)%INDEX)*(SF%TMP_GAS_FRONT-TMPA)
      B1%Q_RAD_IN = B1%EMISSIVITY*SIGMA*B1%TMP_G**4
   ELSE
      B1%TMP_G = M%TMP(BC%IIG,BC%JJG,BC%KKG)
   ENDIF
   B1%RHO_G = RHOP(BC%IIG,BC%JJG,BC%KKG)
   B1%ZZ_G(1:N_TRACKED_SPECIES) = &
      ZZP(BC%IIG,BC%JJG,BC%KKG,1:N_TRACKED_SPECIES)
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
   ...
   IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      CALL GET_TRILINEAR_WEIGHTS_K(M,BC%IIG,BC%JJG, &
         BC%KKG,BC%X,BC%Y,BC%Z,TLW_IND,TLW)
      IF (SF%TMP_GAS_FRONT < 0._EB) &
         B1%TMP_G = SCALAR_TO_POINT_K(M, &
            TLW_IND,TLW,M%TMP)
      B1%RHO_G = SCALAR_TO_POINT_K(M, &
         TLW_IND,TLW,RHOP)
      DO N=1,N_TRACKED_SPECIES
         B1%ZZ_G(N) = SCALAR_TO_POINT_K(M, &
            TLW_IND,TLW,ZZP(:,:,:,N))
      ENDDO
   ELSE
      IF (SF%TMP_GAS_FRONT < 0._EB) &
      ...
      B1%ZZ_G(1:N_TRACKED_SPECIES) = &
         ZZP(BC%IIG,BC%JJG,BC%KKG,1:N_TRACKED_SPECIES)
      UBAR = 0.5_EB*(UU(BC%IIG,BC%JJG,BC%KKG) + &
         UU(BC%IIG-1,BC%JJG,BC%KKG)) - LP%U
      VBAR = 0.5_EB*(VV(BC%IIG,BC%JJG,BC%KKG) + &
         VV(BC%IIG,BC%JJG-1,BC%KKG)) - LP%V
      WBAR = 0.5_EB*(WW(BC%IIG,BC%JJG,BC%KKG) + &
         WW(BC%IIG,BC%JJG,BC%KKG-1)) - LP%W
...
ELSEIF (PRESENT(THIN_WALL_INDEX)) THEN
   ...
   IF (TW%WALL_INDEX_M>0 .OR. TW%WALL_INDEX_P>0) THEN
      IF (TW%WALL_INDEX_M>0) THEN
         B1M => M%BOUNDARY_PROP1( &
            M%WALL(TW%WALL_INDEX_M)%B1_INDEX)
      ELSE
         B1M => M%BOUNDARY_PROP1( &
            M%WALL(TW%WALL_INDEX_P)%B1_INDEX)
      ENDIF
      IF (TW%WALL_INDEX_P>0) THEN
         B1P => M%BOUNDARY_PROP1( &
            M%WALL(TW%WALL_INDEX_P)%B1_INDEX)
      ELSE
         B1P => M%BOUNDARY_PROP1( &
            M%WALL(TW%WALL_INDEX_M)%B1_INDEX)
...
END SUBROUTINE NEAR_SURFACE_GAS_VARIABLES_KERNEL


!> \brief Trilinear interpolation of a scalar field to a point
!> (kernel version using explicit mesh argument)

REAL(EB) FUNCTION SCALAR_TO_POINT_K(M,TLW_IND,TLW,FIELD)

TYPE(MESH_TYPE), INTENT(IN) :: M
INTEGER, INTENT(IN) :: TLW_IND(1:3)
REAL(EB), INTENT(IN) :: TLW(0:1,0:1,0:1)
REAL(EB), INTENT(IN), DIMENSION( &
   -1:M%IBP1+1,-1:M%JBP1+1,-1:M%KBP1+1) :: FIELD
INTEGER :: II,JJ,KK

SCALAR_TO_POINT_K = 0._EB
DO KK=0,1
   DO JJ=0,1
      DO II=0,1
         SCALAR_TO_POINT_K = SCALAR_TO_POINT_K + &
            TLW(II,JJ,KK) * FIELD( &
               TLW_IND(IAXIS)+II, &
               TLW_IND(JAXIS)+JJ, &
               TLW_IND(KAXIS)+KK)
...
END FUNCTION SCALAR_TO_POINT_K


!> \brief Get trilinear interpolation weights
!> (kernel version using explicit mesh argument)

SUBROUTINE GET_TRILINEAR_WEIGHTS_K(M,IIG,JJG,KKG, &
   P_X,P_Y,P_Z,TLW_IND,TLW)
...
TLW_IND(JAXIS) = JJG
TLW_IND(KAXIS) = KKG
IF (P_X < M%XC(IIG)) TLW_IND(IAXIS) = IIG - 1
IF (P_Y < M%YC(JJG)) TLW_IND(JAXIS) = JJG - 1
IF (P_Z < M%ZC(KKG)) TLW_IND(KAXIS) = KKG - 1

P = (P_X - M%XC(TLW_IND(IAXIS))) / MAX( &
   TWENTY_EPSILON_EB, &
   M%XC(TLW_IND(IAXIS)+1) - M%XC(TLW_IND(IAXIS)))
R = (P_Y - M%YC(TLW_IND(JAXIS))) / MAX( &
   TWENTY_EPSILON_EB, &
   M%YC(TLW_IND(JAXIS)+1) - M%YC(TLW_IND(JAXIS)))
S = (P_Z - M%ZC(TLW_IND(KAXIS))) / MAX( &
   TWENTY_EPSILON_EB, &
   M%ZC(TLW_IND(KAXIS)+1) - M%ZC(TLW_IND(KAXIS)))
...
DO KK=0,1
   DO JJ=0,1
      DO II=0,1
         IF (M%CELL(M%CELL_INDEX( &
            TLW_IND(IAXIS)+II, &
            TLW_IND(JAXIS)+JJ, &
            TLW_IND(KAXIS)+KK))%SOLID) &
...
END SUBROUTINE GET_TRILINEAR_WEIGHTS_K
