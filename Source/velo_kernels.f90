...
RECURSIVE SUBROUTINE VELOCITY_FLUX_KERNEL(M,T,DT,NM,APPLY_TO_ESTIMATED_VARIABLES,GX,GY,GZ,WORK_BRANCH)
...
INTEGER :: I,J,K,IEXP,IEXM,IEYP,IEYM,IEZP,IEZM,IC,IC1,IC2,WORK_BRANCH_VAL
REAL(EB), POINTER, DIMENSION(:,:,:) :: TXY,TXZ,TYZ,OMX,OMY,OMZ,UU,VV,WW,RHOP,DP
REAL(EB), POINTER, DIMENSION(:,:,:) :: WK1,WK2,WK3,WK4,WK5,WK6,WK7,WK8,WK9

! Select WORK scratch arrays based on branch
WORK_BRANCH_VAL = 1
IF (PRESENT(WORK_BRANCH)) WORK_BRANCH_VAL = WORK_BRANCH
IF (WORK_BRANCH_VAL==2) THEN
   WK1=>M%WORK1_B; WK2=>M%WORK2_B; WK3=>M%WORK3_B
   WK4=>M%WORK4_B; WK5=>M%WORK5_B; WK6=>M%WORK6_B
   WK7=>M%WORK7_B; WK8=>M%WORK8_B; WK9=>M%WORK9_B
ELSE
   WK1=>M%WORK1; WK2=>M%WORK2; WK3=>M%WORK3
   WK4=>M%WORK4; WK5=>M%WORK5; WK6=>M%WORK6
   WK7=>M%WORK7; WK8=>M%WORK8; WK9=>M%WORK9
ENDIF
...
RECURSIVE SUBROUTINE VELOCITY_FLUX_BLOCK_KERNEL(M,T,DT,NM,APPLY_TO_ESTIMATED_VARIABLES,K1,K2,WORK_BRANCH)
...
REAL(EB) :: MUX,MUY,MUZ,UP,UM,VP,VM,WP,WM,VTRM,OMXP,OMXM,OMYP,OMYM,OMZP,OMZM,TXYP,TXYM,TXZP,TXZM,TYZP,TYZM, &
            ...
            VOMZ,WOMY,UOMY,VOMX,UOMZ,WOMX, &
            RRHO,TXXP,TXXM,TYYP,TYYM,TZZP,TZZM,DTXXDX,DTYYDY,DTZZDZ
INTEGER :: I,J,K,IEXP,IEXM,IEYP,IEYM,IEZP,IEZM,IC,WORK_BRANCH_VAL
INTEGER :: K1_VORT,K1_FVZ,K2_FVZ
REAL(EB), POINTER, DIMENSION(:,:,:) :: TXY,TXZ,TYZ,OMX,OMY,OMZ,UU,VV,WW,RHOP,DP
REAL(EB), POINTER, DIMENSION(:,:,:) :: WK1,WK2,WK3,WK4,WK5,WK6
...
IF (CC_IBM) CALL CHECK_CFLVN_LINKED_CELLS(M,NM,DT,UVWMAX,R_DX2,MUTRM)
...
RECURSIVE SUBROUTINE VELOCITY_BC_PROCESS_EDGES_KERNEL(M,NM,T,APPLY_TO_ESTIMATED_VARIABLES,K1_IN,K2_IN,DRAG_UVWMAX_LOCAL)
...
INTEGER, INTENT(IN) :: NM
LOGICAL, INTENT(IN) :: APPLY_TO_ESTIMATED_VARIABLES
INTEGER, INTENT(IN), OPTIONAL :: K1_IN, K2_IN
REAL(EB), INTENT(INOUT), OPTIONAL :: DRAG_UVWMAX_LOCAL
...
TYPE(SURFACE_TYPE), POINTER :: SF
INTEGER :: K_FILTER_MIN, K_FILTER_MAX
REAL(EB) :: DRAG_LOCAL
...
! Compute K filtering range for block decomposition
IF (PRESENT(K1_IN)) THEN
   K_FILTER_MIN = MERGE(0, K1_IN, K1_IN==1)  ! Include K=0 boundary edges in first block
ELSE
   K_FILTER_MIN = 0
ENDIF
IF (PRESENT(K2_IN)) THEN
   K_FILTER_MAX = K2_IN
ELSE
   K_FILTER_MAX = HUGE(1)
ENDIF
...
EDGE_LOOP: DO IE=1,EDGE_COUNT(NM)
   ...
   ! K-block filter for intra-mesh parallelism
   IF (ED%K < K_FILTER_MIN .OR. ED%K > K_FILTER_MAX) CYCLE EDGE_LOOP
            ...
            BOUNDARY_CONDITION: SELECT CASE(VELOCITY_BC_INDEX)
               ...
               CASE (BOUNDARY_FUEL_MODEL_BC) BOUNDARY_CONDITION
                  ...
                  MU_DUIDXJ(ICD_SGN) = I_SGN*RHO_WALL*DRAG_FACTOR*VEG_HT*HT_SCALE_FACTOR**2* &
                                       VEL_GAS*VEL_T
                  DRAG_LOCAL = MAX(DRAG_LOCAL,DRAG_FACTOR*HT_SCALE_FACTOR**2*VEL_T)
                  ALTERED_GRADIENT(ICD_SGN) = .TRUE.
...
! Write back DRAG_UVWMAX accumulation
IF (PRESENT(DRAG_UVWMAX_LOCAL)) THEN
   DRAG_UVWMAX_LOCAL = DRAG_LOCAL
ELSE
   M%DRAG_UVWMAX = DRAG_LOCAL
ENDIF
...
!> Thread-safe version of WALL_VELOCITY_NO_GRADH.
!> For sparse pressure solvers (ULMAT/GLMAT/UGLMAT), the pressure gradient at
!> wall cells is incorrect, so this routine overrides velocities there using
!> the formula without the grad(H) term.
!>
!> Three modes:
!>   STORE_UN=.TRUE., CORRECTOR_FLAG=.TRUE.: Store U/V/W at wall cells into M%UN_WALLS_NOGRADH
!>   STORE_UN=.FALSE., PREDICTOR_FLAG=.TRUE.: Recompute US/VS/WS at wall cells (no grad H)
!>   STORE_UN=.FALSE., PREDICTOR_FLAG=.FALSE.: Recompute U/V/W at wall cells using stored values
RECURSIVE SUBROUTINE WALL_VELOCITY_NO_GRADH_KERNEL(M, DT, STORE_UN, PREDICTOR_FLAG)
...
TYPE(MESH_TYPE), INTENT(INOUT), TARGET :: M
REAL(EB), INTENT(IN) :: DT
LOGICAL, INTENT(IN) :: STORE_UN, PREDICTOR_FLAG
INTEGER :: II,JJ,KK,IIG,JJG,KKG,IOR,IW,N_INTERNAL_WALL_CELLS_AUX,IC,ICG,N_TOTAL
REAL(EB) :: VEL_N
TYPE(WALL_TYPE), POINTER :: WC
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
...
N_TOTAL = M%N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS_AUX

STORE_UN_COND : IF ( STORE_UN .AND. .NOT.PREDICTOR_FLAG) THEN

   IF (ALLOCATED(M%UN_WALLS_NOGRADH)) DEALLOCATE(M%UN_WALLS_NOGRADH)
   ALLOCATE( M%UN_WALLS_NOGRADH(1:N_TOTAL) )
   M%UN_WALLS_NOGRADH(:) = 0._EB

   STORE_LOOP : DO IW=1,N_TOTAL
      WC => M%WALL(IW)
      BC => M%BOUNDARY_COORD(WC%BC_INDEX)
      IIG= BC%IIG; JJG= BC%JJG; KKG= BC%KKG; IOR= BC%IOR
      SELECT CASE(IOR)
      CASE( IAXIS); M%UN_WALLS_NOGRADH(IW) = M%U(IIG-1,JJG  ,KKG  )
      CASE(-IAXIS); M%UN_WALLS_NOGRADH(IW) = M%U(IIG  ,JJG  ,KKG  )
      CASE( JAXIS); M%UN_WALLS_NOGRADH(IW) = M%V(IIG  ,JJG-1,KKG  )
      CASE(-JAXIS); M%UN_WALLS_NOGRADH(IW) = M%V(IIG  ,JJG  ,KKG  )
      CASE( KAXIS); M%UN_WALLS_NOGRADH(IW) = M%W(IIG  ,JJG  ,KKG-1)
      CASE(-KAXIS); M%UN_WALLS_NOGRADH(IW) = M%W(IIG  ,JJG  ,KKG  )
      END SELECT
   ENDDO STORE_LOOP
   RETURN
...
PREDICTOR_COND : IF (PREDICTOR_FLAG) THEN

   WALL_CELL_LOOP_1: DO IW=1,N_TOTAL
      WC => M%WALL(IW)
      ...
      IF (WC%BOUNDARY_TYPE==NULL_BOUNDARY .AND. &
          .NOT.M%CELL(ICG)%SOLID .AND. .NOT.M%CELL(IC)%SOLID) CYCLE
      IOR = BC%IOR
      SELECT CASE(IOR)
      CASE( IAXIS); M%US(IIG-1,JJG  ,KKG  ) = M%U(IIG-1,JJG  ,KKG  ) - DT*M%FVX(IIG-1,JJG  ,KKG  )
      CASE(-IAXIS); M%US(IIG  ,JJG  ,KKG  ) = M%U(IIG  ,JJG  ,KKG  ) - DT*M%FVX(IIG  ,JJG  ,KKG  )
      CASE( JAXIS); M%VS(IIG  ,JJG-1,KKG  ) = M%V(IIG  ,JJG-1,KKG  ) - DT*M%FVY(IIG  ,JJG-1,KKG  )
      CASE(-JAXIS); M%VS(IIG  ,JJG  ,KKG  ) = M%V(IIG  ,JJG  ,KKG  ) - DT*M%FVY(IIG  ,JJG  ,KKG  )
      CASE( KAXIS); M%WS(IIG  ,JJG  ,KKG-1) = M%W(IIG  ,JJG  ,KKG-1) - DT*M%FVZ(IIG  ,JJG  ,KKG-1)
      CASE(-KAXIS); M%WS(IIG  ,JJG  ,KKG  ) = M%W(IIG  ,JJG  ,KKG  ) - DT*M%FVZ(IIG  ,JJG  ,KKG  )
      END SELECT
   ENDDO WALL_CELL_LOOP_1

ELSE ! Corrector

   WALL_CELL_LOOP_2: DO IW=1,N_TOTAL
      WC => M%WALL(IW)
      IF (.NOT.(WC%BOUNDARY_TYPE==SOLID_BOUNDARY .OR. WC%BOUNDARY_TYPE==NULL_BOUNDARY .OR. &
                WC%BOUNDARY_TYPE==MIRROR_BOUNDARY)) CYCLE
      BC => M%BOUNDARY_COORD(WC%BC_INDEX)
      ...
      IF (WC%BOUNDARY_TYPE==NULL_BOUNDARY .AND. &
          .NOT.M%CELL(ICG)%SOLID .AND. .NOT.M%CELL(IC)%SOLID) CYCLE
      IOR = BC%IOR
      VEL_N = M%UN_WALLS_NOGRADH(IW)
      SELECT CASE(IOR)
      CASE( IAXIS)
         M%U(IIG-1,JJG  ,KKG  ) = 0.5_EB*(VEL_N + M%US(IIG-1,JJG  ,KKG  ) - DT*M%FVX(IIG-1,JJG  ,KKG  ))
      CASE(-IAXIS)
         M%U(IIG  ,JJG  ,KKG  ) = 0.5_EB*(VEL_N + M%US(IIG  ,JJG  ,KKG  ) - DT*M%FVX(IIG  ,JJG  ,KKG  ))
      CASE( JAXIS)
         M%V(IIG  ,JJG-1,KKG  ) = 0.5_EB*(VEL_N + M%VS(IIG  ,JJG-1,KKG  ) - DT*M%FVY(IIG  ,JJG-1,KKG  ))
      CASE(-JAXIS)
         M%V(IIG  ,JJG  ,KKG  ) = 0.5_EB*(VEL_N + M%VS(IIG  ,JJG  ,KKG  ) - DT*M%FVY(IIG  ,JJG  ,KKG  ))
      CASE( KAXIS)
         M%W(IIG  ,JJG  ,KKG-1) = 0.5_EB*(VEL_N + M%WS(IIG  ,JJG  ,KKG-1) - DT*M%FVZ(IIG  ,JJG  ,KKG-1))
      CASE(-KAXIS)
         M%W(IIG  ,JJG  ,KKG  ) = 0.5_EB*(VEL_N + M%WS(IIG  ,JJG  ,KKG  ) - DT*M%FVZ(IIG  ,JJG  ,KKG  ))
      END SELECT
   ENDDO WALL_CELL_LOOP_2

   DEALLOCATE(M%UN_WALLS_NOGRADH)

ENDIF PREDICTOR_COND

END SUBROUTINE WALL_VELOCITY_NO_GRADH_KERNEL
