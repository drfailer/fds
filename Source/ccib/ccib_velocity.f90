...
RECURSIVE SUBROUTINE CC_BAROCLINIC_CORRECTION(T,NM,M)

! Thread-safe: uses local shadows (Pattern 3) instead of module-level aliases.
! CONTAINS helpers inherit shadows via host association.
...
REAL(EB):: PRFCT,WG1,WG2,IDX,AT,FRC_B
LOGICAL :: FLG
REAL(EB), POINTER, DIMENSION(:,:,:) :: HP,RHOP

! Local shadows of module-level aliases:
REAL(EB), POINTER, DIMENSION(:,:,:) :: RHO,RHOS,H,HS,KRES
REAL(EB), POINTER, DIMENSION(:,:,:) :: FVX,FVY,FVZ,FVX_B,FVY_B,FVZ_B
REAL(EB), POINTER, DIMENSION(:) :: RDXN,RDYN,RDZN,DX,DY,DZ,X,Y,Z
...
! Setup shadows:
RHO => M%RHO; RHOS => M%RHOS; H => M%H; HS => M%HS; KRES => M%KRES
FVX => M%FVX; FVY => M%FVY; FVZ => M%FVZ
FVX_B => M%FVX_B; FVY_B => M%FVY_B; FVZ_B => M%FVZ_B
RDXN => M%RDXN; RDYN => M%RDYN; RDZN => M%RDZN
DX => M%DX; DY => M%DY; DZ => M%DZ
X => M%X; Y => M%Y; Z => M%Z
CUT_FACE => M%CUT_FACE; CUT_CELL => M%CUT_CELL; RC_FACE => M%RC_FACE
WALL => M%WALL; EXTERNAL_WALL => M%EXTERNAL_WALL
...
CF_LOOP : DO ICF=1,M%N_CUTFACE_MESH
   CF => CUT_FACE(ICF); IF (CF%STATUS/=CC_GASPHASE) CYCLE CF_LOOP
   IF (ICF<=M%N_BBCUTFACE_MESH) THEN
   ...
   DO JCF=1,CF%NFACE
      ...
      CALL GET_CUTFACE_BARO_TORQUE(PRFCT,CF,JCF,IDX,WG1,WG2,II1,JJ1,KK1,II2,JJ2,KK2,CF%FN_B(JCF),HP)
   ENDDO
   CF%FV_B = DOT_PRODUCT(CF%FN_B(1:CF%NFACE),CF%AREA(1:CF%NFACE)) / AT
ENDDO CF_LOOP

! 2. Get Baroclinic torque terms for RC-faces:
RC_LOOP : DO ICF=1,M%CC_NRCFACE_Z
   RCF => RC_FACE(ICF);
   ...
   IF (RCF%IWC>0) THEN
      ...
      ENDIF
   ENDIF
   CALL GET_RCFACE_BARO_TORQUE(PRFCT,ICF,WG1,WG2,FRC_B,HP,RHOP)
...
CALL GET_LINKED_FV(M,NM,DO_BAROCLINIC=.TRUE.)
...
! Thread-safe CONTAINS — inherits CUT_CELL, KRES via host association.
SUBROUTINE GET_CUTFACE_BARO_TORQUE(PRFCT,CF,JCF,IDX,WG1,WG2,II1,JJ1,KK1,II2,JJ2,KK2,FCF_B,HP)
...
END SUBROUTINE GET_CUTFACE_BARO_TORQUE

! Thread-safe CONTAINS — inherits RC_FACE, CUT_CELL, X, Y, Z, RDXN, RDYN, RDZN, KRES via host association.
SUBROUTINE GET_RCFACE_BARO_TORQUE(PRFCT,ICF,WG1,WG2,FRC_B,HP,RHOP)
REAL(EB),INTENT(IN)   :: PRFCT
...
END SUBROUTINE GET_RCFACE_BARO_TORQUE
...
PRFCT =  0._EB; IF (MESHES(NM)%PREDICTOR) PRFCT =  1._EB
...
ELSE STORE_IF
   ...
   PRED_CORR_IF : IF (MESHES(NM)%PREDICTOR) THEN
...
RECURSIVE SUBROUTINE CC_MATCH_VELOCITY_FLUX(NM,M)

! Thread-safe: receives M parameter, uses M% member access.
...
TYPE(CC_CUTFACE_TYPE), POINTER :: CF
! Thread-safe: uses M parameter instead of POINT_TO_MESH
...
EXTERNAL_WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS
   ...
   SELECT CASE(IOR)
      ...
      CASE( 1)
         ...
         ICF = M%FCVAR(0,JJ,KK,CC_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
            ...
            ENDDO
         ENDDO
         IF(ICF<1) M%FVX(0,JJ,KK) = 0.5_EB*(M%FVX(0,JJ,KK) + FN_OTHER/DA_OTHER)

      CASE(-1)

         ICF = M%FCVAR(M%IBAR,JJ,KK,CC_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
            ...
            ENDDO
         ENDDO
         IF(ICF<1) M%FVX(M%IBAR,JJ,KK) = 0.5_EB*(M%FVX(M%IBAR,JJ,KK) + FN_OTHER/DA_OTHER)

      CASE( 2)

         ICF = M%FCVAR(II,0,KK,CC_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
            ...
            ENDDO
         ENDDO
         IF(ICF<1) M%FVY(II,0,KK) = 0.5_EB*(M%FVY(II,0,KK) + FN_OTHER/DA_OTHER)

      CASE(-2)

         ICF = M%FCVAR(II,M%JBAR,KK,CC_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
          ...
          ENDDO
       ENDDO
       IF(ICF<1) M%FVY(II,M%JBAR,KK) = 0.5_EB*(M%FVY(II,M%JBAR,KK) + FN_OTHER/DA_OTHER)


      CASE( 3)

         ICF = M%FCVAR(II,JJ,0,CC_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
            ...
            ENDDO
         ENDDO
         IF(ICF<1) M%FVZ(II,JJ,0) = 0.5_EB*(M%FVZ(II,JJ,0) + FN_OTHER/DA_OTHER)

      CASE(-3)

         ICF = M%FCVAR(II,JJ,M%KBAR,CC_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
            ...
            ENDDO
         ENDDO
         IF(ICF<1) M%FVZ(II,JJ,M%KBAR) = 0.5_EB*(M%FVZ(II,JJ,M%KBAR) + FN_OTHER/DA_OTHER)
...
REAL(EB) :: DA_OTHER,VEL_OTHER,VEL_LNK_OTHER,PRFCT,NOM_CELLS
...
PREDICT_NORMALS: IF (M%PREDICTOR) THEN
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   CALL POINT_TO_MESH(NM)
   CALL GET_LINKED_VELOCITIES(MESHES(NM),NM,MESHES(NM)%PREDICTOR,CMP_FLG=.TRUE.)
ENDDO

! Flux match Cartesian face velocity back to cut-faces:
CALL CC_VELOCITY_CUTFACES(MESHES(LOWER_MESH_INDEX)%PREDICTOR)

IF (DEBUG_CC_SCALAR_TRANSPORT) THEN
   DUMLOG = DIAGNOSTICS
   IF (MESHES(LOWER_MESH_INDEX)%PREDICTOR) CALL CC_CHECK_DIVERGENCE(T,DT,.TRUE.)
   IF (MESHES(LOWER_MESH_INDEX)%CORRECTOR) CALL CC_CHECK_DIVERGENCE(T,DT,.FALSE.)
ELSE
   IF (MESHES(LOWER_MESH_INDEX)%CORRECTOR .AND. (DIAGNOSTICS .OR. STORE_CUTCELL_DIVERGENCE)) &
      CALL CC_CHECK_DIVERGENCE(T,DT,.FALSE.)
ENDIF
...
SUBROUTINE GET_LINKED_FV(M,NM,DO_BAROCLINIC)

! Thread-safe: uses M% member access instead of module-level aliases.
...
ALLOCATE(FLINK(0:M%NUNK_F),ALINK(0:M%NUNK_F)); FLINK = 0._EB; ALINK = 0._EB

IF (DO_BAROCLINIC) THEN
   FVXP => M%FVX_B; FVYP => M%FVY_B; FVZP => M%FVZ_B
ELSE
   FVXP => M%FVX  ; FVYP => M%FVY  ; FVZP => M%FVZ
ENDIF
...
DO K=0,M%KBAR
   DO J=0,M%JBAR
      DO I=0,M%IBAR
         ! X face:
         IF (M%FCVAR(I,J,K,CC_UNKF,IAXIS)>0) THEN
            FLINK(M%FCVAR(I,J,K,CC_UNKF,IAXIS)) = FLINK(M%FCVAR(I,J,K,CC_UNKF,IAXIS)) + FVXP(I,J,K)*M%DY(J)*M%DZ(K)
            ALINK(M%FCVAR(I,J,K,CC_UNKF,IAXIS)) = ALINK(M%FCVAR(I,J,K,CC_UNKF,IAXIS)) + M%DY(J)*M%DZ(K)
         ENDIF
         ! Y face:
         IF (M%FCVAR(I,J,K,CC_UNKF,JAXIS)>0) THEN
            FLINK(M%FCVAR(I,J,K,CC_UNKF,JAXIS)) = FLINK(M%FCVAR(I,J,K,CC_UNKF,JAXIS)) + FVYP(I,J,K)*M%DX(I)*M%DZ(K)
            ALINK(M%FCVAR(I,J,K,CC_UNKF,JAXIS)) = ALINK(M%FCVAR(I,J,K,CC_UNKF,JAXIS)) + M%DX(I)*M%DZ(K)
         ENDIF
         ! Z face:
         IF (M%FCVAR(I,J,K,CC_UNKF,KAXIS)>0) THEN
            FLINK(M%FCVAR(I,J,K,CC_UNKF,KAXIS)) = FLINK(M%FCVAR(I,J,K,CC_UNKF,KAXIS)) + FVZP(I,J,K)*M%DX(I)*M%DY(J)
            ALINK(M%FCVAR(I,J,K,CC_UNKF,KAXIS)) = ALINK(M%FCVAR(I,J,K,CC_UNKF,KAXIS)) + M%DX(I)*M%DY(J)
         ENDIF
      ENDDO
   ENDDO
ENDDO
...
DO ICF=1,M%CC_NRCFACE_Z
   ...
   SELECT CASE(X1AXIS)
   CASE(IAXIS)
      FLINK(M%RC_FACE(ICF)%UNKF) = FLINK(M%RC_FACE(ICF)%UNKF) + FVXP(I,J,K)*M%DY(J)*M%DZ(K)
      ALINK(M%RC_FACE(ICF)%UNKF) = ALINK(M%RC_FACE(ICF)%UNKF) + M%DY(J)*M%DZ(K)
   CASE(JAXIS)
      FLINK(M%RC_FACE(ICF)%UNKF) = FLINK(M%RC_FACE(ICF)%UNKF) + FVYP(I,J,K)*M%DX(I)*M%DZ(K)
      ALINK(M%RC_FACE(ICF)%UNKF) = ALINK(M%RC_FACE(ICF)%UNKF) + M%DX(I)*M%DZ(K)
   CASE(KAXIS)
      FLINK(M%RC_FACE(ICF)%UNKF) = FLINK(M%RC_FACE(ICF)%UNKF) + FVZP(I,J,K)*M%DX(I)*M%DY(J)
      ALINK(M%RC_FACE(ICF)%UNKF) = ALINK(M%RC_FACE(ICF)%UNKF) + M%DX(I)*M%DY(J)
   END SELECT
ENDDO
! Cut-faces:
IF(DO_BAROCLINIC) THEN
...
DO K=0,M%KBAR
   DO J=0,M%JBAR
      DO I=0,M%IBAR
         IF (M%FCVAR(I,J,K,CC_UNKF,IAXIS)>0)                  FVXP(I,J,K) = FLINK(M%FCVAR(I,J,K,CC_UNKF,IAXIS))
         IF (M%FCVAR(I,J,K,CC_UNKF,JAXIS)>0 .AND. .NOT.TWO_D) FVYP(I,J,K) = FLINK(M%FCVAR(I,J,K,CC_UNKF,JAXIS))
         IF (M%FCVAR(I,J,K,CC_UNKF,KAXIS)>0)                  FVZP(I,J,K) = FLINK(M%FCVAR(I,J,K,CC_UNKF,KAXIS))
      ENDDO
   ENDDO
ENDDO
DO ICF=1,M%CC_NRCFACE_Z
   ...
   X1AXIS = M%RC_FACE(ICF)%IJK(KAXIS+1)
   SELECT CASE(X1AXIS)
   CASE(IAXIS); FVXP(I,J,K) = FLINK(M%RC_FACE(ICF)%UNKF)
   CASE(JAXIS); IF(.NOT.TWO_D) FVYP(I,J,K) = FLINK(M%RC_FACE(ICF)%UNKF)
   CASE(KAXIS); FVZP(I,J,K) = FLINK(M%RC_FACE(ICF)%UNKF)
   END SELECT
ENDDO
IF(DO_BAROCLINIC) THEN
   DO ICF=1,M%N_CUTFACE_MESH
...
RECURSIVE SUBROUTINE CC_NO_FLUX(DT,NM,FORCE_FLG,M)
...
REAL(EB):: U_IBM,V_IBM,W_IBM,DUUDT,DVVDT,DWWDT,RFODT,UN,AFCT,IDX
...
IF (M%PREDICTOR) HP => M%H
IF (M%CORRECTOR) HP => M%HS

FORCE_IF : IF (FORCE_FLG) THEN

   U_IBM = 0._EB; V_IBM = 0._EB; W_IBM = 0._EB
   IF (PRESSURE_ITERATIONS==1) THEN
      CUTFACE_LOOP_2A : DO ICF=1,M%N_CUTFACE_MESH
         CF => M%CUT_FACE(ICF); IF ( CF%STATUS /= CC_GASPHASE) CYCLE CUTFACE_LOOP_2A
         I      = CF%IJK(IAXIS)
         J      = CF%IJK(JAXIS)
         K      = CF%IJK(KAXIS)
         X1AXIS = CF%IJK(KAXIS+1)
         SELECT CASE(X1AXIS)
               CASE(IAXIS)
                  ...
                  IDX = M%RDXN(I)
                  DO JCF=1,CF%NFACE
                     IF (M%PREDICTOR) DUUDT = (U_IBM-CF%VEL(JCF))/DT
                     IF (M%CORRECTOR) DUUDT = (2._EB*U_IBM-(CF%VEL(JCF)+CF%VELS(JCF)))/DT
                     IDX=1._EB/(CF%XCENHIGH(X1AXIS,JCF) - CF%XCENLOW( X1AXIS,JCF))
               ...
               CASE(JAXIS)
                  ...
                  IDX = M%RDYN(J)
                  DO JCF=1,CF%NFACE
                     IF (M%PREDICTOR) DVVDT = (V_IBM-CF%VEL(JCF))/DT
                     IF (M%CORRECTOR) DVVDT = (2._EB*V_IBM-(CF%VEL(JCF)+CF%VELS(JCF)))/DT
                     IDX=1._EB/(CF%XCENHIGH(X1AXIS,JCF) - CF%XCENLOW( X1AXIS,JCF))
               ...
               CASE(KAXIS)
                  ...
                  IDX = M%RDZN(K)
                  DO JCF=1,CF%NFACE
                     IF (M%PREDICTOR) DWWDT = (W_IBM-CF%VEL(JCF))/DT
                     IF (M%CORRECTOR) DWWDT = (2._EB*W_IBM-(CF%VEL(JCF)+CF%VELS(JCF)))/DT
                     IDX=1._EB/(CF%XCENHIGH(X1AXIS,JCF) - CF%XCENLOW( X1AXIS,JCF))
...
CFACE_LOOP : DO ICF=1,M%N_EXTERNAL_CFACE_CELLS+M%N_INTWALL_CFACE_CELLS+M%N_INTERNAL_CFACE_CELLS
   CFA  =>M%CFACE(ICF)
   IW   =M%CUT_FACE(CFA%CUT_FACE_IND1)%IWC ! Index of underlaying wall cell.
   ...
   IF ( .NOT.(CFA%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY .OR. CFA%BOUNDARY_TYPE==PERIODIC_BOUNDARY) ) THEN
      NOM=0; IF(ICF<=M%N_EXTERNAL_CFACE_CELLS) NOM = M%EXTERNAL_WALL(IW)%NOM
      II  = BC%II
      JJ  = BC%JJ
      KK  = BC%KK
      ...
      IF (NOM/=0 .OR. ANY(CFA%BOUNDARY_TYPE==(/SOLID_BOUNDARY,NULL_BOUNDARY,MIRROR_BOUNDARY/))) THEN
         AFCT = 1._EB
         IF(ICF<=M%N_EXTERNAL_CFACE_CELLS+M%N_INTWALL_CFACE_CELLS) AFCT = -SIGN(1._EB,REAL(IOR,EB))
         IF (M%PREDICTOR) THEN
            UN    = AFCT*B1%U_NORMAL_S
            DUUDT = RFODT*(UN-M%CUT_FACE(IFACE)%VEL(JFACE))
         ELSE
            UN = AFCT*B1%U_NORMAL
            DUUDT = 2._EB*RFODT*(UN-0.5_EB*(M%CUT_FACE(IFACE)%VEL(JFACE)+M%CUT_FACE(IFACE)%VELS(JFACE)))
         ENDIF
         M%CUT_FACE(IFACE)%FN(JFACE) = - DUUDT
      ENDIF
   ENDIF
ENDDO CFACE_LOOP

! Now set WALL_WORK(IW) to zero in EXTERNAL M%WALL CELLS of type CC_SOLID:
! This Follows what is being done for external boundaries inside OBSTS (NULL_BOUNDARY).
EWC_LOOP : DO IW=1,M%N_EXTERNAL_WALL_CELLS
   WC => M%WALL(IW)
   ...
   SELECT CASE(IOR)
   CASE( IAXIS)
      IF(M%FCVAR(II  ,JJ,KK,CC_FGSC,IAXIS)==CC_SOLID) M%WALL_WORK1(IW) = 0._EB
   CASE(-IAXIS)
      IF(M%FCVAR(II-1,JJ,KK,CC_FGSC,IAXIS)==CC_SOLID) M%WALL_WORK1(IW) = 0._EB
   CASE( JAXIS)
      IF(M%FCVAR(II,JJ  ,KK,CC_FGSC,JAXIS)==CC_SOLID) M%WALL_WORK1(IW) = 0._EB
   CASE(-JAXIS)
      IF(M%FCVAR(II,JJ-1,KK,CC_FGSC,JAXIS)==CC_SOLID) M%WALL_WORK1(IW) = 0._EB
   CASE( KAXIS)
      IF(M%FCVAR(II,JJ,KK  ,CC_FGSC,KAXIS)==CC_SOLID) M%WALL_WORK1(IW) = 0._EB
   CASE(-KAXIS)
      IF(M%FCVAR(II,JJ,KK-1,CC_FGSC,KAXIS)==CC_SOLID) M%WALL_WORK1(IW) = 0._EB
   END SELECT
...
RECURSIVE SUBROUTINE CC_COMPUTE_VELOCITY_ERROR(DT,NM,M)
...
REAL(EB):: ITERATIVE_FACTOR, UN_NEW, UN_NEW_OTHER, VELOCITY_ERROR, IDX, DUDT, DVDT, DWDT, DA_OTHER, AREA_FC
...
! The corresponding M%WALL_WORK1(IW) term for pressure boundary conditions is updated where necessary.
IF (M%PREDICTOR) THEN
...
CHECK_WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS
   ...
   IF(ALL((/M%CCVAR(IIG,JJG,KKG,CC_CGSC),M%CCVAR(II,JJ,KK,CC_CGSC)/)==CC_GASPHASE)) CYCLE CHECK_WALL_LOOP
   ...
   IF(WC%CUT_FACE_INDEX>0) THEN
      CF => M%CUT_FACE(WC%CUT_FACE_INDEX)
      ...
      IF (M%PREDICTOR) THEN
         ...
         CASE(IAXIS)
            DO JCF=1,CF%NFACE
               IDX = 1._EB/(CF%XCENHIGH(X1AXIS,JCF)-CF%XCENLOW(X1AXIS,JCF))
               UN_NEW = UN_NEW + (CF%VEL(JCF)-DT*(CF%FN(JCF)+IDX*(M%H(I+1,J,K)-M%H(I,J,K))))*CF%AREA(JCF)
            ENDDO
         CASE(JAXIS)
            DO JCF=1,CF%NFACE
               IDX = 1._EB/(CF%XCENHIGH(X1AXIS,JCF)-CF%XCENLOW(X1AXIS,JCF))
               UN_NEW = UN_NEW + (CF%VEL(JCF)-DT*(CF%FN(JCF)+IDX*(M%H(I,J+1,K)-M%H(I,J,K))))*CF%AREA(JCF)
            ENDDO
         CASE(KAXIS)
            DO JCF=1,CF%NFACE
               IDX = 1._EB/(CF%XCENHIGH(X1AXIS,JCF)-CF%XCENLOW(X1AXIS,JCF))
               UN_NEW = UN_NEW + (CF%VEL(JCF)-DT*(CF%FN(JCF)+IDX*(M%H(I,J,K+1)-M%H(I,J,K))))*CF%AREA(JCF)
            ENDDO
         END SELECT
      ELSE
         SELECT CASE(X1AXIS)
         CASE(IAXIS)
            DO JCF=1,CF%NFACE
               IDX = 1._EB/(CF%XCENHIGH(X1AXIS,JCF)-CF%XCENLOW(X1AXIS,JCF))
               UN_NEW = UN_NEW + 0.5_EB*(CF%VEL(JCF)+CF%VELS(JCF)-DT*(CF%FN(JCF)+IDX*(M%HS(I+1,J,K)-M%HS(I,J,K))))*CF%AREA(JCF)
            ENDDO
         CASE(JAXIS)
            DO JCF=1,CF%NFACE
               IDX = 1._EB/(CF%XCENHIGH(X1AXIS,JCF)-CF%XCENLOW(X1AXIS,JCF))
               UN_NEW = UN_NEW + 0.5_EB*(CF%VEL(JCF)+CF%VELS(JCF)-DT*(CF%FN(JCF)+IDX*(M%HS(I,J+1,K)-M%HS(I,J,K))))*CF%AREA(JCF)
            ENDDO
         CASE(KAXIS)
            DO JCF=1,CF%NFACE
               IDX = 1._EB/(CF%XCENHIGH(X1AXIS,JCF)-CF%XCENLOW(X1AXIS,JCF))
               UN_NEW = UN_NEW + 0.5_EB*(CF%VEL(JCF)+CF%VELS(JCF)-DT*(CF%FN(JCF)+IDX*(M%HS(I,J,K+1)-M%HS(I,J,K))))*CF%AREA(JCF)
            ENDDO
         END SELECT
   ...
   ELSE
      ...
      SELECT CASE(X1AXIS)
      CASE(IAXIS)
         ICC_LO = M%CCVAR(I,J,K,CC_IDCC); ICC_HI = M%CCVAR(I+1,J,K,CC_IDCC)
         IF (ICC_LO>0) THEN; X_LO=CUT_CELL(ICC_LO)%XYZCEN(IAXIS,1); ELSE; X_LO=M%XC(I); ENDIF
         IF (ICC_HI>0) THEN; X_HI=CUT_CELL(ICC_HI)%XYZCEN(IAXIS,1); ELSE; X_HI=M%XC(I+1); ENDIF
         IDX = 1._EB/(X_HI - X_LO)
         IF (M%PREDICTOR) THEN
            UN_NEW = M%U(I,J,K) - DT*(M%FVX(I,J,K) + IDX*(M%H(I+1,J,K)-M%H(I,J,K)))
         ELSE
            UN_NEW = 0.5_EB*( (M%U(I,J,K)+M%US(I,J,K)) - DT*(M%FVX(I,J,K) + IDX*(M%HS(I+1,J,K)-M%HS(I,J,K))) )
         ENDIF
      CASE(JAXIS)
         ICC_LO = M%CCVAR(I,J,K,CC_IDCC); ICC_HI = M%CCVAR(I,J+1,K,CC_IDCC)
         IF (ICC_LO>0) THEN; X_LO=CUT_CELL(ICC_LO)%XYZCEN(JAXIS,1); ELSE; X_LO=M%YC(J); ENDIF
         IF (ICC_HI>0) THEN; X_HI=CUT_CELL(ICC_HI)%XYZCEN(JAXIS,1); ELSE; X_HI=M%YC(J+1); ENDIF
         IDX = 1._EB/(X_HI - X_LO)
         IF (M%PREDICTOR) THEN
            UN_NEW = M%V(I,J,K) - DT*(M%FVY(I,J,K) + IDX*(M%H(I,J+1,K)-M%H(I,J,K)))
         ELSE
            UN_NEW = 0.5_EB*( (M%V(I,J,K)+M%VS(I,J,K)) - DT*(M%FVY(I,J,K) + IDX*(M%HS(I,J+1,K)-M%HS(I,J,K))) )
         ENDIF
      CASE(KAXIS)
         ICC_LO = M%CCVAR(I,J,K,CC_IDCC); ICC_HI = M%CCVAR(I,J,K+1,CC_IDCC)
         IF (ICC_LO>0) THEN; X_LO=CUT_CELL(ICC_LO)%XYZCEN(KAXIS,1); ELSE; X_LO=M%ZC(K); ENDIF
         IF (ICC_HI>0) THEN; X_HI=CUT_CELL(ICC_HI)%XYZCEN(KAXIS,1); ELSE; X_HI=M%ZC(K+1); ENDIF
         IDX = 1._EB/(X_HI - X_LO)
         IF (M%PREDICTOR) THEN
            UN_NEW = M%W(I,J,K) - DT*(M%FVZ(I,J,K) + IDX*(M%H(I,J,K+1)-M%H(I,J,K)))
         ELSE
            UN_NEW = 0.5_EB*( (M%W(I,J,K)+M%WS(I,J,K)) - DT*(M%FVZ(I,J,K) + IDX*(M%HS(I,J,K+1)-M%HS(I,J,K))) )
         ENDIF
      END SELECT
   ENDIF
   ...
   M%WALL_WORK1(IW) = -SIGN(1._EB,REAL(IOR,EB))*ITERATIVE_FACTOR*VELOCITY_ERROR/(IDX*DT)
