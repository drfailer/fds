...
RECURSIVE SUBROUTINE CC_DIVERGENCE_PART_1(T,DT,NM,M)
...
! Thread-safe local shadows (Pattern 3 — inherited by CONTAINS via host association)
...
REAL(EB), POINTER, DIMENSION(:) :: DX, DY, DZ, XC, ZC, UVW_SAVE
REAL(EB), POINTER, DIMENSION(:,:) :: PBAR, PBAR_S, R_PBAR
REAL(EB), POINTER, DIMENSION(:,:,:) :: D, DS, RHO, RHOS, TMP
REAL(EB), POINTER, DIMENSION(:,:,:) :: U, US, V, VS, W, WS
REAL(EB), POINTER, DIMENSION(:,:,:) :: Q, QR, MU, MU_DNS, RSUM, D_SOURCE
REAL(EB), POINTER, DIMENSION(:,:,:) :: WORK1, WORK5, WORK7, WORK9
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: DEL_RHO_D_DEL_Z, M_DOT_PPP, ZZ, ZZS
...
! CC_SCALARS_DATA shadows (convenience pointers, thread-local)
...
! Workspace shadows (thread-local TARGET arrays)
...
REAL(EB) :: RDT,CCM1,CCP1,IDX,AF,TMP_G,H_S,RHOPV(-2:1),TMPV(-1:0),X1F,PRFCT,PRFCTV, &
...
! Thread-safe alias setup (replaces CALL POINT_TO_MESH)
...
XC => M%XC; ZC => M%ZC
UVW_SAVE => M%UVW_SAVE
PBAR => M%PBAR; PBAR_S => M%PBAR_S; R_PBAR => M%R_PBAR
D => M%D; DS => M%DS
RHO => M%RHO; RHOS => M%RHOS; TMP => M%TMP
U => M%U; US => M%US; V => M%V; VS => M%VS; W => M%W; WS => M%WS
Q => M%Q; QR => M%QR
MU => M%MU; MU_DNS => M%MU_DNS; RSUM => M%RSUM
D_SOURCE => M%D_SOURCE
WORK1 => M%WORK1; WORK5 => M%WORK5; WORK7 => M%WORK7; WORK9 => M%WORK9
DEL_RHO_D_DEL_Z => M%DEL_RHO_D_DEL_Z; M_DOT_PPP => M%M_DOT_PPP
ZZ => M%ZZ; ZZS => M%ZZS
...
SPECIES_GT_1_IF: IF (N_TOTAL_SCALARS>1) THEN
   ...
   ! at face k.
   CALL CC_DIFFUSIVE_MASS_FLUXES(NM,M)
      ...
      SELECT CASE(X1AXIS)
         CASE(IAXIS)
            ...
            DO ISIDE=-1,0
               ...
               CASE(CC_FTYPE_CFGAS) ! Cut-cell
                  ICC = RCF%CELL_LIST(2,ISIDE+2)
                  IF (ICC > M%N_CUTCELL_MESH) CYCLE ! Cut-cell is guard-cell cc.
                  JCC = RCF%CELL_LIST(3,ISIDE+2)
                  CUT_CELL(ICC)%U_DOT_DEL_RHO_Z_VOL(N,JCC) = CUT_CELL(ICC)%U_DOT_DEL_RHO_Z_VOL(N,JCC) + &
                  FCT*SGNFCT*(FN_ZZ-RHO_Z_PV(ISIDE))*VELC * AF ! +ve or -ve dot
               END SELECT
...
DO ICF = 1,M%N_CUTFACE_MESH
   IF ( CUT_FACE(ICF)%STATUS /= CC_GASPHASE .OR. M%CUT_FACE(ICF)%IWC<1) CYCLE
   IW = CUT_FACE(ICF)%IWC
   IF( WALL(IW)%BOUNDARY_TYPE==NULL_BOUNDARY ) CYCLE
   WC => WALL(IW)
   B1 => BOUNDARY_PROP1(WC%B1_INDEX)
...
RECURSIVE SUBROUTINE CC_DIFFUSIVE_MASS_FLUXES(NM,M)
...
INTEGER, POINTER :: INTERNAL_CFACE_CELLS_LB, N_INTERNAL_CFACE_CELLS
INTEGER, POINTER :: N_EXTERNAL_CFACE_CELLS, N_INTWALL_CFACE_CELLS
INTEGER, POINTER, DIMENSION(:,:,:) :: CELL_INDEX
INTEGER, POINTER, DIMENSION(:,:,:,:) :: CCVAR
REAL(EB), POINTER, DIMENSION(:) :: DX, DY, DZ
REAL(EB), POINTER, DIMENSION(:,:,:) :: RHO, RHOS, TMP, MU, MU_DNS
...
! Thread-safe: receives M parameter, uses local shadows instead of POINT_TO_MESH.
