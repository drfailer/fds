...
PUBLIC INIT_RADIATION,COMPUTE_RADIATION,COMPUTE_RADIATION_KERNEL,BLACKBODY_FRACTION
...
! Wrapper: calls POINT_TO_MESH then delegates to thread-safe kernel.
...
CALL COMPUTE_RADIATION_KERNEL(MESHES(NM), NM, T, RAD_ITER, RAD_Q_SUM, KFST4_SUM)
T_USED(9)=T_USED(9)+CURRENT_TIME()-TNOW

END SUBROUTINE COMPUTE_RADIATION


RECURSIVE SUBROUTINE COMPUTE_RADIATION_KERNEL(M,NM,T,RAD_ITER, &
   RAD_Q_SUM_OUT,KFST4_SUM_OUT,WORK_BRANCH)

! Thread-safe kernel for COMPUTE_RADIATION.
! Uses local pointer aliases instead of POINT_TO_MESH so that
! multiple threads can execute simultaneously for different meshes.
! CONTAINS subroutines access these aliases through host association.
...
INTEGER, INTENT(IN) :: NM,RAD_ITER
REAL(EB), INTENT(IN) :: T
REAL(EB), INTENT(INOUT) :: RAD_Q_SUM_OUT, KFST4_SUM_OUT
INTEGER, INTENT(IN), OPTIONAL :: WORK_BRANCH

! Host-scope pointer used by CONTAINS subroutines
REAL(EB), POINTER, DIMENSION(:,:,:) :: KFST4_GAS

! --- Local pointer aliases (shadow MESH_POINTERS module variables) ---

! 3D real arrays
REAL(EB), POINTER, DIMENSION(:,:,:) :: QR,Q,CHI_R,TMP,RHO,UII
REAL(EB), POINTER, DIMENSION(:,:,:) :: KAPPA_GAS
REAL(EB), POINTER, DIMENSION(:,:,:) :: WORK1,WORK2,WORK3,WORK4
REAL(EB), POINTER, DIMENSION(:,:,:) :: WORK5,WORK6,WORK7,WORK8,WORK9
REAL(EB), POINTER, DIMENSION(:,:,:) :: RSUM,H,KRES
REAL(EB), POINTER, DIMENSION(:,:,:) :: QR_W
REAL(EB), POINTER, DIMENSION(:,:,:) :: RADIATION_EMISSION
REAL(EB), POINTER, DIMENSION(:,:,:) :: RADIATION_ABSORPTION

! 2D real arrays
REAL(EB), POINTER, DIMENSION(:,:) :: PBAR,CELL_ILW

! 4D real arrays
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZ,UIID
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: AVG_DROP_AREA,AVG_DROP_RAD
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: AVG_DROP_TMP

! 1D real arrays
REAL(EB), POINTER, DIMENSION(:) :: R,DX,DY,DZ,XC,YC,ZC
REAL(EB), POINTER, DIMENSION(:) :: TMP_0,RHO_0,P_0
REAL(EB), POINTER, DIMENSION(:) :: WALL_WORK1,WALL_WORK2
REAL(EB), POINTER, DIMENSION(:) :: FACE_WORK1,FACE_WORK2,FACE_WORK3
...
TYPE(LAGRANGIAN_PARTICLE_TYPE), POINTER, DIMENSION(:) :: &
   LAGRANGIAN_PARTICLE
...
! Scalar integers and logicals
INTEGER, POINTER :: IBAR,JBAR,KBAR,IBP1,JBP1,KBP1
INTEGER, POINTER :: N_INTERNAL_WALL_CELLS,N_EXTERNAL_WALL_CELLS
INTEGER, POINTER :: N_INTERNAL_CFACE_CELLS,INTERNAL_CFACE_CELLS_LB
INTEGER, POINTER :: NLP,N_RADF
...
QR => M%QR; Q => M%Q; CHI_R => M%CHI_R
TMP => M%TMP; RHO => M%RHO; UII => M%UII
KAPPA_GAS => M%KAPPA_GAS
WORK_BRANCH_VAL = 1
IF (PRESENT(WORK_BRANCH)) WORK_BRANCH_VAL = WORK_BRANCH
IF (WORK_BRANCH_VAL==2) THEN
   WORK1 => M%WORK1_B; WORK2 => M%WORK2_B; WORK3 => M%WORK3_B
   WORK4 => M%WORK4_B; WORK5 => M%WORK5_B; WORK6 => M%WORK6_B
   WORK7 => M%WORK7_B; WORK8 => M%WORK8_B; WORK9 => M%WORK9_B
ELSE
   WORK1 => M%WORK1; WORK2 => M%WORK2; WORK3 => M%WORK3
   WORK4 => M%WORK4; WORK5 => M%WORK5; WORK6 => M%WORK6
   WORK7 => M%WORK7; WORK8 => M%WORK8; WORK9 => M%WORK9
ENDIF
RSUM => M%RSUM; H => M%H; KRES => M%KRES
QR_W => M%QR_W
RADIATION_EMISSION => M%RADIATION_EMISSION
RADIATION_ABSORPTION => M%RADIATION_ABSORPTION
PBAR => M%PBAR; CELL_ILW => M%CELL_ILW
ZZ => M%ZZ; UIID => M%UIID
AVG_DROP_AREA => M%AVG_DROP_AREA
AVG_DROP_RAD => M%AVG_DROP_RAD
AVG_DROP_TMP => M%AVG_DROP_TMP
R => M%R; DX => M%DX; DY => M%DY; DZ => M%DZ
XC => M%XC; YC => M%YC; ZC => M%ZC
TMP_0 => M%TMP_0; RHO_0 => M%RHO_0; P_0 => M%P_0
WALL_WORK1 => M%WALL_WORK1; WALL_WORK2 => M%WALL_WORK2
FACE_WORK1 => M%FACE_WORK1; FACE_WORK2 => M%FACE_WORK2
FACE_WORK3 => M%FACE_WORK3
CELL_INDEX => M%CELL_INDEX
PRESSURE_ZONE => M%PRESSURE_ZONE
CCVAR => M%CCVAR; FCVAR => M%FCVAR
CELL => M%CELL; CUT_CELL => M%CUT_CELL
CUT_FACE => M%CUT_FACE; CFACE => M%CFACE
...
VENTS => M%VENTS; RAD_FILE => M%RAD_FILE
...
ELSE
   ...
   IF (INIT_HRRPUV) CALL ADD_VOLUMETRIC_HEAT_SOURCE(0)
ENDIF
...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
         ...
         DO J=1,JBAR
            DO I=1,IBAR
               ...
               IF (CHI_R(I,J,K)*Q(I,J,K)>QR_CLIP) THEN ! Precomputation of quantities for the RTE source term correction
                     ...
                     RAD_Q_SUM_OUT = RAD_Q_SUM_OUT + (BBF*CHI_R(I,J,K)*Q(I,J,K) + KAPPA_GAS(I,J,K)*UIID(I,J,K,IBND))*VOL
                     KFST4_SUM_OUT = KFST4_SUM_OUT + KFST4_GAS(I,J,K)*VOL
               ENDIF
            ENDDO
         ENDDO
      ENDDO
   ...
   ELSE WIDE_BAND_MODEL_IF
      ...
      RTE_SOURCE_CORRECTION_IF: IF (RTE_SOURCE_CORRECTION) THEN ! default RTE_SOURCE_CORRECTION=.TRUE.
         ...
         RAD_Q_SUM_OUT = RAD_Q_SUM_OUT + RAD_Q_SUM_PARTIAL
         KFST4_SUM_OUT = KFST4_SUM_OUT + KFST4_SUM_PARTIAL
...
END SUBROUTINE COMPUTE_RADIATION_KERNEL
