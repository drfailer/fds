...
PUBLIC MERGE_PRESSURE_ZONES_MESH
...
SUBROUTINE DIVERGENCE_PART_1_KERNEL(M,T,DT,NM,SKIP_QR,WORK_BRANCH,PHASE)
...
INTEGER :: PHASE_VAL, WORK_BRANCH_VAL
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: WK1,WK2,WK3,WK4,WK5,WK6,WK7,WK9
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: SK1,SK2,SK3

IF (SOLID_PHASE_ONLY) RETURN

PHASE_VAL = 0
IF (PRESENT(PHASE)) PHASE_VAL = PHASE
...
! Select WORK/SWORK scratch arrays based on branch
IF (WORK_BRANCH_VAL==2) THEN
   WK1=>M%WORK1_B; WK2=>M%WORK2_B; WK3=>M%WORK3_B; WK4=>M%WORK4_B
   WK5=>M%WORK5_B; WK6=>M%WORK6_B; WK7=>M%WORK7_B; WK9=>M%WORK9_B
   SK1=>M%SWORK1_B; SK2=>M%SWORK2_B; SK3=>M%SWORK3_B
ELSE
   WK1=>M%WORK1; WK2=>M%WORK2; WK3=>M%WORK3; WK4=>M%WORK4
   WK5=>M%WORK5; WK6=>M%WORK6; WK7=>M%WORK7; WK9=>M%WORK9
   SK1=>M%SWORK1; SK2=>M%SWORK2; SK3=>M%SWORK3
ENDIF
...
! --- Phase 1: Pre-fork (setup, zero DP) ---

PREFORK_IF: IF (PHASE_VAL == 0 .OR. PHASE_VAL == 1) THEN

   DP = 0._EB

   IF (CC_IBM) THEN
      IF (PHASE_VAL == 0) CALL CC_VELOCITY_FLUX(M,DT,M%PREDICTOR,RHOP,CORRECT_GRAV=.FALSE.)
   ENDIF

ENDIF PREFORK_IF

! --- Phase 2: PREDICT_NORMAL_VELOCITY + Early phases (species diffusion, heat flux, thermal) ---
! PNV is placed here (not in Phase 1) because WALL_BC sets B1%U_NORMAL_S for
! certain wall types (HVAC, SPECIFIED_MASS_FLUX). In the pipelined predictor,
! Branch B runs WALL_BC → Phase 2, so PNV correctly runs after WALL_BC.

EARLY_IF: IF (PHASE_VAL == 0 .OR. PHASE_VAL == 2) THEN

WALL_LOOP3: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   CALL PREDICT_NORMAL_VELOCITY(IW,T,DT)
ENDDO WALL_LOOP3

IF (CC_IBM) CALL CFACE_PREDICT_NORMAL_VELOCITY(M,T,DT)
...
IF (.NOT.CONSTANT_SPECIFIC_HEAT_RATIO) THEN
   CP    => WK5   ! Reuses H_RHO_D_DZDX (no longer needed)
   R_H_G => WK9   ! Reuses RHO_D_TURB (no longer needed)
   CALL COMPUTE_SPECIFIC_HEAT
ENDIF

! --- Thermal conductivity and gradients ---

KDTDX => WK1     ! Reuses RTRM (will be rewritten later)
KDTDY => WK2
KDTDZ => WK3
KP    => WK4     ! Reuses RHO_D (no longer needed)
...
! --- Phase 3: Late phases (advection, RTRM, sources, zone sums) ---

LATE_IF: IF (PHASE_VAL == 0 .OR. PHASE_VAL == 3) THEN

   ! Re-establish pointer aliases when called standalone (after PHASE=2)
   IF (PHASE_VAL == 3) THEN
      RHO_D_DZDX => SK1; RHO_D_DZDY => SK2; RHO_D_DZDZ => SK3
      IF (.NOT.CONSTANT_SPECIFIC_HEAT_RATIO) THEN
         R_H_G => WK9
      ENDIF
      IF (CC_IBM) CALL CC_VELOCITY_FLUX(M,DT,M%PREDICTOR,RHOP,CORRECT_GRAV=.FALSE.)
   ENDIF

   CALL COMPUTE_DIVERGENCE_SOURCES

   IF (CC_IBM) CALL CC_DIVERGENCE_PART_1(T,DT,NM,M)
...
!> Compute per-mesh D_SUM_LOC, P_SUM_LOC, U_SUM_LOC for pressure zone accumulation.
!> Accumulates into per-mesh local arrays (not globals) for thread safety.
!> EXCHANGE_DIVERGENCE_INFO_DRIVER sums these into global DSUM/PSUM/USUM.
...
WALL_LOOP4: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (M%PREDICTOR) M%U_SUM_LOC(IPZ) = M%U_SUM_LOC(IPZ) + B1%U_NORMAL_S*B1%AREA
   IF (M%CORRECTOR) M%U_SUM_LOC(IPZ) = M%U_SUM_LOC(IPZ) + B1%U_NORMAL  *B1%AREA
ENDDO WALL_LOOP4
...
FX_H_S=>WK2    ; FX_H_S = 0._EB
FY_H_S=>WK3    ; FY_H_S = 0._EB
FZ_H_S=>WK4    ; FZ_H_S = 0._EB
U_DOT_DEL_RHO_H_S=>WK6 ; U_DOT_DEL_RHO_H_S=0._EB
...
WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      ...
      CASE(SOLID_BOUNDARY)
         IF (M%PREDICTOR) UN = -SIGN(1._EB,REAL(BC%IOR,EB))*B1%U_NORMAL_S
         IF (M%CORRECTOR) UN = -SIGN(1._EB,REAL(BC%IOR,EB))*B1%U_NORMAL
...
FY_ZZ=>SK2
FZ_ZZ=>SK3
U_DOT_DEL_RHO_Z=>WK7 ; U_DOT_DEL_RHO_Z = 0._EB
...
!> \brief Add RTRM*QR (radiation source term) to the divergence DP.
!> Used after DIVERGENCE_PART_1_KERNEL(SKIP_QR=.TRUE.) and a QR exchange.
!> Since DP was multiplied by RTRM during DIV_P1, the QR contribution must
!> also be scaled: DP += RTRM * QR. RTRM is stored in WORK1 (or WORK1_B).
!> \param M Mesh data structure
!> \param NM Mesh number
!> \param WORK_BRANCH Optional: which WORK pool (1=default, 2=branch B)

RECURSIVE SUBROUTINE DIVERGENCE_PART_1_ADD_QR_KERNEL(M,NM,WORK_BRANCH)

TYPE(MESH_TYPE), INTENT(INOUT), TARGET :: M
INTEGER, INTENT(IN) :: NM
INTEGER, INTENT(IN), OPTIONAL :: WORK_BRANCH
REAL(EB), POINTER, DIMENSION(:,:,:) :: DP,RTRM
INTEGER :: I,J,K,IPZ,WORK_BRANCH_VAL
REAL(EB) :: VC,VC1,QR_CONTRIB
...
! RTRM is stored in WORK1 (or WORK1_B if branch 2 was used for DIV_P1)
WORK_BRANCH_VAL = 1
IF (PRESENT(WORK_BRANCH)) WORK_BRANCH_VAL = WORK_BRANCH
IF (WORK_BRANCH_VAL==2) THEN
   RTRM => M%WORK1_B
...
DO K=1,M%KBAR
   DO J=1,M%JBAR
      DO I=1,M%IBAR
         DP(I,J,K) = DP(I,J,K) + RTRM(I,J,K)*M%QR(I,J,K)
      ENDDO
   ENDDO
ENDDO

! Update D_SUM_LOC to account for the QR contribution added to DP.
! When SKIP_QR was used in DIV_P1, D_SUM_LOC was computed without QR.
IF (N_ZONE>0) THEN
   DO K=1,M%KBAR
      DO J=1,M%JBAR
         VC1 = M%DY(J)*M%DZ(K)
         DO I=1,M%IBAR
            ...
            IF (M%CELL(M%CELL_INDEX(I,J,K))%SOLID) CYCLE
            VC = M%DX(I)*M%RC(I)*VC1
            QR_CONTRIB = RTRM(I,J,K)*M%QR(I,J,K)
            M%D_SUM_LOC(IPZ) = M%D_SUM_LOC(IPZ) + VC*QR_CONTRIB
         ENDDO
      ENDDO
   ENDDO
ENDIF
...
IF_PRESSURE_ZONES: IF (N_ZONE>0) THEN
   ...
   IF (M%PREDICTOR) D_PBAR_DT_P => M%D_PBAR_DT_S
   IF (M%CORRECTOR) D_PBAR_DT_P => M%D_PBAR_DT

   ! Compute change in background pressure

   DO IPZ=1,N_ZONE
      IF (ABS(PSUM(IPZ)) > TWENTY_EPSILON_EB) D_PBAR_DT_P(IPZ) = (DSUM(IPZ) - USUM(IPZ))/PSUM(IPZ)
      IF (M%CORRECTOR) P_ZONE(IPZ)%DPSTAR =  D_PBAR_DT_P(IPZ)
   ENDDO
...
IF (CC_IBM) THEN
   DO K=1,M%KBAR
      ...
      ENDDO
   ENDDO
   CALL GET_LINKED_VELOCITIES(M,NM,M%CORRECTOR,CMP_FLG=.FALSE.)
ENDIF
...
IF (CC_IBM) CALL GET_LINKED_VELOCITIES(M,NM,M%CORRECTOR,CMP_FLG=.FALSE.)
...
SUBROUTINE MERGE_PRESSURE_ZONES_MESH(M)
...
END SUBROUTINE MERGE_PRESSURE_ZONES_MESH
