...
REAL(EB) FUNCTION HEAT_TRANSFER_COEFFICIENT(M,T,DELTA_N_TMP,SF,WALL_INDEX_IN,CFACE_INDEX_IN,PARTICLE_INDEX_IN,BACK_SIDE,&
                                            SKIP_BLOWING)
...
LOGICAL :: BLOWING
INTEGER  :: SURF_GEOMETRY,ITMP,I,HTR,B1_INDEX,B2_INDEX,BC_INDEX
...
IF (PRESENT(PARTICLE_INDEX_IN)) THEN
   B1_INDEX = M%LAGRANGIAN_PARTICLE(PARTICLE_INDEX_IN)%B1_INDEX
   B2_INDEX = M%LAGRANGIAN_PARTICLE(PARTICLE_INDEX_IN)%B2_INDEX
   BC_INDEX = M%LAGRANGIAN_PARTICLE(PARTICLE_INDEX_IN)%BC_INDEX
   DN = SF%CONV_LENGTH
   R_DROP = M%LAGRANGIAN_PARTICLE(PARTICLE_INDEX_IN)%RADIUS
...
ELSEIF (PRESENT(WALL_INDEX_IN)) THEN
   ...
   DN = 1._EB/M%BOUNDARY_PROP1(B1_INDEX)%RDN
ELSEIF (PRESENT(CFACE_INDEX_IN)) THEN
   B1_INDEX = M%CFACE(CFACE_INDEX_IN)%B1_INDEX
   B2_INDEX = M%CFACE(CFACE_INDEX_IN)%B2_INDEX
   BC_INDEX = M%CFACE(CFACE_INDEX_IN)%BC_INDEX
   DN = 1._EB/M%BOUNDARY_PROP1(B1_INDEX)%RDN
ELSE
...
TMP_FILM = M%BOUNDARY_PROP1(B1_INDEX)%TMP_F+SF%FILM_FACTOR*(M%BOUNDARY_PROP1(B1_INDEX)%TMP_G-M%BOUNDARY_PROP1(B1_INDEX)%TMP_F)
...
ELSE H_FIXED_IF
   ...
   HTC_TYPE_IF: IF ( (SIM_MODE==DNS_MODE .OR. SOLID_PHASE_ONLY) .AND. (PRESENT(WALL_INDEX_IN) .OR. PRESENT(CFACE_INDEX_IN)) ) THEN
      ...
      HEAT_TRANSFER_COEFFICIENT = 2._EB * M%BOUNDARY_PROP1(B1_INDEX)%K_G * M%BOUNDARY_PROP1(B1_INDEX)%RDN
      IF (SIM_MODE==DNS_MODE) RETURN

   ELSE HTC_TYPE_IF

      CALL GET_VISCOSITY(M%BOUNDARY_PROP1(B1_INDEX)%ZZ_G,MU_G,TMP_FILM)
      CALL GET_CONDUCTIVITY(M%BOUNDARY_PROP1(B1_INDEX)%ZZ_G,K_G,TMP_FILM)

      HTC_MODEL_SELECT: SELECT CASE(SF%HEAT_TRANSFER_MODEL)
         CASE(DEFAULT_HTC_MODEL,IMPINGING_JET_HTC_MODEL)
            RE = M%BOUNDARY_PROP1(B1_INDEX)%RHO_G*M%BOUNDARY_PROP1(B1_INDEX)%U_TANG*CONV_LENGTH/MU_G
            GR = GRAV*ABS(DELTA_N_TMP)*CONV_LENGTH**3*(M%BOUNDARY_PROP1(B1_INDEX)%RHO_G/MU_G)**2/TMP_FILM
            ...
            ENDIF
            RA = GR*PR_AIR
            CALL NATURAL_CONVECTION_MODEL(NUSSELT_FREE,RA,SF,M%BOUNDARY_COORD(BC_INDEX)%IOR,DELTA_N_TMP)
            NUSSELT_IMPINGE = 0._EB
            IF (SF%HEAT_TRANSFER_MODEL==IMPINGING_JET_HTC_MODEL) THEN
               XX = SQRT( (SF%XYZ(1)-M%BOUNDARY_COORD(BC_INDEX)%X)**2 + &
                          (SF%XYZ(2)-M%BOUNDARY_COORD(BC_INDEX)%Y)**2 + &
                          (SF%XYZ(3)-M%BOUNDARY_COORD(BC_INDEX)%Z)**2 )
               NUSSELT_IMPINGE = SF%H_FIXED*EXP(-0.5_EB * (XX / SF%HTC_SIGMA)**2) * &
                                 CONV_LENGTH/K_G
            ENDIF
            IF (PRESENT(PARTICLE_INDEX_IN)) THEN
            ...
            ELSE
               M%BOUNDARY_PROP2(B2_INDEX)%HEAT_TRANSFER_REGIME=MAXLOC((/ &
                  NUSSELT_FREE,NUSSELT_FORCED,NUSSELT_IMPINGE,2._EB*CONV_LENGTH/DN/),DIM=1)
               SELECT CASE(M%BOUNDARY_PROP2(B2_INDEX)%HEAT_TRANSFER_REGIME)
                  CASE(NATURAL)
                     M%BOUNDARY_PROP2(B2_INDEX)%Z_STAR = NUSSELT_FREE**0.25_EB
                  CASE(FORCED)
                     M%BOUNDARY_PROP2(B2_INDEX)%Z_STAR = SQRT(NUSSELT_FORCED)
                  CASE(IMPACT)
                     M%BOUNDARY_PROP2(B2_INDEX)%Z_STAR = SQRT(NUSSELT_IMPINGE)
                  CASE(RESOLVED)
                     M%BOUNDARY_PROP2(B2_INDEX)%Z_STAR = 1._EB
               END SELECT
         ...
         CASE(LOGLAW_HTC_MODEL)
            CALL GET_SPECIFIC_HEAT(M%BOUNDARY_PROP1(B1_INDEX)%ZZ_G,CP_G,TMP_FILM)
            FRICTION_VELOCITY = M%BOUNDARY_PROP2(B2_INDEX)%U_TAU
            YPLUS = M%BOUNDARY_PROP2(B2_INDEX)%Y_PLUS
            CALL LOGLAW_HEAT_FLUX_MODEL(H_FORCED,YPLUS,FRICTION_VELOCITY,K_G,M%BOUNDARY_PROP1(B1_INDEX)%RHO_G,CP_G,MU_G)
            HEAT_TRANSFER_COEFFICIENT = H_FORCED
         CASE(RAYLEIGH_HTC_MODEL)
            ! not appropriate for a particle, used with SURF and CFACE only
            CALL GET_SPECIFIC_HEAT(M%BOUNDARY_PROP1(B1_INDEX)%ZZ_G,CP_G,TMP_FILM)
            CALL RAYLEIGH_HEAT_FLUX_MODEL(H_NATURAL,ZSTAR,HTR,DN, &
               M%BOUNDARY_PROP1(B1_INDEX)%TMP_F,M%BOUNDARY_PROP1(B1_INDEX)%TMP_G,K_G, &
               M%BOUNDARY_PROP1(B1_INDEX)%RHO_G,CP_G,MU_G,M%BOUNDARY_PROP1(B1_INDEX)%U_TANG)
            M%BOUNDARY_PROP2(B2_INDEX)%Z_STAR = ZSTAR
            M%BOUNDARY_PROP2(B2_INDEX)%HEAT_TRANSFER_REGIME = HTR
            HEAT_TRANSFER_COEFFICIENT = H_NATURAL
         CASE(FM_HTC_MODEL)
            CALL GET_SPECIFIC_HEAT(M%BOUNDARY_PROP1(B1_INDEX)%ZZ_G,CP_G,TMP_FILM)
            CALL FM_HEAT_FLUX_MODEL(H_NATURAL,DN,M%BOUNDARY_PROP1(B1_INDEX)%TMP_F, &
               M%BOUNDARY_PROP1(B1_INDEX)%TMP_G,K_G,M%BOUNDARY_PROP1(B1_INDEX)%RHO_G,CP_G)
            HEAT_TRANSFER_COEFFICIENT = H_NATURAL
      END SELECT HTC_MODEL_SELECT
   ENDIF HTC_TYPE_IF
ENDIF H_FIXED_IF

IF (BLOWING .AND. .NOT. SF%BOUNDARY_FUEL_MODEL .AND. SIM_MODE /= DNS_MODE .AND. &
    ALLOCATED(M%BOUNDARY_PROP1(B1_INDEX)%M_DOT_G_PP_ACTUAL)) THEN
   PHI = 0._EB
   IF (SF%INCLUDE_BOUNDARY_PROP2_TYPE) &
      M%BOUNDARY_PROP2(B2_INDEX)%BLOWING_CORRECTION=0._EB
   ITMP = INT(TMP_FILM)
   DO I=1,N_TRACKED_SPECIES
      IF (ABS(M%BOUNDARY_PROP1(B1_INDEX)%M_DOT_G_PP_ACTUAL(I)) <= &
          TWENTY_EPSILON_EB) CYCLE
      PHI = PHI + M%BOUNDARY_PROP1(B1_INDEX)%M_DOT_G_PP_ACTUAL(I)*CP_Z(ITMP,I)
   ENDDO
   IF (ABS(PHI)>TWENTY_EPSILON_EB .AND. ABS(HEAT_TRANSFER_COEFFICIENT)>=TWENTY_EPSILON_EB) THEN
      ...
      ELSE
         ...
         IF (SF%INCLUDE_BOUNDARY_PROP2_TYPE) M%BOUNDARY_PROP2(B2_INDEX)%BLOWING_CORRECTION = PHI/(EXP(PHI)-1._EB)
      ENDIF
   ENDIF
ENDIF
