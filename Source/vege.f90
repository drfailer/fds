...
IF (.NOT.MESHES(NM)%PREDICTOR) THEN
   IF (CC_IBM) THEN
      DO JJG=1,JBAR
         DO IIG=1,IBAR
            SF => SURFACE(LS_SURF_INDEX(IIG,JJG))
...
REAL(EB), INTENT(IN) :: T,DT
CALL POINT_TO_MESH(NM)
CALL UPDATE_FIRE_SPREAD_OUTPUTS_TS(MESHES(NM),T,DT,NM)
END SUBROUTINE UPDATE_FIRE_SPREAD_OUTPUTS


RECURSIVE SUBROUTINE UPDATE_FIRE_SPREAD_OUTPUTS_TS(M,T,DT,NM)
...
END SUBROUTINE UPDATE_FIRE_SPREAD_OUTPUTS_TS
