...
650 CONTINUE
IF (ABS(ELMBDA)>=TWENTY_EPSILON_EB) GO TO 750
ISING = 1
660 CONTINUE
PERT = 0._EB
DO  I = 1,L
...
IF (LPEROD==0) THEN
  DO  I = 1,L
    ...
    IF (ABS(A(I)-A(1))>=TWENTY_EPSILON_EB) GO TO 110
    IF (ABS(B(I)-B(1))>=TWENTY_EPSILON_EB) GO TO 110
    IF (ABS(C(I)-A(1))>=TWENTY_EPSILON_EB) GO TO 110
    IF (ABS(D(I)-D(1))>=TWENTY_EPSILON_EB) GO TO 110
...
IF (ABS(RS)>=TWENTY_EPSILON_EB .AND. LBDCND>=5) THEN
  IERROR = IERROR + 1
  SAVE(IERROR) = 6._EB
END IF
...
IF (ABS(TF-PI)<=TWENTY_EPSILON_EB .AND. (MBDCND==2.OR.MBDCND==3.OR. MBDCND==6)) THEN
  IERROR = IERROR + 1
  SAVE(IERROR) = 11._EB
END IF
...
640 CONTINUE
IF (ABS(ELMBDA)<=TWENTY_EPSILON_EB) THEN
  GO TO 650
ELSE
  GO TO 770
END IF
...
!                               ON A CRAY-1
  ...
  IF (ABS(Y(K,M))>=TWENTY_EPSILON_EB) Y(K,M) = D(K,M)/Y(K,M)
END DO
DO  I = M - 1,1,-1
  DO  K = 1,IDEG
...
DO  K = 1,NC
  ...
  COEF(K) = SIGN
  DO  I = 1,M
    IF (ABS(W3(I))>TWENTY_EPSILON_EB) COEF(K) = COEF(K)*W1(I)*W2(I)/W3(I)
  END DO
END DO
RETURN
...
!                               RATIONAL QL TRANSFORMATION
  ...
  IF (ABS(G)<=TWENTY_EPSILON_EB) G = B
  H = G
  S = 0.0_EB
  MML = M - L
...
!                               GUARD AGAINST UNDERFLOWED H
  ...
  IF (ABS(H)<=TWENTY_EPSILON_EB) GO TO 170

  IF (ABS(E2(L))<=ABS(C/H)) GO TO 170
  E2(L) = H*E2(L)

  IF (ABS(E2(L))>=TWENTY_EPSILON_EB) GO TO 140
  170     CONTINUE
  P = D(L) + F
...
IF (ABS(RS)<=TWENTY_EPSILON_EB .AND. LBDCND<=4) THEN
  IERROR = IERROR + 1
  SAVE(IERROR) = 5._EB
END IF

IF (ABS(RS)<=TWENTY_EPSILON_EB .AND. ABS(XMU)>=TWENTY_EPSILON_EB) THEN
  IERROR = IERROR + 1
  SAVE(IERROR) = 6._EB
END IF
...
650 CONTINUE
IF (ABS(XMU)<=TWENTY_EPSILON_EB) THEN
  GO TO 665
ELSE
  GO TO 770
END IF
