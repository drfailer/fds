...
MAIN_LOOP: DO
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      COMPUTE_DENSITY_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         IF(CC_IBM .AND. .NOT.FIRST_PASS) CALL CC_RESTORE_UVW_UNLINKED(MESHES(NM),NM,ASSIGN_UNLINKED_VEL=.TRUE.)
...
PRESSURE_ITERATION_LOOP: DO
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      CALL NO_FLUX(DT,NM)
      IF (CC_IBM) CALL CC_NO_FLUX(DT,NM,.FALSE.,MESHES(NM)) ! set WALL_WORK1 to 0 in cells inside geometries.
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      IF (CC_IBM) CALL CC_COMPUTE_VELOCITY_ERROR(DT,NM,MESHES(NM)) ! Inside solids respect to zero velocity.
   ENDDO
