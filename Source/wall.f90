...
USE MESH_POINTERS
USE WALL_KERNELS, ONLY: CALCULATE_RHO_D_F, CALC_DEPOSITION, &
   PYROLYSIS, CALCULATE_RHO_F_KERNEL, &
   NEAR_SURFACE_GAS_VARIABLES_KERNEL
...
PUBLIC WALL_BC,TGA_ANALYSIS,HT3D_TEMPERATURE_EXCHANGE,WALL_BC_PREPROCESSING,WALL_BC_PROCESS_CELLS_KERNEL, &
       WALL_BC_PROCESS_CELLS_BLOCK_KERNEL,WALL_BC_FINALIZE
...
IF (.NOT.INITIALIZATION_PHASE .AND. M%CORRECTOR) THEN
   IF (WALL_COUNTER==WALL_INCREMENT) THEN
      DT_BC    = T - M%BC_CLOCK
      M%BC_CLOCK = T
      CALL_HT_1D = .TRUE.
      M%HT_3D_SWEEP_DIRECTION = M%HT_3D_SWEEP_DIRECTION + 1
      IF (M%HT_3D_SWEEP_DIRECTION>3) M%HT_3D_SWEEP_DIRECTION = 1
   ENDIF
ENDIF
...
IF (M%N_THIN_WALL_CELLS>0 .AND. CALL_HT_1D) THEN
   THIN_WALL_CELL_LOOP_0: DO ITW=1,M%N_THIN_WALL_CELLS
      TW => M%THIN_WALL(ITW)
      BC => M%BOUNDARY_COORD(TW%BC_INDEX)
      B1 => M%BOUNDARY_PROP1(TW%B1_INDEX)
      SF => SURFACE(TW%SURF_INDEX)
      CALL NEAR_SURFACE_GAS_VARIABLES_KERNEL(M, &
         T,SF,BC,B1,TW=TW,THIN_WALL_INDEX=ITW)
...
! ========================================
! THREE-PHASE WALL BOUNDARY CONDITION PROCESSING
! ========================================
!
! Phase 1 (Preprocessing): Already completed above via WALL_CELL_LOOP_0 and THIN_WALL_CELL_LOOP_0
!   - ASSIGN_GHOST_VALUE for INTERPOLATED_BOUNDARY cells (cross-mesh reads)
!   - NEAR_SURFACE_GAS_VARIABLES_KERNEL and HEAT_TRANSFER_COEFFICIENT setup
!
! Phase 2 (Parallel Processing): Process ~90% of cells without cross-mesh dependencies
!   - Wall cells without HAS_INTERPOLATED_BC or HAS_BACK_MESH
!   - All CFACE cells
!   - All particles (except DEPOSIT_PARTICLE_MASS)
!
! Phase 3 (Finalization): Sequential processing of remaining ~10% with cross-mesh dependencies
!   - Wall cells with HAS_BACK_MESH (thin walls spanning meshes)
!   - Thin wall lateral heat transfer
!   - Particle off-gassing via DEPOSIT_PARTICLE_MASS

! Phase 2: Parallel cell processing (ready for Hedgehog integration)
CALL WALL_BC_PROCESS_CELLS_KERNEL(MESHES(NM),NM,M%PREDICTOR,T,DT,DT_BC,CALL_HT_1D)

! Phase 3: Sequential finalization for cross-mesh processing
CALL WALL_BC_FINALIZE(NM,T,DT_BC,CALL_HT_1D)
...
SUBROUTINE ASSIGN_GHOST_VALUE(NM,WALL_INDEX,BC,B1)

USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_GAS_CONSTANT
USE COMPLEX_GEOMETRY, ONLY : CC_CGSC, CC_SOLID
INTEGER, INTENT(IN) :: NM, WALL_INDEX
REAL(EB) :: ARO,ZZ_GET(1:MAX_SPECIES),RHO_OTHER,RHO_ZZ_OTHER(1:MAX_SPECIES),&
            RSUM_TMP,RHO_OTHER_2,RHO_ZZ_OTHER_2(1:MAX_SPECIES),PBAR_P_2
...
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: B1
! Thread-safe local pointer aliases (Pattern 3)
REAL(EB), POINTER, DIMENSION(:,:,:) :: RHOP, TMP
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP
REAL(EB), POINTER, DIMENSION(:,:) :: PBAR_P
REAL(EB), POINTER, DIMENSION(:,:,:) :: RSUM

M => MESHES(NM)

EWC => M%EXTERNAL_WALL(WALL_INDEX)
IF (EWC%NOM==0) RETURN
...
DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
   DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
      DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
         ...
         SELECT CASE(BC%IOR)
            CASE( 1) ; ARO = MIN(1._EB , (MM%DY(JJO)*MM%DZ(KKO))/(M%DY(BC%JJ)*M%DZ(BC%KK)) ) ; II2=II2-1
            CASE(-1) ; ARO = MIN(1._EB , (MM%DY(JJO)*MM%DZ(KKO))/(M%DY(BC%JJ)*M%DZ(BC%KK)) ) ; II2=II2+1
            CASE( 2) ; ARO = MIN(1._EB , (MM%DX(IIO)*MM%DZ(KKO))/(M%DX(BC%II)*M%DZ(BC%KK)) ) ; JJ2=JJ2-1
            CASE(-2) ; ARO = MIN(1._EB , (MM%DX(IIO)*MM%DZ(KKO))/(M%DX(BC%II)*M%DZ(BC%KK)) ) ; JJ2=JJ2+1
            CASE( 3) ; ARO = MIN(1._EB , (MM%DX(IIO)*MM%DY(JJO))/(M%DX(BC%II)*M%DY(BC%JJ)) ) ; KK2=KK2-1
            CASE(-3) ; ARO = MIN(1._EB , (MM%DX(IIO)*MM%DY(JJO))/(M%DX(BC%II)*M%DY(BC%JJ)) ) ; KK2=KK2+1
         END SELECT
...
INTERPOLATED_IF: IF (WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY) THEN
   ...
   IF (ABS(EWC%AREA_RATIO-1._EB)<0.01_EB) THEN
      ...
      KKO = EWC%KKO_MIN
      SELECT CASE(BC%IOR)
         CASE( 1) ; ICG = M%CELL_INDEX(BC%IIG+1,BC%JJG,BC%KKG) ; ICO = MM%CELL_INDEX(IIO-1,JJO,KKO)
         CASE(-1) ; ICG = M%CELL_INDEX(BC%IIG-1,BC%JJG,BC%KKG) ; ICO = MM%CELL_INDEX(IIO+1,JJO,KKO)
         CASE( 2) ; ICG = M%CELL_INDEX(BC%IIG,BC%JJG+1,BC%KKG) ; ICO = MM%CELL_INDEX(IIO,JJO-1,KKO)
         CASE(-2) ; ICG = M%CELL_INDEX(BC%IIG,BC%JJG-1,BC%KKG) ; ICO = MM%CELL_INDEX(IIO,JJO+1,KKO)
         CASE( 3) ; ICG = M%CELL_INDEX(BC%IIG,BC%JJG,BC%KKG+1) ; ICO = MM%CELL_INDEX(IIO,JJO,KKO-1)
         CASE(-3) ; ICG = M%CELL_INDEX(BC%IIG,BC%JJG,BC%KKG-1) ; ICO = MM%CELL_INDEX(IIO,JJO,KKO+1)
      END SELECT
      IF (CC_IBM) THEN ! Test if one of surrounding cells is CC_SOLID.
         IF(M%CCVAR(BC%IIG,BC%JJG,BC%KKG,CC_CGSC)==CC_SOLID .OR. M%CCVAR(BC%II,BC%JJ,BC%KK,CC_CGSC)==CC_SOLID) &
            CC_SOLID_FLAG = .TRUE.
      ENDIF
      IF (.NOT.M%CELL(ICG)%SOLID.AND..NOT.MM%CELL(ICO)%SOLID.AND..NOT.CC_SOLID_FLAG) SECOND_ORDER_INTERPOLATED_BOUNDARY = .TRUE.
   ENDIF
...
SUBROUTINE SURFACE_HEAT_TRANSFER(NM,PREDICTOR_FLAG,T,SF,BC,B1,WALL_INDEX,CFACE_INDEX,PARTICLE_INDEX)
...
REAL(EB) :: ARO,QNET,RAMP_FACTOR,RSUM_F,PBAR_F,TSI,UN, &
            RHO_ZZ_F(1:MAX_SPECIES),ZZ_GET(1:MAX_SPECIES), &
            RHO_OTHER, PBAR_OTHER,D_Z_N(0:MAX_I_MAX_TEMP),D_Z_G,D_Z_OTHER,TMP_OTHER,DDO, &
...
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP
REAL(EB), POINTER, DIMENSION(:,:) :: PBAR_P
REAL(EB), POINTER, DIMENSION(:) :: TMP_0

M => MESHES(NM)
TMP_0 => M%TMP_0

! Set pointers to appropriate arrays based on predictor/corrector phase
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
   LP => M%LAGRANGIAN_PARTICLE(PARTICLE_INDEX)
   ...
   IF (LPC%MASSLESS_TARGET) THEN  ! the particle's sole purpose is to record a heat flux
      ...
      ELSE  ! the HTC will be computed using the surrounding gas phase environment
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(M,T,B1%TMP_G-PY%GAUGE_TEMPERATURE,SF, &
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER  ! Only for WALL cells
      ...
      M%TMP(BC%II,BC%JJ,BC%KK)    = B1%TMP_F
      M%TMP(BC%II2,BC%JJ2,BC%KK2) = B1%TMP_F
      ...
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,M%RSUM(BC%II,BC%JJ,BC%KK))
      RHOP(BC%II,BC%JJ,BC%KK) = PBAR_P(BC%KK,B1%PRESSURE_ZONE)/(M%RSUM(BC%II,BC%JJ,BC%KK)*M%TMP(BC%II,BC%JJ,BC%KK))
      RHOP(BC%II2,BC%JJ2,BC%KK2) = RHOP(BC%II,BC%JJ,BC%KK)
   ...
   CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(M,T,DTMP,SF,WALL_INDEX_IN=WALL_INDEX)
      ELSEIF (PRESENT(CFACE_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(M,T,DTMP,SF,CFACE_INDEX_IN=CFACE_INDEX)
      ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(M,T,DTMP,SF,PARTICLE_INDEX_IN=PARTICLE_INDEX)
      ENDIF
   ...
   CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER  ! Only for EXTERNAL_WALL_CELLs
      ...
      SINGLE_SPEC_IF: IF (N_TOTAL_SCALARS > 1) THEN
         ...
         IF (ATMOSPHERIC_INTERPOLATION) THEN
            B1%TMP_F = (M%TMP(BC%II,BC%JJ,BC%KK)*M%DZ(BC%KKG) + M%TMP(BC%IIG,BC%JJG,BC%KKG)*M%DZ(BC%KK)) / &
                       (M%DZ(BC%KK)+M%DZ(BC%KKG))
            B1%ZZ_F(1:N_TOTAL_SCALARS) = (ZZP(BC%II,BC%JJ,BC%KK,1:N_TOTAL_SCALARS)*M%DZ(BC%KKG) + &
                                          ZZP(BC%IIG,BC%JJG,BC%KKG,1:N_TOTAL_SCALARS)*M%DZ(BC%KK)) / (M%DZ(BC%KK)+M%DZ(BC%KKG))
            ...
            SELECT CASE(ABS(BC%IOR))
               ...
               CASE(3)
                  ...
                  PBAR_F = (PBAR_P(BC%KK,B1%PRESSURE_ZONE)*M%DZ(BC%KKG) + PBAR_P(BC%KKG,B1%PRESSURE_ZONE)*M%DZ(BC%KK)) / &
                           (M%DZ(BC%KK)+M%DZ(BC%KKG))
            END SELECT
            B1%RHO_F = PBAR_F/(RSUM_F*B1%TMP_F)
         ELSE
            SELECT CASE(BC%IOR)
               CASE( 1); RHO_ZZ_F(1:N_TOTAL_SCALARS) = M%FX(BC%II  ,BC%JJ,BC%KK,1:N_TOTAL_SCALARS)
               CASE(-1); RHO_ZZ_F(1:N_TOTAL_SCALARS) = M%FX(BC%II-1,BC%JJ,BC%KK,1:N_TOTAL_SCALARS)
               CASE( 2); RHO_ZZ_F(1:N_TOTAL_SCALARS) = M%FY(BC%II,BC%JJ  ,BC%KK,1:N_TOTAL_SCALARS)
               CASE(-2); RHO_ZZ_F(1:N_TOTAL_SCALARS) = M%FY(BC%II,BC%JJ-1,BC%KK,1:N_TOTAL_SCALARS)
               CASE( 3); RHO_ZZ_F(1:N_TOTAL_SCALARS) = M%FZ(BC%II,BC%JJ,BC%KK  ,1:N_TOTAL_SCALARS)
               CASE(-3); RHO_ZZ_F(1:N_TOTAL_SCALARS) = M%FZ(BC%II,BC%JJ,BC%KK-1,1:N_TOTAL_SCALARS)
            END SELECT
      ...
      ELSE SINGLE_SPEC_IF
         ...
         B1%ZZ_F(1) = 1._EB
         M%TMP(BC%II,BC%JJ,BC%KK) = PBAR_P(BC%KK,B1%PRESSURE_ZONE)/(RSUM0*RHOP(BC%II,BC%JJ,BC%KK))
         SELECT CASE(BC%IOR)
            CASE DEFAULT
               PBAR_F = PBAR_P(BC%KKG,B1%PRESSURE_ZONE)
            CASE (-3,3)
...
SUBROUTINE CALCULATE_ZZ_F(NM,PREDICTOR_FLAG,T,DT,WALL_INDEX,CFACE_INDEX,PARTICLE_INDEX)
...
REAL(EB) :: RVC,M_DOT_PPP_SINGLE,ZZ_GET(1:MAX_SPECIES),DENOM
...
IF (PRESENT(WALL_INDEX)) THEN
   ...
   OBST_INDEX = M%WALL(WALL_INDEX)%OBST_INDEX
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   CFA => M%CFACE(CFACE_INDEX)
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
   ...
   IC  = M%CELL_INDEX(BC%IIG,BC%JJG,BC%KKG)
   OBST_INDEX = 0
ENDIF
...
IF (.NOT.PREDICTOR_FLAG .AND. SF%E_COEFFICIENT>0._EB .AND. I_WATER>0 .AND. (PRESENT(WALL_INDEX).OR.PRESENT(CFACE_INDEX))) THEN
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FRACTION) METHOD_OF_MASS_TRANSFER
      ...
      IF (ABS(B1%T_IGN-T_BEGIN)<SPACING(B1%T_IGN) .AND. ANY(SF%RAMP(1:N_TRACKED_SPECIES)%INDEX>=1)) THEN
         IF (PREDICTOR_FLAG) TSI = T + DT
         IF (.NOT.PREDICTOR_FLAG) TSI = T
      ELSE
         IF (PREDICTOR_FLAG) TSI = T + DT - B1%T_IGN
         IF (.NOT.PREDICTOR_FLAG) TSI = T      - B1%T_IGN
      ENDIF
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (SF%N_QDOTPP_REF > 0 .AND. B1%T_IGN <=T .AND. PREDICTOR_FLAG) THEN
      ...
      IF (T < B1%T_IGN .OR. B1%T_IGN+B1%BURN_DURATION<T .OR. INITIALIZATION_PHASE) THEN
         B1%ZZ_F(1:N_TRACKED_SPECIES) = B1%ZZ_G(1:N_TRACKED_SPECIES)
         IF (PREDICTOR_FLAG) B1%U_NORMAL_S = 0._EB
         IF (.NOT.PREDICTOR_FLAG) B1%U_NORMAL  = 0._EB
      ...
      IF (SF%N_QDOTPP_REF > 0 .AND. N_REACTIONS>=1 .AND. PREDICTOR_FLAG .AND. B1%T_IGN <=T) THEN
      ...
      IF (.NOT.PREDICTOR_FLAG .AND. SF%EMBER_YIELD>0._EB .AND. (PRESENT(WALL_INDEX).OR.PRESENT(CFACE_INDEX))) THEN
         N = SF%PART_INDEX
      ...
      CONSUME_MASS: IF (PRESENT(WALL_INDEX) .AND. .NOT.PREDICTOR_FLAG) THEN
      ...
      ELSEIF (B1%LAYER_REMOVED .OR. (PRESENT(WALL_INDEX) .AND. .NOT.CELL(IC)%SOLID .AND. .NOT.CELL(IC)%EXTERIOR)) THEN
         ...
         UN = 0._EB
         B1%ZZ_F(:) = B1%ZZ_G(:)
         IF (.NOT.PREDICTOR_FLAG) THEN  ! calculate the mass production rate of gases in the adjacent gas cell
            RVC = RDX(BC%IIG)*RRN(BC%IIG)*RDY(BC%JJG)*RDZ(BC%KKG)
            DO NS=1,N_TRACKED_SPECIES
               IF (ABS(B1%M_DOT_G_PP_ADJUST(NS))<=TWENTY_EPSILON_EB) CYCLE
               MW_RATIO = SPECIES_MIXTURE(NS)%RCON/M%RSUM(BC%IIG,BC%JJG,BC%KKG)
      ...
      IF (PREDICTOR_FLAG) B1%U_NORMAL_S = -UN
      IF (.NOT.PREDICTOR_FLAG) B1%U_NORMAL   = -UN
...
INTEGER, INTENT(IN) :: NM
REAL(EB) :: RADIUS,M_DOT_SINGLE,CP,MW_RATIO,H_G,ZZ_GET(1:MAX_SPECIES),M_GAS,LENGTH,WIDTH,H_S_B,RSUM_G
INTEGER :: NS
TYPE(MESH_TYPE), POINTER :: M
...
DO NS=1,N_TRACKED_SPECIES
   ...
   M%D_SOURCE(BC%IIG,BC%JJG,BC%KKG) = M%D_SOURCE(BC%IIG,BC%JJG,BC%KKG) + M_DOT_SINGLE*(MW_RATIO/M_GAS)
   M%M_DOT_PPP(BC%IIG,BC%JJG,BC%KKG,NS) = M%M_DOT_PPP(BC%IIG,BC%JJG,BC%KKG,NS) + M_DOT_SINGLE*LP%RVC
   ZZ_GET = 0._EB
   ZZ_GET(NS) = 1._EB
...
M%D_SOURCE(BC%IIG,BC%JJG,BC%KKG) = M%D_SOURCE(BC%IIG,BC%JJG,BC%KKG) - B1%Q_DOT_G_PP*B1%AREA/H_G * LP%PWT

! Calculate contribution to divergence term due to convective heat transfer from particle

M%D_SOURCE(BC%IIG,BC%JJG,BC%KKG) = M%D_SOURCE(BC%IIG,BC%JJG,BC%KKG) - B1%Q_CON_F*B1%AREA/H_G * LP%PWT
...
!> \brief Process wall boundary conditions for cells without cross-mesh dependencies
!> \param M Mesh data structure
!> \param NM Mesh index (for compatibility with callees)
!> \param PREDICTOR_FLAG Logical flag for predictor vs corrector step
!> \param T Current time
!> \param DT Current time step
!> \param CALL_HT_1D Flag to call 1-D heat transfer
!>
!> This kernel processes the majority of wall cells (~90%) that do not require
!> cross-mesh access (no INTERPOLATED_BC or BACK_MESH coupling).

SUBROUTINE WALL_BC_PROCESS_CELLS_KERNEL(M,NM,PREDICTOR_FLAG,T,DT,DT_BC,CALL_HT_1D)
...
! Process WALL cells (skip cells with cross-mesh dependencies)
WALL_CELL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS

   WC => M%WALL(IW)

   ! Skip cells requiring sequential cross-mesh processing
   IF (WC%HAS_INTERPOLATED_BC .OR. WC%HAS_BACK_MESH) CYCLE WALL_CELL_LOOP
   ...
   IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      CALL SURFACE_HEAT_TRANSFER(NM,PREDICTOR_FLAG,T,SF,BC,B1,WALL_INDEX=IW)
   ...
   IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY .AND. &
      ...
      CALL WALL_MODEL(SLIP_COEF,B2%U_TAU,B2%Y_PLUS, &
                      M%MU_DNS(BC%IIG,BC%JJG,BC%KKG)/M%RHO(BC%IIG,BC%JJG,BC%KKG), &
                      SF%ROUGHNESS,0.5_EB/B1%RDN,B1%U_TANG)
   ENDIF

   IF (DEPOSITION .AND. .NOT.INITIALIZATION_PHASE .AND. .NOT.PREDICTOR_FLAG .AND. .NOT.SOLID_PHASE_ONLY) THEN
      IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY .AND. &
   ...
   IF (WC%BOUNDARY_TYPE/=OPEN_BOUNDARY .AND. WC%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY) &
      CALL CALCULATE_ZZ_F(NM,PREDICTOR_FLAG,T,DT,WALL_INDEX=IW)
...
! Process CFACE cells (immersed boundaries)
CFACE_LOOP: DO ICF=M%INTERNAL_CFACE_CELLS_LB+1,M%INTERNAL_CFACE_CELLS_LB+M%N_INTERNAL_CFACE_CELLS
   ...
   CALL CFACE_THERMAL_GASVARS(M,ICF,SF,B1,T)

   IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      CALL SURFACE_HEAT_TRANSFER(NM,PREDICTOR_FLAG,T,SF,BC,B1,CFACE_INDEX=ICF)
   ELSEIF (CALL_HT_1D) THEN
      B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(M,T,B1%TMP_G-B1%TMP_F,SF,CFACE_INDEX_IN=ICF)
      CALL SOLID_HEAT_TRANSFER(NM,T,DT_BC,CFACE_INDEX=ICF)
   ENDIF

   IF (N_TRACKED_SPECIES>1) CALL CALCULATE_RHO_D_F(M,B1,BC,CFACE_INDEX=ICF)
   ...
   IF (DEPOSITION .AND. .NOT.INITIALIZATION_PHASE .AND. .NOT.PREDICTOR_FLAG .AND. .NOT.SOLID_PHASE_ONLY) THEN
      IF (CFA%BOUNDARY_TYPE==SOLID_BOUNDARY .AND. &
         (ANY(SF%LEAK_PATH>0) .OR. &
         (B1%NODE_INDEX==0 .AND. SF%VEL<TWENTY_EPSILON_EB .AND. SF%VOLUME_FLOW<TWENTY_EPSILON_EB))) THEN
         CALL CALC_DEPOSITION(M,DT,BC,B1,B2,CFACE_INDEX=ICF)
      ENDIF
   ENDIF
   ...
   CALL CALCULATE_ZZ_F(NM,PREDICTOR_FLAG,T,DT,CFACE_INDEX=ICF)
   CALL CALCULATE_RHO_F_KERNEL(M,BC,B1,CFACE_INDEX=ICF)

ENDDO CFACE_LOOP

! Process SOLID PARTICLES (skip DEPOSIT_PARTICLE_MASS - handled in Phase 3)
IF (SOLID_PARTICLES) THEN
   ...
   PARTICLE_LOOP: DO IP=1,M%NLP
      ...
      CALL NEAR_SURFACE_GAS_VARIABLES_KERNEL(M,T,SF,BC,B1,LP=LP,PARTICLE_INDEX=IP)

      IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
         CALL SURFACE_HEAT_TRANSFER(NM,PREDICTOR_FLAG,T,SF,BC,B1,PARTICLE_INDEX=IP)
      ELSEIF (CALL_HT_1D) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(M,T,B1%TMP_G-B1%TMP_F,SF,PARTICLE_INDEX_IN=IP)
         CALL SOLID_HEAT_TRANSFER(NM,T,DT_BC,PARTICLE_INDEX=IP)
      ENDIF

      IF (LPC%SOLID_PARTICLE) THEN
         CALL CALCULATE_ZZ_F(NM,PREDICTOR_FLAG,T,DT,PARTICLE_INDEX=IP)
         ! Note: DEPOSIT_PARTICLE_MASS is NOT called here - handled in Phase 3 finalization
      ENDIF
...
!> \brief Process wall boundary conditions for a K-range sub-block of a mesh
!> \param M Mesh data structure
!> \param NM Mesh index
!> \param PREDICTOR_FLAG True if predictor phase
!> \param T Current time
!> \param DT Time step
!> \param DT_BC Boundary condition time step
!> \param CALL_HT_1D Flag to call 1-D heat transfer
!> \param K1 K-range start (1-based inclusive)
!> \param K2 K-range end (1-based inclusive)
!>
!> Same as WALL_BC_PROCESS_CELLS_KERNEL but filters wall cells, CFACE cells,
!> and particles by their gas cell K-coordinate (KKG) to only process those
!> in [K1,K2].

RECURSIVE SUBROUTINE WALL_BC_PROCESS_CELLS_BLOCK_KERNEL(M,NM,PREDICTOR_FLAG,T,DT,DT_BC,CALL_HT_1D,K1,K2)
...
! Process WALL cells (filter by KKG range, skip cross-mesh dependencies)
WALL_CELL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   ! K-block filter: skip wall cells outside this block's K-range
   IF (BC%KKG < K1 .OR. BC%KKG > K2) CYCLE WALL_CELL_LOOP
...
! Process CFACE cells (immersed boundaries, filter by KKG range)
IF (CC_IBM) THEN
CFACE_LOOP: DO ICF=M%INTERNAL_CFACE_CELLS_LB+1,M%INTERNAL_CFACE_CELLS_LB+M%N_INTERNAL_CFACE_CELLS
   CFA => M%CFACE(ICF)
   IF (CFA%BOUNDARY_TYPE==NULL_BOUNDARY) CYCLE CFACE_LOOP
   ...
   ! K-block filter: skip CFACE cells outside this block's K-range
   IF (BC%KKG < K1 .OR. BC%KKG > K2) CYCLE CFACE_LOOP
...
! Process SOLID PARTICLES (filter by KKG range)
IF (SOLID_PARTICLES) THEN
   ...
   PARTICLE_LOOP: DO IP=1,M%NLP
      ...
      ! K-block filter: skip particles outside this block's K-range
      IF (BC%KKG < K1 .OR. BC%KKG > K2) CYCLE PARTICLE_LOOP
...
END SUBROUTINE WALL_BC_PROCESS_CELLS_BLOCK_KERNEL
...
! Process wall cells with INTERPOLATED_BOUNDARY (periodic/inter-mesh).
! These were skipped in WALL_BC_PROCESS_CELLS_KERNEL because SURFACE_HEAT_TRANSFER
! for INTERPOLATED_BC accesses OMESH data. Process them here sequentially.
WALL_INTERP_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS

   WC => M%WALL(IW)

   IF (.NOT.WC%HAS_INTERPOLATED_BC) CYCLE WALL_INTERP_LOOP
   IF (WC%BOUNDARY_TYPE==NULL_BOUNDARY) CYCLE WALL_INTERP_LOOP
   ...
   IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      CALL SURFACE_HEAT_TRANSFER(NM,M%PREDICTOR,T,SF,BC,B1,WALL_INDEX=IW)
   ELSEIF (CALL_HT_1D) THEN
      CALL SOLID_HEAT_TRANSFER(NM,T,SF%HT_DIM*DT_BC,WALL_INDEX=IW)
   ENDIF

ENDDO WALL_INTERP_LOOP
...
WALL_FINALIZE_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
...
IF (SOLID_PARTICLES .AND. M%CORRECTOR) THEN

   PARTICLE_FINALIZE_LOOP: DO IP=1,M%NLP

      LP => M%LAGRANGIAN_PARTICLE(IP)
      LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)

      IF (LPC%SOLID_PARTICLE) THEN
         CALL DEPOSIT_PARTICLE_MASS(NM,LP,LPC)
      ENDIF
...
!> \brief Wall BC preprocessing - sets up gas phase variables before main processing
!> \param NM Mesh index
!> \param T Current time
!> \param DT_BC Boundary condition time step (only used if CALL_HT_1D is true)
!> \param CALL_HT_1D Flag to call 1-D heat transfer
!>
!> This routine must be called sequentially before WALL_BC_PROCESS_CELLS_KERNEL.
!> Handles ASSIGN_GHOST_VALUE, NEAR_SURFACE_GAS_VARIABLES_KERNEL, and
!> HEAT_TRANSFER_COEFFICIENT setup for all wall cells.
...
! Run through thermally-thick WALL and THIN_WALL cells and set gas phase near-surface values
! and heat transfer coefficient. This must be done before SOLID_HEAT_TRANSFER to avoid data races.
...
END SUBROUTINE WALL_BC_PREPROCESSING
...
REAL(EB) :: ZZ_GET(1:MAX_SPECIES),UN,MFT,RSUM_F,H_D,H_G
INTEGER  :: COUNTER,DU
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: B1
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE(SURFACE_TYPE), POINTER :: SF
...
IF (MFT >= 0._EB) THEN
   ...
   UN = MFT/B1%RHO_F
   IF (MESHES(LOWER_MESH_INDEX)%PREDICTOR) B1%U_NORMAL_S = UN
   IF (MESHES(LOWER_MESH_INDEX)%CORRECTOR) B1%U_NORMAL  = UN
ELSE
...
REAL(EB) :: RDT,DTMP,QDXKF,QDXKB,RR,RFACF,RFACB,RFACF2,RFACB2,Q_RAD_IN_B,RFLUX_UP,RFLUX_DOWN,EMISSIVITY_BACK, &
            ...
            RADIUS,HTC_LIMIT,CP1,CP2,DENOM,THICKNESS,DT_FO,DDSUM,NODE_RDT(MAX_WALL_CELLS_DIM),RMR
REAL(EB), DIMENSION(MAX_SPECIES) :: M_DOT_G_PP_ADJUST,M_DOT_G_PP_ADJUST_NET,M_DOT_G_PP_ACTUAL,M_DOT_G_PP_ACTUAL_NET
REAL(EB), DIMENSION(MAX_MATERIALS) :: M_DOT_S_PP,M_DOT_S_PP_NET,T_BOIL_EFF
REAL(EB), DIMENSION(MAX_LPC) :: Q_DOT_PART_S,M_DOT_PART_S
REAL(EB), DIMENSION(MAX_WALL_CELLS_DIM) :: TMP_S,RHO_H_S
...
REAL(EB), DIMENSION(MAX_WALL_CELLS_DIM) :: AAS,BBS,CCS,DDS,DDT,Q_S,Q_IR,Q_ADD,TWO_DX_KAPPA_S,DX_S,MF_FRAC,REGRID_FACTOR,DX_S_NEW
REAL(EB), DIMENSION(0:MAX_WALL_CELLS_DIM+1) :: RHO_S,DELTA_TMP,RDX_S,TMP_NEW
REAL(EB), DIMENSION(0:MAX_WALL_CELLS_DIM) :: X_S_NEW,RDXN_S,R_S,R_S_NEW,DX_WGT_S
REAL(EB), DIMENSION(MAX_LAYERS) :: LAYER_MASS
INTEGER, DIMENSION(0:MAX_WALL_CELLS_DIM+1) :: LAYER_INDEX
...
LOGICAL :: ISOLATED_THIN_WALL,ISOLATED_THIN_WALL_BACK,REMESH_LAYER(MAX_LAYERS_HT3D),REMESH_CHECK,DIRICHLET_BACK,&
           CELL_ZERO(MAX_LAYERS_HT3D),TMP_CHECK(MAX_LAYERS_HT3D),CELL_ZERO_CELL(MAX_WALL_CELLS_DIM)
...
! Thread-safe: local pointer aliases shadow module-level MESH_POINTERS aliases (Pattern 3)
TYPE(MESH_TYPE), POINTER :: M
...
TYPE(OBSTRUCTION_TYPE), POINTER, DIMENSION(:) :: OBSTRUCTION
REAL(EB), POINTER, DIMENSION(:) :: TMP_0
REAL(EB), POINTER, DIMENSION(:,:,:) :: TMP
INTEGER, POINTER, DIMENSION(:,:,:) :: CELL_INDEX
TYPE(CELL_TYPE), POINTER, DIMENSION(:) :: CELL
INTEGER :: HT_3D_SWEEP_DIRECTION
...
!      NWP                  = SF%N_CELLS_INI
   ...
   SELECT CASE(BACKING)
      ...
      CASE(VOID)  ! Non-insulated backing to an ambient void
         ...
         HTCB = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,DTMP,SF,BACK_SIDE=.TRUE.)
         Q_CON_B = HTCB*DTMP
   ...
   IF (SF%BOUNDARY_FUEL_MODEL) THEN
      ...
      DO I=1,NWP
         ...
         IF (PRESENT(WALL_INDEX)) THEN
            HTCF = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,DTMP,SF,WALL_INDEX_IN=WALL_INDEX)
         ELSEIF(PRESENT(CFACE_INDEX)) THEN
            HTCF = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,DTMP,SF,CFACE_INDEX_IN=CFACE_INDEX)
         ENDIF
...
REAL(EB), DIMENSION(MAX_SPECIES) :: M_DOT_G_PPP_ADJUST,M_DOT_G_PPP_ACTUAL,ZZ_GET
...
O2_LOOP: DO ITER=1,MAX_ITER
   ...
   POINT_LOOP1: DO I=1,NWP
      ...
      IF (PRESENT(PARTICLE_INDEX)) THEN
         CALL PYROLYSIS(MESHES(NM),ONE_D%N_MATL,ONE_D%MATL_INDEX,SURF_INDEX,&
                        BC%IIG,BC%JJG,BC%KKG,ONE_D%TMP(I),B1%TMP_F,Y_O2_F,BC%IOR,&
                        RHO_DOT(1:ONE_D%N_MATL,I),RHO_TEMP(1:ONE_D%N_MATL),&
                        ONE_D%X(I-1),ASH_DEPTH,DX_S,DT_BC_SUB,&
                        M_DOT_G_PPP_ADJUST,M_DOT_G_PPP_ACTUAL,M_DOT_S_PPP,&
                        Q_S(I),Q_DOT_G_PPP,Q_DOT_O2_PPP,&
                        Q_DOT_PART,M_DOT_PART,T_BOIL_EFF,B1%B_NUMBER,&
                        LAYER_INDEX(I),REMOVE_LAYER,ONE_D,B1,&
                        SOLID_CELL_INDEX=I,&
      ...
      ELSE
         CALL PYROLYSIS(MESHES(NM),ONE_D%N_MATL,ONE_D%MATL_INDEX,SURF_INDEX,&
                        BC%IIG,BC%JJG,BC%KKG,ONE_D%TMP(I),B1%TMP_F,Y_O2_F,BC%IOR,&
                        RHO_DOT(1:ONE_D%N_MATL,I),RHO_TEMP(1:ONE_D%N_MATL),&
                        0.5_EB*(ONE_D%X(I-1)+ONE_D%X(I)),ASH_DEPTH,DX_S,&
                        DT_BC_SUB,M_DOT_G_PPP_ADJUST,M_DOT_G_PPP_ACTUAL,&
                        M_DOT_S_PPP,Q_S(I),Q_DOT_G_PPP,Q_DOT_O2_PPP,&
                        Q_DOT_PART,M_DOT_PART,T_BOIL_EFF,B1%B_NUMBER,&
                        LAYER_INDEX(I),REMOVE_LAYER,ONE_D,B1,SOLID_CELL_INDEX=I)
      ENDIF
