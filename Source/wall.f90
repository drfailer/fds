...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
   ...
   IF (LPC%MASSLESS_TARGET) THEN  ! the particle's sole purpose is to record a heat flux
      ...
      ELSE  ! the HTC will be computed using the surrounding gas phase environment
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,B1%TMP_G-PY%GAUGE_TEMPERATURE,SF, &
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,DTMP,SF,WALL_INDEX_IN=WALL_INDEX)
      ELSEIF (PRESENT(CFACE_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,DTMP,SF,CFACE_INDEX_IN=CFACE_INDEX)
      ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,DTMP,SF,PARTICLE_INDEX_IN=PARTICLE_INDEX)
      ENDIF
