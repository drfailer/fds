...
!>
!> Same as WALL_BC_PROCESS_CELLS_KERNEL but filters wall cells and particles
!> by their gas cell K-coordinate (KKG) to only process those in [K1,K2].
!> CFACE cells are skipped (CC_IBM excluded from block decomposition).
...
! CFACE cells skipped (CC_IBM excluded from block decomposition)
