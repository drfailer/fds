...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (N_TRACKED_SPECIES>1 .AND. WC%BOUNDARY_TYPE/=OPEN_BOUNDARY .AND. WC%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY) THEN
      CALL CALCULATE_RHO_D_F(MESHES(NM),B1,BC,WALL_INDEX=IW)
   ENDIF
   ...
   IF (DEPOSITION .AND. .NOT.INITIALIZATION_PHASE .AND. CORRECTOR .AND. .NOT.SOLID_PHASE_ONLY) THEN
      IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY .AND. &
         ...
         CALL CALC_DEPOSITION(MESHES(NM),DT,BC,B1,B2,WALL_INDEX=IW)
      ENDIF
   ENDIF
   ...
   IF (WC%BOUNDARY_TYPE/=OPEN_BOUNDARY .AND. WC%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY) &
      CALL CALCULATE_ZZ_F(NM,PREDICTOR,T,DT,WALL_INDEX=IW)

   IF (WC%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY) &
      CALL CALCULATE_RHO_F_KERNEL(MESHES(NM),BC,B1, &
         WALL_INDEX=IW)
...
CFACE_LOOP: DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      CALL SURFACE_HEAT_TRANSFER(NM,PREDICTOR,T,SF,BC,B1,CFACE_INDEX=ICF)
   ELSEIF (CALL_HT_1D) THEN
      B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,B1%TMP_G-B1%TMP_F,SF,CFACE_INDEX_IN=ICF)
      CALL SOLID_HEAT_TRANSFER(NM,T,DT_BC,CFACE_INDEX=ICF)
   ENDIF

   IF (N_TRACKED_SPECIES>1) CALL CALCULATE_RHO_D_F(MESHES(NM),B1,BC,CFACE_INDEX=ICF)
   ...
   IF (DEPOSITION .AND. .NOT.INITIALIZATION_PHASE .AND. CORRECTOR .AND. .NOT.SOLID_PHASE_ONLY) THEN
      IF (CFA%BOUNDARY_TYPE==SOLID_BOUNDARY .AND. &
         (ANY(SF%LEAK_PATH>0) .OR. &
         (B1%NODE_INDEX==0 .AND. SF%VEL<TWENTY_EPSILON_EB .AND. SF%VOLUME_FLOW<TWENTY_EPSILON_EB))) THEN
         CALL CALC_DEPOSITION(MESHES(NM),DT,BC,B1,B2,CFACE_INDEX=ICF)
      ENDIF
   ENDIF
   ...
   CALL CALCULATE_ZZ_F(NM,PREDICTOR,T,DT,CFACE_INDEX=ICF)
   CALL CALCULATE_RHO_F_KERNEL(MESHES(NM),BC,B1, &
      CFACE_INDEX=ICF)
...
IF (SOLID_PARTICLES) THEN
   ...
   PARTICLE_LOOP: DO IP=1, NLP
      ...
      CALL NEAR_SURFACE_GAS_VARIABLES_KERNEL(MESHES(NM), &
         T,SF,BC,B1,LP=LP,PARTICLE_INDEX=IP)

      IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
         CALL SURFACE_HEAT_TRANSFER(NM,PREDICTOR,T,SF,BC,B1,PARTICLE_INDEX=IP)
      ELSEIF (CALL_HT_1D) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,B1%TMP_G-B1%TMP_F,SF,PARTICLE_INDEX_IN=IP)
         CALL SOLID_HEAT_TRANSFER(NM,T,DT_BC,PARTICLE_INDEX=IP)
      ENDIF

      IF (LPC%SOLID_PARTICLE) THEN
         CALL CALCULATE_ZZ_F(NM,PREDICTOR,T,DT,PARTICLE_INDEX=IP)
