...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
      ...
      ELSE
         IF (SF%BLOWING) THEN
            IF (OPT_WALL_INDEX > 0) THEN
               HTC=HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,TMP(BC%IIG,BC%JJG,BC%KKG)-B1%TMP_F,SF,WALL_INDEX_IN=OPT_WALL_INDEX,&
                                             SKIP_BLOWING=.TRUE.)
            ELSEIF (OPT_CFACE_INDEX > 0) THEN
               HTC=HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,TMP(BC%IIG,BC%JJG,BC%KKG)-B1%TMP_F,SF,CFACE_INDEX_IN=OPT_CFACE_INDEX,&
                                             SKIP_BLOWING=.TRUE.)
            ELSE
               HTC=HEAT_TRANSFER_COEFFICIENT(MESHES(NM),T,TMP(BC%IIG,BC%JJG,BC%KKG)-B1%TMP_F,SF,PARTICLE_INDEX_IN=OPT_LP_INDEX,&
                                             SKIP_BLOWING=.TRUE.)
            ENDIF
   ...
   CASE(66) ! VISCOUS STRESS WALL (takes optional FORCE_DIRECTION vector)
      ...
      ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
         ...
         IND1 = CFA%CUT_FACE_IND1
         IND2 = CFA%CUT_FACE_IND2
         CALL GET_UVWGAS_CFACE(MESHES(NM),U_CELL,V_CELL,W_CELL,IND1,IND2,U,V,W,PREDFCT=1._EB)
