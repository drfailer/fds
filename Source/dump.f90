...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,WRITE_STATUS_FILES, &
       ...
       WRITE_SMOKEVIEW_FILE,DUMP_MESH_OUTPUTS,DUMP_MESH_OUTPUTS_TS,UPDATE_GLOBAL_OUTPUTS,DUMP_DEVICES,DUMP_HRR,&
       ...
       UPDATE_DEVICES_1_TS,UPDATE_HRR_MASS,UPDATE_HRR_TS,UPDATE_MASS_TS,REDUCE_HRR_MASS
...
!> \brief Sequential part of UPDATE_GLOBAL_OUTPUTS: HRR + MASS + FIRE_SPREAD accumulation.
!> Called from CorrFinalCollector after UPDATE_DEVICES_1_TS runs in parallel.

SUBROUTINE UPDATE_HRR_MASS(T,DT,NM)
...
END SUBROUTINE UPDATE_HRR_MASS


SUBROUTINE REDUCE_HRR_MASS(DT)
REAL(EB), INTENT(IN) :: DT
INTEGER :: NM
Q_DOT = 0._EB
M_DOT = 0._EB
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   Q_DOT = Q_DOT + Q_DOT_MESH(:,NM)
   M_DOT = M_DOT + M_DOT_MESH(:,NM)
   MASS_DT = MASS_DT + DT*MASS_INTEGRAL_MESH(:,NM)
ENDDO
Q_DOT_SUM = Q_DOT_SUM + DT*Q_DOT
M_DOT_SUM = M_DOT_SUM + DT*M_DOT
END SUBROUTINE REDUCE_HRR_MASS
...
SMOKE3D_INITIALIZATION: IF (SMOKE3D) THEN
   ...
   DO N=1,N_SMOKE3D
      ...
      ELSE
         ...
         OPEN(LU_SMOKE3D(N+N_SMOKE3D,NM),FILE=FN_SMOKE3D(N+N_SMOKE3D,NM),FORM='FORMATTED',STATUS='OLD',&
              POSITION='APPEND',DECIMAL=DECIMAL_SPECIFIER)
         IF (SMOKE3D_FILE(N)%DISPLAY_TYPE=='GAS') THEN
            OPEN(LU_SMOKE3D(N+2*N_SMOKE3D,NM),FILE=FN_SMOKE3D(N+2*N_SMOKE3D,NM),FORM='UNFORMATTED',&
...
DO N=1,M%N_SLCF
   ...
   ! write out slice file info to .sf files
      ...
      OPEN(LU_SLCF(N+N_SLCF_MAX,NM),  FILE=FN_SLCF(N + N_SLCF_MAX,NM),  FORM='FORMATTED',  STATUS='REPLACE',&
           ACTION='READWRITE',DECIMAL=DECIMAL_SPECIFIER)
  ...
  ! i1,i2,j1,j2,k1,k2
   ...
   ELSE
      IF (SL%SLICETYPE=='STRUCTURED') THEN
         ...
         IF (SL%RLE) OPEN(LU_SLCF(N+2*N_SLCF_MAX,NM),FILE=FN_SLCF(N+2*N_SLCF_MAX,NM),&
              FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
      ENDIF
      OPEN(LU_SLCF(N+N_SLCF_MAX,NM),FILE=FN_SLCF(N+N_SLCF_MAX,NM),FORM='FORMATTED',STATUS='OLD',&
           ACTION='READWRITE',DECIMAL=DECIMAL_SPECIFIER)
   ENDIF
ENDDO
...
BOUNDARY_FILES: DO NF=1,N_BNDF
   ...
   ELSE
      ...
      OPEN(LU_BNDF(NF+N_BNDF,NM),FILE=FN_BNDF(NF+N_BNDF,NM),FORM='FORMATTED',STATUS='OLD',&
           ACTION='READWRITE',DECIMAL=DECIMAL_SPECIFIER)
   ENDIF
...
PARTICLE_IF: IF (PARTICLE_FILE) THEN
   ...
   ELSE
      ...
      OPEN(LU_PART(NM+NMESHES),FILE=FN_PART(NM+NMESHES),FORM='FORMATTED',STATUS='OLD',&
           POSITION='APPEND',DECIMAL=DECIMAL_SPECIFIER)
   ENDIF
ENDIF PARTICLE_IF
...
USE FIRE_KERNELS, ONLY: GET_FLAME_TEMPERATURE
REAL(EB), INTENT(IN) :: DT
INTEGER :: NM,I,NN,N,NR,NL,NS,ITMP, CELL_COUNT,KK
REAL(EB) :: ZZ_GET(1:MAX_SPECIES),ZZ_REAC(1:MAX_SPECIES),ZZ_PROD(1:MAX_SPECIES),&
...
USE CC_PRESSURE, ONLY: COPY_UNST_DM_TO_CART
REAL(EB), INTENT(IN) :: T,DT
REAL(EB) :: STIME
...
USE TURB_KERNELS, ONLY: FILL_EDGES_KERNEL
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM
REAL(EB) :: SUM
REAL(FB) :: STIME
...
IF (IS%DEBUG) THEN
   ...
   DO K=0,M%KBAR
      DO J=0,M%JBAR
         DO I=0,M%IBAR
            M%QQ(I,J,K,1) = SQRT( (M%XPLT(I)-ISO_CENX)**2 + (M%YPLT(J)-ISO_CENY)**2 + (M%ZPLT(K)-ISO_CENZ)**2)
         ENDDO
      ENDDO
   ENDDO
...
ELSE
   ...
   CALL FILL_EDGES_KERNEL(M,QUANTITY)
   ...
   DO K=0,M%KBAR
      DO J=0,M%JBAR
         DO I=0,M%IBAR
            M%QQ(I,J,K,1) = REAL(S(I,J,K)*(QUANTITY(I,J,K)*B(I,J,K)        + QUANTITY(I+1,J,K)*B(I+1,J,K)+ &
...
INDEX2_IF: IF ( IS%INDEX2 /= -1 ) THEN
   ...
   IF (IS%DEBUG) THEN
      ...
      DO K=0,M%KBAR+1
         IF (K.EQ.M%KBAR+1) THEN
            ZZ = 2.0_FB*M%ZPLT(M%KBAR) - M%ZPLT(M%KBAR-1)
         ELSE
            ZZ = M%ZPLT(K)
         ENDIF
         DO J=0,M%JBAR+1
   ...
   ELSE
      ...
      CALL FILL_EDGES_KERNEL(M,QUANTITY2)
      ...
      DO KK=0,M%KBAR+1
         ...
         DO JJ=0,M%JBAR+1
            ...
            DO II=0,M%IBAR+1
               I = MIN(II, M%IBAR)
               M%QQ2(I,J,K,1) = REAL(S(I,J,K)*(QUANTITY2(I,J,K)*B(I,J,K)        + QUANTITY2(I+1,J,K)*B(I+1,J,K)+ &
...
IF (IS%DEBUG) THEN
   ...
   ISO_NLEVEL = 1
   CALL ISO_TO_FILE(LU_ISOF(N,NM),LU_ISOF2(N,NM),NM,M%IBAR,M%JBAR,M%KBAR,STIME,M%QQ,M%QQ2,HAVE_ISO2,&
        ISO_LEVEL(1:ISO_NLEVEL), ISO_NLEVEL, M%IBLK, IS%SKIP, IS%DELTA, M%XPLT, M%IBP1, M%YPLT, M%JBP1, M%ZPLT, M%KBP1)
ELSE
   CALL ISO_TO_FILE(LU_ISOF(N,NM),LU_ISOF2(N,NM),NM,M%IBAR,M%JBAR,M%KBAR,STIME,M%QQ,M%QQ2,HAVE_ISO2,&
        IS%VALUE(1:IS%N_VALUES), IS%N_VALUES, M%IBLK, IS%SKIP, IS%DELTA, M%XPLT, M%IBP1, M%YPLT, M%JBP1, M%ZPLT, M%KBP1)
ENDIF

FLUSH(ABS(LU_ISOF(N,NM)))
IF (IS%INDEX2 /= -1 ) FLUSH(ABS(LU_ISOF2(N,NM)))
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   ALLOCATE(QQ_PACK(M%IBP1*M%JBP1*M%KBP1))
   QQ_PACK = PACK(M%QQ(0:M%IBAR,0:M%JBAR,0:M%KBAR,1),MASK=.TRUE.)
   CALL SMOKE3D_TO_FILE(NM,STIME,DXX,N,QQ_PACK,M%IBP1,M%JBP1,M%KBP1)
   DEALLOCATE(QQ_PACK)
...
DO K=0,M%KBAR
   DO J=0,M%JBAR
      DO I=0,M%IBAR
         M%QQ(I,J,K,1) = REAL((FF(I,J,K)  +FF(I+1,J,K)  +FF(I,J,K+1)  +FF(I+1,J,K+1)+ &
...
IF (CC_IBM) THEN
   DO K=0,M%KBAR
      DO J=0,M%JBAR
         DO I=0,M%IBAR
            IF(M%VERTVAR(I,J,K,CC_VGSC) /= CC_SOLID) CYCLE
            M%QQ(I,J,K,1) = 0._FB
         ENDDO
      ENDDO
   ENDDO
...
IF (M%N_SLCF==0 .AND. .NOT.PLOT3D) RETURN
...
DO IC=1,CELL_COUNT(NM)
   IF (M%CELL(IC)%SOLID) B(M%CELL(IC)%I,M%CELL(IC)%J,M%CELL(IC)%K) = 0._EB
   IF (M%CELL(IC)%EXTERIOR) THEN
      IF (M%CELL(IC)%EXTERIOR_EDGE) THEN
         B(M%CELL(IC)%I,M%CELL(IC)%J,M%CELL(IC)%K) = 0._EB
      ELSE
         CALL SEARCH_OTHER_MESHES(M%XC(M%CELL(IC)%I),M%YC(M%CELL(IC)%J),M%ZC(M%CELL(IC)%K),NOM,IIO,JJO,KKO)
         IF (NOM==0) THEN
            B(M%CELL(IC)%I,M%CELL(IC)%J,M%CELL(IC)%K) = 0._EB
         ELSE
            M2 => MESHES(NOM)
            ICO = M2%CELL_INDEX(IIO,JJO,KKO)
            IF (M2%CELL(ICO)%SOLID) B(M%CELL(IC)%I,M%CELL(IC)%J,M%CELL(IC)%K) = 0._EB
         ENDIF
      ENDIF
   ENDIF
ENDDO
...
IF (PLOT3D) THEN  ! Write out information to .smv file
   ...
   IF (M%N_STRINGS+17>M%N_STRINGS_MAX) THEN
      CALL RE_ALLOCATE_STRINGS(NM)
   ENDIF
   M%N_STRINGS = M%N_STRINGS + 1
   WRITE(M%STRING(M%N_STRINGS),'(A,I8,A,I2.2,I6)')  'PL3D ',ITM,'.',ITM1,NM
   M%N_STRINGS = M%N_STRINGS + 1
   WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(FN_PL3D(NM))
   DO IQ=1,5
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(PLOT3D_SMOKEVIEW_LABEL(IQ))
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(PLOT3D_SMOKEVIEW_BAR_LABEL(IQ))
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(OUTPUT_QUANTITY(PLOT3D_QUANTITY_INDEX(IQ))%UNITS)
   ENDDO
...
IF (AGL_TERRAIN_SLICE) THEN
   ...
   DO J=J1,J2
      DO I=I1,I2
         M%QQ(I,J,K1,IQQ) = REAL(0.25_EB*(QUANTITY(I,J      ,K1)+QUANTITY(I+I_INC,J      ,K1)+&
...
ELSEIF (CC_CELL_CENTERED) THEN
   ...
   DO K=KK1,KK2
      DO J=JJ1,JJ2
         DO I=II1,II2
            M%QQ(I,J,K,IQQ) = REAL(QUANTITY(I,J,K),FB)
         ENDDO
      ENDDO
   ENDDO
   ...
   DO K=K1,K2
      DO J=J1,J2
         DO I=I1,I2
            SELECT CASE(OUTPUT_QUANTITY(IND)%CELL_POSITION)
               CASE(CELL_CENTER)
                  M%QQ(I,J,K,IQQ) = REAL(CORNER_VALUE(QUANTITY,B,S,IND),FB)
               CASE(CELL_FACE)
                  M%QQ(I,J,K,IQQ) = REAL(FACE_VALUE(),FB)
               CASE(CELL_EDGE)
                  M%QQ(I,J,K,IQQ) = REAL(EDGE_VALUE(QUANTITY,S,IND),FB)
            END SELECT
         ENDDO
      ENDDO
   ENDDO
...
IF (.NOT.PLOT3D) THEN
   ...
   IF (SL%SLICETYPE=='STRUCTURED') THEN ! write out slice file using original slice file format
      ...
      IF (.NOT. SL%DEBUG) WRITE(LU_SLCF(IQ,NM)) (((M%QQ(I,J,K,1),I=I1,I2),J=J1,J2),K=K1,K2)
      IF (SL%DEBUG) THEN
         ...
         IF (NX*NY*NZ>0) THEN
            ...
            DO K = K1, K2
               ...
               DO J = J1, J2
                  ...
                  DO I = I1, I2
                     IFACT = (I - I1)*NY*NZ
                     QQ_PACK(1+IFACT+JFACT+KFACT) = M%QQ(I,J,K,1)
                  ENDDO
               ENDDO
            ENDDO
            ...
            DEALLOCATE(QQ_PACK)
         ENDIF
         FLUSH(LU_SLCF(IQ3,NM))
      ENDIF

      IF (.NOT.SL%DEBUG) THEN
         IF (CC_CELL_CENTERED) THEN
            SLICE_MIN = M%QQ(MIN(I1+1,I2),MIN(J1+1,J2),MIN(K1+1,K2),1)
            SLICE_MAX = SLICE_MIN
            DO K = MIN(K1+1,K2), K2
               DO J = MIN(J1+1,J2), J2
                  DO I = MIN(I1+1,I2), I2
                     SLICE_MIN = MIN(SLICE_MIN,M%QQ(I,J,K,1))
                     SLICE_MAX = MAX(SLICE_MAX,M%QQ(I,J,K,1))
                  ENDDO
               ENDDO
            ENDDO
         ELSE
            SLICE_MIN = M%QQ(I1,J1,K1,1)
            SLICE_MAX = SLICE_MIN
            DO K = K1, K2
               DO J = J1, J2
                  DO I = I1, I2
...
IF (PLOT3D) THEN
ZERO = 0._EB
WRITE(LU_PL3D(NM)) M%IBP1,M%JBP1,M%KBP1
WRITE(LU_PL3D(NM)) ZERO,ZERO,ZERO,ZERO
WRITE(LU_PL3D(NM)) ((((M%QQ(I,J,K,IQ),I=0,M%IBAR),J=0,M%JBAR),K=0,M%KBAR),IQ=1,5)
CLOSE(LU_PL3D(NM))
DO IQ = 1, 5
   PLOT3D_MIN = M%QQ(0,0,0,IQ)
   PLOT3D_MAX = PLOT3D_MIN
   DO K = 0, M%KBAR
      DO J = 0, M%JBAR
         DO I = 0, M%IBAR
           PLOT3D_MIN = MIN(PLOT3D_MIN,M%QQ(I,J,K,IQ))
           PLOT3D_MAX = MAX(PLOT3D_MAX,M%QQ(I,J,K,IQ))
         END DO
      END DO
   END DO
...
DO K = 0, M%KBAR
   DO J = 0, M%JBAR
      DO I = 0, M%IBAR
        UVW_MAX = MAX(ABS(M%QQ(I,J,K,2)), ABS(M%QQ(I,J,K,3)), ABS(M%QQ(I,J,K,4)), 1.0_FB)
        UVEL = M%QQ(I,J,K,2)/UVW_MAX
        VVEL = M%QQ(I,J,K,3)/UVW_MAX
        WVEL = M%QQ(I,J,K,4)/UVW_MAX
...
IF (ICMM>0) THEN
   SELECT CASE(IND)
      CASE(6)
         ...
         IF (M%EDGE(IE)%U_AVG>-1.E5_EB) THEN ; AA(0,0)=M%EDGE(IE)%U_AVG ; AA(0,1)=M%EDGE(IE)%U_AVG ; ENDIF
         IE = M%CELL(ICMM)%EDGE_INDEX(12)
         IF (M%EDGE(IE)%U_AVG>-1.E5_EB) THEN ; AA(0,0)=M%EDGE(IE)%U_AVG ; AA(1,0)=M%EDGE(IE)%U_AVG ; ENDIF
         IE = M%CELL(ICPM)%EDGE_INDEX(8)
         IF (M%EDGE(IE)%U_AVG>-1.E5_EB) THEN ; AA(1,0)=M%EDGE(IE)%U_AVG ; AA(1,1)=M%EDGE(IE)%U_AVG ; ENDIF
         IE = M%CELL(ICMP)%EDGE_INDEX(12)
         IF (M%EDGE(IE)%U_AVG>-1.E5_EB) THEN ; AA(0,1)=M%EDGE(IE)%U_AVG ; AA(1,1)=M%EDGE(IE)%U_AVG ; ENDIF
      CASE(7)
         ICPM = M%CELL_INDEX(I+1,J,K)
         ICMP = M%CELL_INDEX(I,J,K+1)
         IE = M%CELL(ICMM)%EDGE_INDEX(4)
         IF (M%EDGE(IE)%V_AVG>-1.E5_EB) THEN ; AA(0,0)=M%EDGE(IE)%V_AVG ; AA(0,1)=M%EDGE(IE)%V_AVG ; ENDIF
         IE = M%CELL(ICMM)%EDGE_INDEX(12)
         IF (M%EDGE(IE)%V_AVG>-1.E5_EB) THEN ; AA(0,0)=M%EDGE(IE)%V_AVG ; AA(1,0)=M%EDGE(IE)%V_AVG ; ENDIF
         IE = M%CELL(ICPM)%EDGE_INDEX(4)
         IF (M%EDGE(IE)%V_AVG>-1.E5_EB) THEN ; AA(1,0)=M%EDGE(IE)%V_AVG ; AA(1,1)=M%EDGE(IE)%V_AVG ; ENDIF
         IE = M%CELL(ICMP)%EDGE_INDEX(12)
         IF (M%EDGE(IE)%V_AVG>-1.E5_EB) THEN ; AA(0,1)=M%EDGE(IE)%V_AVG ; AA(1,1)=M%EDGE(IE)%V_AVG ; ENDIF
      CASE(8)
         ICPM = M%CELL_INDEX(I+1,J,K)
         ICMP = M%CELL_INDEX(I,J+1,K)
         IE = M%CELL(ICMM)%EDGE_INDEX(4)
         IF (M%EDGE(IE)%W_AVG>-1.E5_EB) THEN ; AA(0,0)=M%EDGE(IE)%W_AVG ; AA(0,1)=M%EDGE(IE)%W_AVG ; ENDIF
         IE = M%CELL(ICMM)%EDGE_INDEX(8)
         IF (M%EDGE(IE)%W_AVG>-1.E5_EB) THEN ; AA(0,0)=M%EDGE(IE)%W_AVG ; AA(1,0)=M%EDGE(IE)%W_AVG ; ENDIF
         IE = M%CELL(ICPM)%EDGE_INDEX(4)
         IF (M%EDGE(IE)%W_AVG>-1.E5_EB) THEN ; AA(1,0)=M%EDGE(IE)%W_AVG ; AA(1,1)=M%EDGE(IE)%W_AVG ; ENDIF
         IE = M%CELL(ICMP)%EDGE_INDEX(8)
         IF (M%EDGE(IE)%W_AVG>-1.E5_EB) THEN ; AA(0,1)=M%EDGE(IE)%W_AVG ; AA(1,1)=M%EDGE(IE)%W_AVG ; ENDIF
   END SELECT
ENDIF
...
RECURSIVE SUBROUTINE UPDATE_DEVICES_1_TS(M,T,DT,NM)
...
! Shadow module-level pointer aliases with local pointers to M% members
TYPE(CELL_TYPE), POINTER, DIMENSION(:) :: CELL
INTEGER, POINTER, DIMENSION(:,:,:) :: CELL_INDEX
INTEGER, POINTER, DIMENSION(:,:,:,:,:) :: FCVAR
INTEGER, POINTER, DIMENSION(:,:,:,:) :: CCVAR
REAL(EB), POINTER, DIMENSION(:) :: DX, DY, DZ, DXN, DYN, DZN
REAL(EB), POINTER, DIMENSION(:) :: RC, XC, YC, ZC, X, Y, Z, RDX, RDY, RDZ
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: U, V, W, RHO
INTEGER :: IBAR, JBAR, KBAR, IBP1, JBP1, KBP1
INTEGER :: N_EXTERNAL_WALL_CELLS, N_INTERNAL_WALL_CELLS, NLP
INTEGER :: INTERNAL_CFACE_CELLS_LB, N_INTERNAL_CFACE_CELLS
! Shadow dump module-level pointers
...
RC => M%RC; XC => M%XC; YC => M%YC; ZC => M%ZC
X => M%X; Y => M%Y; Z => M%Z
...
END SUBROUTINE UPDATE_DEVICES_1_TS
...
USE CC_VELOCITY_KERNELS, ONLY: CC_CUTCELL_VELOCITY
...
REAL(EB) :: H_TC,TMP_TC,RE_D,NUSSELT,VEL,K_G,MU_G,COSTHETA,FAC,&
            ...
            VELSR,WATER_VOL_FRAC,RHS,DT_C,DT_E,T_RATIO,Y_E_LAG, H_G,H_G_SUM,CPBAR,CP,ZZ_GET(1:MAX_SPECIES),RCON,&
            EXPON,Y_SPECIES,MEC,Y_SPECIES2,Y_H2O,R_Y_H2O,R_DN,SGN,Y_ALL(MAX_SPECIES),H_S,D_Z_N(0:MAX_I_MAX_TEMP),&
...
IF (Z_INDEX > 0) THEN
   Y_SPECIES = M%ZZ(II,JJ,KK,Z_INDEX)
   RCON = SPECIES_MIXTURE(Z_INDEX)%RCON
ELSEIF (Y_INDEX > 0) THEN
   ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE( 1)  ! DENSITY
      GAS_PHASE_OUTPUT_RES = M%RHO(II,JJ,KK)*Y_SPECIES
   CASE( 2)  ! F_X
      GAS_PHASE_OUTPUT_RES = M%FVX(II,JJ,KK)
   CASE( 3)  ! F_Y
      GAS_PHASE_OUTPUT_RES = M%FVY(II,JJ,KK)
   CASE( 4)  ! F_Z
      GAS_PHASE_OUTPUT_RES = M%FVZ(II,JJ,KK)
   CASE( 5)  ! TEMPERATURE
      GAS_PHASE_OUTPUT_RES = M%TMP(II,JJ,KK) - TMPM
   CASE( 6)  ! U-VELOCITY
      GAS_PHASE_OUTPUT_RES = M%U(II,JJ,KK)
   CASE( 7)  ! V-VELOCITY
      GAS_PHASE_OUTPUT_RES = M%V(II,JJ,KK)
   CASE( 8)  ! W-VELOCITY
      GAS_PHASE_OUTPUT_RES = M%W(II,JJ,KK)
   CASE( 9)  ! PRESSURE
      GAS_PHASE_OUTPUT_RES = M%PBAR(KK,M%PRESSURE_ZONE(II,JJ,KK)) + &
                             M%RHO(II,JJ,KK)*(0.5_EB*(M%H(II,JJ,KK)+M%HS(II,JJ,KK))-M%KRES(II,JJ,KK)) - M%P_0(KK)
   CASE(10)  ! VELOCITY
      SELECT CASE(ABS(VELO_INDEX))
         CASE DEFAULT
            SGN = 1._EB
         CASE(1)
            SGN = SIGN(1._EB,M%U(II,JJ,KK))*SIGN(1,VELO_INDEX)
         CASE(2)
            SGN = SIGN(1._EB,M%V(II,JJ,KK))*SIGN(1,VELO_INDEX)
         CASE(3)
            SGN = SIGN(1._EB,M%W(II,JJ,KK))*SIGN(1,VELO_INDEX)
      END SELECT
      GAS_PHASE_OUTPUT_RES = SGN*SQRT(0.25_EB*((M%U(MAX(0,II-1),JJ,KK)+M%U(MIN(M%IBAR,II),JJ,KK))**2+&
                                               (M%V(II,MAX(0,JJ-1),KK)+M%V(II,MIN(M%JBAR,JJ),KK))**2+&
                                               (M%W(II,JJ,MAX(0,KK-1))+M%W(II,JJ,MIN(M%KBAR,KK)))**2))
   CASE(11)  ! HRRPUV
      GAS_PHASE_OUTPUT_RES = M%Q(II,JJ,KK)*0.001_EB
   CASE(12)  ! H
      GAS_PHASE_OUTPUT_RES = 0.5_EB*(M%HS(II,JJ,KK)+M%H(II,JJ,KK))
   CASE(13)  ! MIXTURE FRACTION
      ...
      DO NR=1,N_REACTIONS
         IF (REACTION(NR)%SIMPLE_CHEMISTRY .AND. REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS > 0) THEN
            ! Unburned fuel
            GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%ZZ(II,JJ,KK,REACTION(NR)%FUEL_SMIX_INDEX)
            IF (REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS == 1) THEN
               ! Single step products
                GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%ZZ(II,JJ,KK,REACTION(NR)%PROD_SMIX_INDEX)/(1._EB+REACTION(NR)%S)
            ELSE
               ! Two step first intermediate products
                GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%ZZ(II,JJ,KK,REACTION(NR)%PROD_SMIX_INDEX)/(1._EB+REACTION(NR)%S)
                ! Two step second products
                GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%ZZ(II,JJ,KK,REACTION(REACTION(NR)%PAIR_INDEX)%PROD_SMIX_INDEX)/ &
                   ((1._EB+REACTION(NR)%S)*(1._EB+REACTION(REACTION(NR)%PAIR_INDEX)%S))
            ENDIF
         ENDIF
         IF (.NOT. REACTION(NR)%SIMPLE_CHEMISTRY) &
            GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%ZZ(II,JJ,KK,REACTION(NR)%FUEL_SMIX_INDEX) + &
                                   M%ZZ(II,JJ,KK,REACTION(NR)%PROD_SMIX_INDEX)/(1._EB+REACTION(NR)%S)
      ENDDO
   CASE(14)  ! DIVERGENCE
      GAS_PHASE_OUTPUT_RES = M%D(II,JJ,KK)
   CASE(15)  ! MIXING TIME
      GAS_PHASE_OUTPUT_RES = M%MIX_TIME(II,JJ,KK)
   CASE(16)  ! ABSORPTION COEFFICIENT
      III = MAX(1,MIN(II,M%IBAR))
      JJJ = MAX(1,MIN(JJ,M%JBAR))
      KKK = MAX(1,MIN(KK,M%KBAR))
      GAS_PHASE_OUTPUT_RES = M%KAPPA_GAS(III,JJJ,KKK)
   CASE(17)  ! VISCOSITY
      GAS_PHASE_OUTPUT_RES = M%MU(II,JJ,KK)
   CASE(18)  ! INTEGRATED INTENSITY
      GAS_PHASE_OUTPUT_RES = M%UII(II,JJ,KK)*0.001_EB
   CASE(19)  ! RADIATION LOSS
      GAS_PHASE_OUTPUT_RES = M%QR(II,JJ,KK)*0.001_EB
   CASE(20)  ! PARTICLE RADIATION LOSS
      IF (N_LP_ARRAY_INDICES>0) THEN
         GAS_PHASE_OUTPUT_RES = M%QR_W(II,JJ,KK)*0.001_EB
      ELSE
         GAS_PHASE_OUTPUT_RES = 0._EB
      ENDIF
   CASE(21)  ! RELATIVE HUMIDITY
      ...
      ELSE
         ...
         ENDIF
         GAS_PHASE_OUTPUT_RES = RELATIVE_HUMIDITY(Y_H2O,M%TMP(II,JJ,KK),M%PBAR(KK,M%PRESSURE_ZONE(II,JJ,KK)))
      ENDIF
   CASE(22)  ! HS
      GAS_PHASE_OUTPUT_RES = M%HS(II,JJ,KK)
   CASE(23)  ! KINETIC ENERGY (per unit mass) -- do not average because this operation is dissipative
      UU   = M%U(MIN(M%IBAR,II),JJ,KK)
      VV   = M%V(II,MIN(M%JBAR,JJ),KK)
      WW   = M%W(II,JJ,MIN(M%KBAR,KK))
      GAS_PHASE_OUTPUT_RES  = 0.5_EB*( UU**2 + VV**2 + WW**2 )

   CASE(24)  ! STRAIN RATE X
      III = MAX(1,MIN(II,M%IBAR))
      GAS_PHASE_OUTPUT_RES = (M%W(III,JJ+1,KK)-M%W(III,JJ,KK))*M%RDYN(JJ) + (M%V(III,JJ,KK+1)-M%V(III,JJ,KK))*M%RDZN(KK)
   CASE(25)  ! STRAIN RATE Y
      JJJ = MAX(1,MIN(JJ,M%JBAR))
      GAS_PHASE_OUTPUT_RES = (M%U(II,JJJ,KK+1)-M%U(II,JJJ,KK))*M%RDZN(KK) + (M%W(II+1,JJJ,KK)-M%W(II,JJJ,KK))*M%RDXN(II)
   CASE(26)  ! STRAIN RATE Z
      KKK = MAX(1,MIN(KK,M%KBAR))
      GAS_PHASE_OUTPUT_RES = (M%V(II+1,JJ,KKK)-M%V(II,JJ,KKK))*M%RDXN(II) + (M%U(II,JJ+1,KKK)-M%U(II,JJ,KKK))*M%RDYN(JJ)
   CASE(27)  ! VORTICITY X
      III = MAX(1,MIN(II,M%IBAR))
      GAS_PHASE_OUTPUT_RES = (M%W(III,JJ+1,KK)-M%W(III,JJ,KK))*M%RDYN(JJ) - (M%V(III,JJ,KK+1)-M%V(III,JJ,KK))*M%RDZN(KK)
   CASE(28)  ! VORTICITY Y
   ...
   CASE(30)  ! C_SMAG
      ...
      SELECT CASE (TURB_MODEL)
         CASE (CONSMAG,DYNSMAG)
            ...
            JJJ = MAX(1,MIN(JJ,M%JBAR))
            KKK = MAX(1,MIN(KK,M%KBAR))
            DELTA = M%LES_FILTER_WIDTH(III,JJJ,KKK)
            GAS_PHASE_OUTPUT_RES = SQRT(M%CSD2(III,JJJ,KKK))/DELTA
      END SELECT
   CASE(31)  ! SPECIFIC HEAT
      ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_SPECIFIC_HEAT(ZZ_GET,CP,M%TMP(II,JJ,KK))
      GAS_PHASE_OUTPUT_RES = CP*0.001_EB

   CASE(32)  ! ORIENTED VELOCITY
      GAS_PHASE_OUTPUT_RES = M%U(II,JJ,KK)*ORIENTATION_VECTOR(1,DV%ORIENTATION_INDEX) + &
                             M%V(II,JJ,KK)*ORIENTATION_VECTOR(2,DV%ORIENTATION_INDEX) + &
                             M%W(II,JJ,KK)*ORIENTATION_VECTOR(3,DV%ORIENTATION_INDEX)

   CASE(33,50)  ! CONDUCTIVITY, MOLECULAR CONDUCTIVITY
      IF (SIM_MODE==DNS_MODE .OR. IND==50) THEN
         ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
         CALL GET_CONDUCTIVITY(ZZ_GET,GAS_PHASE_OUTPUT_RES,M%TMP(II,JJ,KK))
      ELSE
         GAS_PHASE_OUTPUT_RES = M%MU(II,JJ,KK)*CPOPR
      ENDIF
   ...
   CASE(36)  ! POTENTIAL TEMPERATURE
      GAS_PHASE_OUTPUT_RES = GET_POTENTIAL_TEMPERATURE(M%TMP(II,JJ,KK),M%ZC(KK))

   CASE(37)  ! DIFFUSIVITY
      SELECT CASE (SIM_MODE)
         CASE DEFAULT
            GAS_PHASE_OUTPUT_RES = M%MU(II,JJ,KK)*RSC_T/M%RHO(II,JJ,KK)
         CASE (DNS_MODE)
            D_Z_N = D_Z(:,Z_INDEX)
            CALL INTERPOLATE1D_UNIFORM(LBOUND(D_Z_N,1),D_Z_N,M%TMP(II,JJ,KK),GAS_PHASE_OUTPUT_RES)
      END SELECT
   ...
   CASE(46:47)  ! SPECIFIC ENTHALPY and ENTHALPY
      ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_ENTHALPY(ZZ_GET,H_G,M%TMP(II,JJ,KK))
      IF (IND==46) GAS_PHASE_OUTPUT_RES = H_G*0.001_EB
      IF (IND==47) GAS_PHASE_OUTPUT_RES = M%RHO(II,JJ,KK)*H_G*0.001_EB

   CASE(48:49)  ! SPECIFIC SENSIBLE ENTHALPY and SENSIBLE ENTHALPY
      ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,M%TMP(II,JJ,KK))
      IF (IND==48) GAS_PHASE_OUTPUT_RES = H_S*0.001_EB
      IF (IND==49) GAS_PHASE_OUTPUT_RES = M%RHO(II,JJ,KK)*H_S*0.001_EB

   CASE(51)  ! RESOLVED KINETIC ENERGY (per unit mass)
      GAS_PHASE_OUTPUT_RES = M%KRES(II,JJ,KK)
   ...
   CASE(53)  ! CELL U
      III = MAX(1,MIN(II,M%IBAR))
      GAS_PHASE_OUTPUT_RES = 0.5_EB*(M%U(III,JJ,KK)+M%U(MAX(1,III-1),JJ,KK))
   CASE(54)  ! CELL V
      JJJ = MAX(1,MIN(JJ,M%JBAR))
      GAS_PHASE_OUTPUT_RES = 0.5_EB*(M%V(II,JJJ,KK)+M%V(II,MAX(1,JJJ-1),KK))
   CASE(55)  ! CELL W
      KKK = MAX(1,MIN(KK,M%KBAR))
      GAS_PHASE_OUTPUT_RES = 0.5_EB*(M%W(II,JJ,KKK)+M%W(II,JJ,MAX(1,KKK-1)))

   CASE(56)  ! SUBGRID KINETIC ENERGY (per unit mass)
      DELTA = M%LES_FILTER_WIDTH(II,JJ,KK)
      GAS_PHASE_OUTPUT_RES = SUBGRID_KINETIC_ENERGY(M%MU(II,JJ,KK)-M%MU_DNS(II,JJ,KK),M%RHO(II,JJ,KK),C_DEARDORFF,DELTA)
   ...
   CASE(61)  ! DRAG FORCE X
      GAS_PHASE_OUTPUT_RES = -0.5_EB*(M%RHO(II,JJ,KK)+M%RHO(II+1,JJ,KK))*M%FVX_D(II,JJ,KK)
   CASE(62)  ! DRAG FORCE Y
      GAS_PHASE_OUTPUT_RES = -0.5_EB*(M%RHO(II,JJ,KK)+M%RHO(II,JJ+1,KK))*M%FVY_D(II,JJ,KK)
   CASE(63)  ! DRAG FORCE Z
      GAS_PHASE_OUTPUT_RES = -0.5_EB*(M%RHO(II,JJ,KK)+M%RHO(II,JJ,KK+1))*M%FVZ_D(II,JJ,KK)

   CASE(64)  ! EFFECTIVE FLAME TEMPERATURE
      III = MAX(1,MIN(II,M%IBAR))
      JJJ = MAX(1,MIN(JJ,M%JBAR))
      KKK = MAX(1,MIN(KK,M%KBAR))
      IF (M%CHI_R(III,JJJ,KKK)*M%Q(II,JJ,KK)>QR_CLIP) THEN
         GAS_PHASE_OUTPUT_RES = M%TMP(II,JJ,KK)*RTE_SOURCE_CORRECTION_FACTOR**0.25_EB - TMPM
      ELSE
         GAS_PHASE_OUTPUT_RES = M%TMP(II,JJ,KK) - TMPM
      ENDIF
   ...
   CASE(68:69)  ! SPECIFIC INTERNAL ENERGY and INTERNAL ENERGY (per unit volume)
      ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_ENTHALPY(ZZ_GET,H_G,M%TMP(II,JJ,KK))
      IF (IND==68) GAS_PHASE_OUTPUT_RES = ( H_G - M%PBAR(KK,M%PRESSURE_ZONE(II,JJ,KK))/M%RHO(II,JJ,KK) )*0.001_EB
      IF (IND==69) GAS_PHASE_OUTPUT_RES = ( M%RHO(II,JJ,KK)*H_G - M%PBAR(KK,M%PRESSURE_ZONE(II,JJ,KK)) )*0.001_EB

   CASE(70)  ! CFL
      IF (M%CELL(M%CELL_INDEX(II,JJ,KK))%SOLID) THEN
         GAS_PHASE_OUTPUT_RES = 0._EB
      ELSE
         IIM1 = MAX(II-1,0)
         JJM1 = MAX(JJ-1,0)
         KKM1 = MAX(KK-1,0)
         UODX = MAXVAL(ABS(M%US(IIM1:II,JJ,KK)))*M%RDX(II)
         VODY = MAXVAL(ABS(M%VS(II,JJM1:JJ,KK)))*M%RDY(JJ)
         WODZ = MAXVAL(ABS(M%WS(II,JJ,KKM1:KK)))*M%RDZ(KK)
         SELECT CASE (CFL_VELOCITY_NORM)
            CASE(0) ; UVW = MAX(UODX,VODY,WODZ) + ABS(M%DS(II,JJ,KK))
            CASE(1) ; UVW = UODX + VODY + WODZ  + ABS(M%DS(II,JJ,KK))
            CASE(2) ; UVW = SQRT(UODX**2+VODY**2+WODZ**2) + ABS(M%DS(II,JJ,KK))
   ...
   CASE(71)  ! VN
      IF (TWO_D) THEN
         R_DX2 = M%RDX(II)**2 + M%RDZ(KK)**2
      ELSE
         R_DX2 = M%RDX(II)**2 + M%RDY(JJ)**2 + M%RDZ(KK)**2
      ENDIF
      GAS_PHASE_OUTPUT_RES = DT*2._EB*R_DX2*MAX(M%D_Z_MAX(II,JJ,KK),MAX(RPR_T,RSC_T)*M%MU(II,JJ,KK)/M%RHO(II,JJ,KK))

   CASE(72)  ! CFL MAX
      GAS_PHASE_OUTPUT_RES = M%CFL
   CASE(73)  ! VN MAX
      GAS_PHASE_OUTPUT_RES = M%VN
   CASE(74)  ! POISSON ERROR
      GAS_PHASE_OUTPUT_RES = M%POIS_ERR
   CASE(75)  ! DIVERGENCE ERROR
      GAS_PHASE_OUTPUT_RES = M%RESMAX
   CASE(76)  ! RADIAL VELOCITY
      GAS_PHASE_OUTPUT_RES = ( M%XC(II)*0.5_EB*(M%U(II,JJ,KK)+M%U(II-1,JJ,KK)) + &
                               M%YC(JJ)*0.5_EB*(M%V(II,JJ,KK)+M%V(II,JJ-1,KK)) )/ &
                             SQRT(M%XC(II)**2+M%YC(JJ)**2)

   CASE(77)  ! LEVEL SET VALUE
      GAS_PHASE_OUTPUT_RES = M%PHI_LS(II,JJ)
   CASE(78)  ! RADIATION EMISSION
      GAS_PHASE_OUTPUT_RES = M%RADIATION_EMISSION(II,JJ,KK)*0.001_EB
   CASE(79)  ! RADIATION ABSORPTION
      GAS_PHASE_OUTPUT_RES = M%RADIATION_ABSORPTION(II,JJ,KK)*0.001_EB
   ...
   CASE(83)  ! Q CRITERION : Q = 1/2 (tr(Dij)^2 - tr(Dij^2))
      ...
      IF (II == 0   ) III = II+1
      IF (II == M%IBP1) III = II-1
      IF (JJ == 0   ) JJJ = JJ+1
      IF (JJ == M%JBP1) JJJ = JJ-1
      IF (KK == 0   ) KKK = KK+1
      IF (KK == M%KBP1) KKK = KK-1
      IM1 = III-1
      JM1 = JJJ-1
      KM1 = KKK-1
      IIM1 = MAX(1,III-1)
      ...
      JP1 = JJJ+1
      KP1 = KKK+1
      DUDX = M%RDX(III)*(M%U(III,JJJ,KKK)-M%U(IM1,JJJ,KKK))
      DUDY = 0.25_EB*M%RDY(JJJ)*(M%U(III,JP1,KKK)-M%U(III,JJM1,KKK)+M%U(IM1,JP1,KKK)-M%U(IM1,JJM1,KKK))
      DUDZ = 0.25_EB*M%RDZ(KKK)*(M%U(III,JJJ,KP1)-M%U(III,JJJ,KKM1)+M%U(IM1,JJJ,KP1)-M%U(IM1,JJJ,KKM1))
      DVDX = 0.25_EB*M%RDX(III)*(M%V(IP1,JJJ,KKK)-M%V(IIM1,JJJ,KKK)+M%V(IP1,JM1,KKK)-M%V(IIM1,JM1,KKK))
      DVDY = M%RDY(JJJ)*(M%V(III,JJJ,KKK)-M%V(III,JM1,KKK))
      DVDZ = 0.25_EB*M%RDZ(KKK)*(M%V(III,JJJ,KP1)-M%V(III,JJJ,KKM1)+M%V(III,JM1,KP1)-M%V(III,JM1,KKM1))
      DWDX = 0.25_EB*M%RDX(III)*(M%W(IP1,JJJ,KKK)-M%W(IIM1,JJJ,KKK)+M%W(IP1,JJJ,KM1)-M%W(IIM1,JJJ,KM1))
      DWDY = 0.25_EB*M%RDY(JJJ)*(M%W(III,JP1,KKK)-M%W(III,JJM1,KKK)+M%W(III,JP1,KM1)-M%W(III,JJM1,KM1))
      DWDZ = M%RDZ(KKK)*(M%W(III,JJJ,KKK)-M%W(III,JJJ,KM1))
   ...
   CASE(84)  ! STRAIN RATE
      ...
      JP1 = MIN(M%JBAR,JJ+1)
      KP1 = MIN(M%KBAR,KK+1)
      DUDX = M%RDX(II)*(M%U(II,JJ,KK)-M%U(IM1,JJ,KK))
      DVDY = M%RDY(JJ)*(M%V(II,JJ,KK)-M%V(II,JM1,KK))
      DWDZ = M%RDZ(KK)*(M%W(II,JJ,KK)-M%W(II,JJ,KM1))
      ONTHDIV = ONTH*(DUDX+DVDY+DWDZ)
      S11 = DUDX - ONTHDIV
      S22 = DVDY - ONTHDIV
      S33 = DWDZ - ONTHDIV
      DUDY = 0.25_EB*M%RDY(JJ)*(M%U(II,JP1,KK)-M%U(II,JJM1,KK)+M%U(IM1,JP1,KK)-M%U(IM1,JJM1,KK))
      DUDZ = 0.25_EB*M%RDZ(KK)*(M%U(II,JJ,KP1)-M%U(II,JJ,KKM1)+M%U(IM1,JJ,KP1)-M%U(IM1,JJ,KKM1))
      DVDX = 0.25_EB*M%RDX(II)*(M%V(IP1,JJ,KK)-M%V(IIM1,JJ,KK)+M%V(IP1,JM1,KK)-M%V(IIM1,JM1,KK))
      DVDZ = 0.25_EB*M%RDZ(KK)*(M%V(II,JJ,KP1)-M%V(II,JJ,KKM1)+M%V(II,JM1,KP1)-M%V(II,JM1,KKM1))
      DWDX = 0.25_EB*M%RDX(II)*(M%W(IP1,JJ,KK)-M%W(IIM1,JJ,KK)+M%W(IP1,JJ,KM1)-M%W(IIM1,JJ,KM1))
      DWDY = 0.25_EB*M%RDY(JJ)*(M%W(II,JP1,KK)-M%W(II,JJM1,KK)+M%W(II,JP1,KM1)-M%W(II,JJM1,KM1))
   ...
   CASE(85)  ! KOLMOGOROV LENGTH SCALE
      ...
      DISSIPATION_RATE = M%MU(II,JJ,KK)/M%RHO(II,JJ,KK)*SS**2
      GAS_PHASE_OUTPUT_RES = ((M%MU_DNS(II,JJ,KK)/M%RHO(II,JJ,KK))**3/(DISSIPATION_RATE+EPS))**0.25_EB
   ...
   CASE(87)  ! MOLECULAR VISCOSITY
      GAS_PHASE_OUTPUT_RES = M%MU_DNS(II,JJ,KK)
   CASE(88)  ! DISSIPATION RATE
      SS = GAS_PHASE_OUTPUT(T,DT,NM,II,JJ,KK,84,IND2,Y_INDEX,Z_INDEX,ELEM_INDX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,&
                            REAC_INDEX,MATL_INDEX)
      GAS_PHASE_OUTPUT_RES = M%MU(II,JJ,KK)/M%RHO(II,JJ,KK)*SS**2
   CASE(89)  ! KINEMATIC VISCOSITY
      GAS_PHASE_OUTPUT_RES = M%MU(II,JJ,KK)/M%RHO(II,JJ,KK)
   ...
   CASE(91:93) ! MASS FLUX
      IP=II ; JP=JJ ; KP=KK
      SELECT CASE(IND)
         CASE(91) ; IP=II+1 ; VEL=M%U(II,JJ,KK)  ! MASS FLUX X
         CASE(92) ; JP=JJ+1 ; VEL=M%V(II,JJ,KK)  ! MASS FLUX Y
         CASE(93) ; KP=KK+1 ; VEL=M%W(II,JJ,KK)  ! MASS FLUX Z
      END SELECT
      ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(IP,JP,KP,1:N_TRACKED_SPECIES)
      Y_SPECIES2 = 1.0_EB
      IF (Z_INDEX > 0) THEN
      ...
      ENDIF
      GAS_PHASE_OUTPUT_RES = 0.5_EB*(M%RHO(II,JJ,KK)*Y_SPECIES+M%RHO(IP,JP,KP)*Y_SPECIES2)*VEL

   CASE(94)  ! VOLUME FRACTION
      GAS_PHASE_OUTPUT_RES =  RCON*Y_SPECIES/M%RSUM(II,JJ,KK)/(1._EB-R_Y_H2O/M%RSUM(II,JJ,KK))
   CASE(95)  ! VISIBILITY
      IF (Z_INDEX>0) THEN
      ...
      ENDIF
      EXT_COEF = Y_SPECIES*M%RHO(II,JJ,KK)*MEC
      GAS_PHASE_OUTPUT_RES = VISIBILITY_FACTOR/MAX(EC_LL,EXT_COEF)
   CASE(96)  ! AEROSOL VOLUME FRACTION
      IF (Z_INDEX >0) THEN
         GAS_PHASE_OUTPUT_RES = Y_SPECIES*M%RHO(II,JJ,KK)/SPECIES(SPECIES_MIXTURE(Z_INDEX)%SINGLE_SPEC_INDEX)%DENSITY_SOLID
      ELSEIF (Y_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = Y_SPECIES*M%RHO(II,JJ,KK)/SPECIES(Y_INDEX)%DENSITY_SOLID
      ENDIF
   CASE(97)  ! EXTINCTION COEFFICIENT
      ...
      ENDIF
      EXT_COEF = Y_SPECIES*M%RHO(II,JJ,KK)*MEC
      GAS_PHASE_OUTPUT_RES = Y_SPECIES*M%RHO(II,JJ,KK)*MEC
   CASE(98)  ! OPTICAL DENSITY
      ...
      ELSEIF (Y_INDEX>0) THEN
         MEC = SPECIES(Y_INDEX)%MASS_EXTINCTION_COEFFICIENT
      ENDIF
      GAS_PHASE_OUTPUT_RES = Y_SPECIES*M%RHO(II,JJ,KK)*MEC/2.3_EB

   CASE(99)  ! PRESSURE POISSON RESIDUAL
      GAS_PHASE_OUTPUT_RES = M%PP_RESIDUAL(II,JJ,KK)
   CASE(100) ! PRESSURE ZONE
      GAS_PHASE_OUTPUT_RES = M%PRESSURE_ZONE(II,JJ,KK)

   CASE(101)  ! FIC
      ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      GAS_PHASE_OUTPUT_RES = FIC(ZZ_GET,M%RSUM(II,JJ,KK))

   CASE(102)  ! BULK DENSITY
      IC = M%CELL_INDEX(II,JJ,KK)
      IF (.NOT.M%CELL(IC)%SOLID .OR. M%CELL(IC)%OBST_INDEX<1) THEN
         GAS_PHASE_OUTPUT_RES = 0._EB
      ELSE
         GAS_PHASE_OUTPUT_RES = M%OBSTRUCTION(M%CELL(IC)%OBST_INDEX)%MASS*M%RDX(II)*M%RRN(II)*M%RDY(JJ)*M%RDZ(KK)
      ENDIF
   ...
   CASE(109)  ! FED
      ...
      ELSE
         FED_ACTIVITY = 2
      ENDIF
      GAS_PHASE_OUTPUT_RES = FED(ZZ_GET,M%RSUM(II,JJ,KK),FED_ACTIVITY)

   CASE(110)  ! THERMOCOUPLE
      IF (T > T_BEGIN) THEN
         TMP_G = M%TMP(II,JJ,KK)
         IF (PY%HEAT_TRANSFER_COEFFICIENT<0._EB) THEN
            ...
            VEL2    = UU**2+VV**2+WW**2
            ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
            CALL GET_VISCOSITY(ZZ_GET,MU_G,M%TMP(II,JJ,KK))
            CALL GET_CONDUCTIVITY(ZZ_GET,K_G,M%TMP(II,JJ,KK))
            RE_D    = M%RHO(II,JJ,KK)*SQRT(VEL2)*PY%DIAMETER/MU_G
         ...
         DV%TMP_L = (DV%TMP_L + FAC*(H_TC*(TMP_G-0.5_EB*DV%TMP_L) + &
                     PY%EMISSIVITY*(0.25_EB*M%UII(II,JJ,KK)+SIGMA*DV%TMP_L**4))) / &
   ...
   CASE(111:113)  ! ENTHALPY FLUX
      IP=II ; JP=JJ ; KP=KK
      SELECT CASE(IND)
         CASE(111) ; IP=II+1 ; VEL=M%U(II,JJ,KK) ; R_DN=M%RDXN(II)  ! ENTHALPY FLUX X
         CASE(112) ; JP=JJ+1 ; VEL=M%V(II,JJ,KK) ; R_DN=M%RDYN(JJ)  ! ENTHALPY FLUX Y
         CASE(113) ; KP=KK+1 ; VEL=M%W(II,JJ,KK) ; R_DN=M%RDZN(KK)  ! ENTHALPY FLUX Z
      END SELECT
      TMP_TC = 0.5_EB*(M%TMP(II,JJ,KK)+M%TMP(IP,JP,KP))
      ZZ_GET(1:N_TRACKED_SPECIES) = 0.5_EB*(M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)+M%ZZ(IP,JP,KP,1:N_TRACKED_SPECIES))
      CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_G_SUM,TMP_TC)
      CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_G,TMPA)
      GAS_PHASE_OUTPUT_RES = VEL*0.5_EB*(M%RHO(II,JJ,KK)+M%RHO(IP,JP,KP))*(H_G_SUM-H_G)
      IF (SIM_MODE==DNS_MODE) THEN
         CALL GET_CONDUCTIVITY(ZZ_GET,K_G,M%TMP(II,JJ,KK))
      ELSE
         K_G = M%MU(II,JJ,KK)*CPOPR
      ENDIF
      GAS_PHASE_OUTPUT_RES = (GAS_PHASE_OUTPUT_RES - K_G*(M%TMP(IP,JP,KP)-M%TMP(II,JJ,KK))*R_DN)*0.001
   ...
   CASE(114) ! BI-DIRECTIONAL PROBE
      ...
      ELSE
         PROBE_TMP = M%TMP(II,JJ,KK)
      ENDIF
      PROBE_RHO = MW_AIR*P_STP/(R0*PROBE_TMP)
      UU = 0.5_EB*(M%U(MAX(0,II-1),JJ,KK)+M%U(MIN(M%IBAR,II),JJ,KK))
      VV = 0.5_EB*(M%V(II,MAX(0,JJ-1),KK)+M%V(II,MIN(M%JBAR,JJ),KK))
      WW = 0.5_EB*(M%W(II,JJ,MAX(0,KK-1))+M%W(II,JJ,MIN(M%KBAR,KK)))
      VEL2 = UU**2+VV**2+WW**2
      VEL = SQRT(VEL2)
      ...
      ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_VISCOSITY(ZZ_GET,MU_G,M%TMP(II,JJ,KK))
      RE_D = MIN(3800._EB,MAX(40._EB,M%RHO(II,JJ,KK)*VEL*PY%PROBE_DIAMETER/MU_G))
      ...
      IF (PY%CALIBRATION_CONSTANT > 0._EB) THEN
         GAS_PHASE_OUTPUT_RES = SIGN(1._EB,COSTHETA)*VEL*PY%CALIBRATION_CONSTANT*FAC*SQRT(M%RHO(II,JJ,KK)/PROBE_RHO)
      ELSE
         PROBE_DELTA_P = (VEL*FAC)**2*M%RHO(II,JJ,KK)*0.5_EB
   ...
   CASE(130) ! EXTINCTION
      ...
      DO NR=1,N_REACTIONS
         ...
         ENDDO
      ENDDO
      IF (ZZ_FUEL<TWENTY_EPSILON_EB .OR. ZZ_OX<TWENTY_EPSILON_EB .OR. M%Q(II,JJ,KK)<TWENTY_EPSILON_EB) GAS_PHASE_OUTPUT_RES = -1._EB
      IF (ZZ_FUEL>ZZ_MIN_GLOBAL .AND. ZZ_OX > ZZ_MIN_GLOBAL .AND. M%Q(II,JJ,KK) < TWENTY_EPSILON_EB) GAS_PHASE_OUTPUT_RES = 1._EB

   CASE(131) ! CHEMISTRY SUBITERATIONS
      GAS_PHASE_OUTPUT_RES = M%CHEM_SUBIT(II,JJ,KK)

   CASE(132) ! REAC SOURCE TERM
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = M%REAC_SOURCE_TERM(II,JJ,KK,Z_INDEX)
      ELSEIF (Y_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = DOT_PRODUCT(Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),M%REAC_SOURCE_TERM(II,JJ,KK,1:N_TRACKED_SPECIES))
      ENDIF

   CASE(133) ! SUM LUMPED MASS FRACTIONS
      GAS_PHASE_OUTPUT_RES = SUM(M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES))
   ...
   CASE(135) ! MACH NUMBER
      ...
      GAS_PHASE_OUTPUT_RES = SQRT(2._EB*M%KRES(II,JJ,KK))/SQRT(RCON*M%TMP(II,JJ,KK)*GAMMA_LOC)

   CASE(136) ! UNMIXED FRACTION
      GAS_PHASE_OUTPUT_RES = INITIAL_UNMIXED_FRACTION*EXP(-DT/M%MIX_TIME(II,JJ,KK))

   CASE(138) ! HRRPUV REAC
      GAS_PHASE_OUTPUT_RES = M%Q_REAC(II,JJ,KK,REAC_INDEX)*0.001_EB

   CASE(140) ! FVX_B
      GAS_PHASE_OUTPUT_RES = M%FVX_B(II,JJ,KK)
   CASE(141) ! FVY_B
      GAS_PHASE_OUTPUT_RES = M%FVY_B(II,JJ,KK)
   CASE(142) ! FVZ_B
      GAS_PHASE_OUTPUT_RES = M%FVZ_B(II,JJ,KK)

   CASE(143) ! COMBUSTION EFFICIENCY
      IF (M%Q(II,JJ,KK)>TWENTY_EPSILON_EB) THEN
         GAS_PHASE_OUTPUT_RES = MIN(DT/M%MIX_TIME(II,JJ,KK),1._EB)
      ELSE
         GAS_PHASE_OUTPUT_RES = 0._EB
      ENDIF
   ...
   CASE(150) ! SUM LUMPED VOLUME FRACTIONS
      ...
      GAS_PHASE_OUTPUT_RES = 0._EB
      DO N=1,N_TRACKED_SPECIES
         GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%ZZ(II,JJ,KK,N)/SPECIES_MIXTURE(N)%MW*MW
      ENDDO
   ...
   CASE(154:155) ! TRANSMISSION, PATH OBSCURATION
      ...
      DO NN=1,SDV%N_PATH
         ...
         ELSEIF (PY%Z_INDEX>0) THEN
            Y_MF_INT = M%ZZ(I,J,K,PY%Z_INDEX)
         ELSE
            ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(I,J,K,1:N_TRACKED_SPECIES)
            CALL GET_MASS_FRACTION(ZZ_GET,SOOT_INDEX,Y_MF_INT)
         ENDIF
         EXT_COEF = EXT_COEF + Y_MF_INT*M%RHO(I,J,K)*SDV%D_PATH(NN)
      ENDDO
   ...
   CASE(156) ! SPRINKLER LINK TEMPERATURE
      ...
      K = DV%K(1)
      TMP_G = M%TMP(I,J,K)
      VEL2  = 0.25_EB*( (M%U(I,J,K)+M%U(I-1,J,K))**2 +(M%V(I,J,K)+M%V(I,J-1,K))**2 + (M%W(I,J,K)+M%W(I,J,K-1))**2 )
      VEL   = SQRT(VEL2)
      VELSR = SQRT(VEL)
      ...
      IF (H2O_INDEX > 0) THEN
         DO NN = 1,N_LAGRANGIAN_CLASSES
            IF (LAGRANGIAN_PARTICLE_CLASS(NN)%Y_INDEX==H2O_INDEX) WATER_VOL_FRAC = WATER_VOL_FRAC + &
               M%AVG_DROP_DEN(I,J,K,LAGRANGIAN_PARTICLE_CLASS(NN)%ARRAY_INDEX)/LAGRANGIAN_PARTICLE_CLASS(NN)%DENSITY
         ENDDO
      ENDIF
   ...
   CASE(158) ! CHAMBER OBSCURATION
      ...
      GAS_PHASE_OUTPUT_RES = (1._EB-EXP(-MASS_EXT_COEF*M%RHO(I,J,K)*DV%Y_C))*100._EB  ! Obscuration
   ...
   CASE(164) ! FIRE DEPTH
      ...
      DO NN=1,SDV%N_PATH
         ...
         J = SDV%J_PATH(NN)
         K = SDV%K_PATH(NN)
         IF (M%Q(I,J,K)>(1.E3_EB*DV%SETPOINT)) THEN
   ...
   CASE(170) ! MPUV
      ...
      GAS_PHASE_OUTPUT_RES = M%AVG_DROP_DEN(II,JJ,KK,LPC%ARRAY_INDEX)

   CASE(171) ! ADD
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      GAS_PHASE_OUTPUT_RES = M%AVG_DROP_RAD(II,JJ,KK,LPC%ARRAY_INDEX)*2.E6_EB

   CASE(172) ! ADT
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      GAS_PHASE_OUTPUT_RES = M%AVG_DROP_TMP(II,JJ,KK,LPC%ARRAY_INDEX) - TMPM

   CASE(173) ! ADA
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      GAS_PHASE_OUTPUT_RES = M%AVG_DROP_AREA(II,JJ,KK,LPC%ARRAY_INDEX)

   CASE(174) ! QABS
      GAS_PHASE_OUTPUT_RES = 0._EB
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      IF (ABS(M%AVG_DROP_AREA(II,JJ,KK,LPC%ARRAY_INDEX))>TWENTY_EPSILON_EB) THEN
         DO N = 1,NUMBER_SPECTRAL_BANDS
            ...
            ENDIF
            CALL INTERPOLATE1D(LPC%R50,LPC%WQABS(:,N),M%AVG_DROP_RAD(II,JJ,KK,LPC%ARRAY_INDEX),Q_SUM)
      ...
      IF (ABS(M%AVG_DROP_AREA(II,JJ,KK,LPC%ARRAY_INDEX))>TWENTY_EPSILON_EB) THEN
         DO N = 1,NUMBER_SPECTRAL_BANDS
            ...
            ELSE
               BBF = BLACKBODY_FRACTION(WL_LOW(N),WL_HIGH(N),RADTMP)
            ENDIF
            CALL INTERPOLATE1D(LPC%R50,LPC%WQSCA(:,N),M%AVG_DROP_RAD(II,JJ,KK,LPC%ARRAY_INDEX),Q_SUM)
            GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + BBF*Q_SUM
         ENDDO
         GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES/REAL(NUMBER_SPECTRAL_BANDS,EB)
      ENDIF
   ...
   CASE(176) ! PARTICLE FLUX X
      GAS_PHASE_OUTPUT_RES = M%WORK4(II,JJ,KK)

   CASE(177) ! PARTICLE FLUX Y
      GAS_PHASE_OUTPUT_RES = M%WORK5(II,JJ,KK)

   CASE(178) ! PARTICLE FLUX Z
      GAS_PHASE_OUTPUT_RES = M%WORK6(II,JJ,KK)
   ...
   CASE(179) ! MPUV_Z
      ...
      DO NN = 1,N_LAGRANGIAN_CLASSES
         ...
         IF (LPC%LIQUID_DROPLET .AND. LPC%Y_INDEX==Y_INDEX) &
            GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%AVG_DROP_DEN(II,JJ,KK,LPC%ARRAY_INDEX)
      ENDDO
   ...
   CASE(180) ! ADD_Z
      ...
      DO NN = 1,N_LAGRANGIAN_CLASSES
         LPC => LAGRANGIAN_PARTICLE_CLASS(NN)
         IF (LPC%LIQUID_DROPLET .AND. LPC%Y_INDEX==Y_INDEX) &
            GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%AVG_DROP_RAD(II,JJ,KK,LAGRANGIAN_PARTICLE_CLASS(NN)%ARRAY_INDEX)
      ENDDO
   ...
   CASE(181) ! ADT_Z
      ...
      DO NN = 1,N_LAGRANGIAN_CLASSES
         LPC => LAGRANGIAN_PARTICLE_CLASS(NN)
         IF (LPC%LIQUID_DROPLET .AND. LPC%Y_INDEX==Y_INDEX) &
            GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES +  M%AVG_DROP_TMP(II,JJ,KK,LAGRANGIAN_PARTICLE_CLASS(NN)%ARRAY_INDEX)-TMPM
      ENDDO
   ...
   CASE(182) ! ADA_Z
      ...
      DO NN = 1,N_LAGRANGIAN_CLASSES
         LPC => LAGRANGIAN_PARTICLE_CLASS(NN)
         IF (LPC%LIQUID_DROPLET .AND. LPC%Y_INDEX==Y_INDEX) &
            GAS_PHASE_OUTPUT_RES = GAS_PHASE_OUTPUT_RES + M%AVG_DROP_AREA(II,JJ,KK,LAGRANGIAN_PARTICLE_CLASS(NN)%ARRAY_INDEX)
      ENDDO
   ...
   CASE(185) ! NUMBER OF PARTICLES
      GAS_PHASE_OUTPUT_RES = M%NLP

   CASE(186) ! DROPLET VOLUME FRACTION
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      GAS_PHASE_OUTPUT_RES = MIN(1._EB,M%AVG_DROP_DEN(II,JJ,KK,LPC%ARRAY_INDEX)/LPC%DENSITY)

   CASE(190) ! CELL PHASE
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (M%CELL(M%CELL_INDEX(II,JJ,KK))%SOLID) GAS_PHASE_OUTPUT_RES=1._EB

   CASE(191) ! SCALAR UNKNOWN NUMBER
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (CC_IBM) GAS_PHASE_OUTPUT_RES = REAL(M%CCVAR(II,JJ,KK,CC_UNKZ),EB)

   CASE(192) ! F_X UNKNOWN NUMBER
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (CC_IBM) THEN
         GAS_PHASE_OUTPUT_RES = REAL(M%FCVAR(II,JJ,KK,CC_UNKF,IAXIS),EB)
         IF(M%FCVAR(II,JJ,KK,CC_IDRC,IAXIS)>0) GAS_PHASE_OUTPUT_RES = REAL(M%RC_FACE(M%FCVAR(II,JJ,KK,CC_IDRC,IAXIS))%UNKF,EB)
      ENDIF

   CASE(193) ! F_Y UNKNOWN NUMBER
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (CC_IBM) THEN
         GAS_PHASE_OUTPUT_RES = REAL(M%FCVAR(II,JJ,KK,CC_UNKF,JAXIS),EB)
         IF(M%FCVAR(II,JJ,KK,CC_IDRC,JAXIS)>0) GAS_PHASE_OUTPUT_RES = REAL(M%RC_FACE(M%FCVAR(II,JJ,KK,CC_IDRC,JAXIS))%UNKF,EB)
      ENDIF

   CASE(194) ! F_Z UNKNOWN NUMBER
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (CC_IBM) THEN
         GAS_PHASE_OUTPUT_RES = REAL(M%FCVAR(II,JJ,KK,CC_UNKF,KAXIS),EB)
         IF(M%FCVAR(II,JJ,KK,CC_IDRC,KAXIS)>0) GAS_PHASE_OUTPUT_RES = REAL(M%RC_FACE(M%FCVAR(II,JJ,KK,CC_IDRC,KAXIS))%UNKF,EB)
      ENDIF
   ...
   CASE(231) ! PDPA
      ...
      PDPA_IF: IF ( (PY%PDPA_START<=T .AND. T<=PY%PDPA_END) .OR. .NOT.PY%PDPA_INTEGRATE ) THEN
         ...
         PDPA_FORMULA_SELECT: SELECT CASE(PY%QUANTITY)
            ...
            CASE('PARTICLE FLUX M%X');       PDPA_FORMULA = 2
            CASE('PARTICLE FLUX M%Y');       PDPA_FORMULA = 2
            CASE('PARTICLE FLUX M%Z');       PDPA_FORMULA = 2
            CASE('M%U-VELOCITY');            PDPA_FORMULA = 1
            CASE('M%V-VELOCITY');            PDPA_FORMULA = 1
            CASE('M%W-VELOCITY');            PDPA_FORMULA = 1
         ...
         PDPA_PARTICLE_LOOP: DO I=1,M%NLP
            LP=>M%LAGRANGIAN_PARTICLE(I)
            LPC=>LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
            ...
            PDPA_QUANTITY_SELECT: SELECT CASE(PY%QUANTITY)
               ...
               CASE('ENTHALPY');              PHI = 0._EB
                  ...
                  ELSEIF (LPC%SURF_INDEX>0) THEN
                     ...
                     ENDIF
                  ENDIF
               CASE('PARTICLE FLUX M%X');       PHI = LPC%FTPR*R_D**3*LP%U
               CASE('PARTICLE FLUX M%Y');       PHI = LPC%FTPR*R_D**3*LP%V
               CASE('PARTICLE FLUX M%Z');       PHI = LPC%FTPR*R_D**3*LP%W
               CASE('M%U-VELOCITY');            PHI = LP%U
               CASE('M%V-VELOCITY');            PHI = LP%V
               CASE('M%W-VELOCITY');            PHI = LP%W
   ...
   CASE(251)  ! WIND CHILL INDEX
      ...
      TMP_G = M%TMP(II,JJ,KK) - TMPM ! Temperature as Celsius
      VEL = 1.5_EB*SQRT(2._EB*M%KRES(II,JJ,KK)) ! Flow (wind) speed as m/s at 10 m height
   ...
   CASE(253)  ! ZONE PRESSURE SOLVER TYPE
      ...
      IF (PRES_FLAG==ULMAT_FLAG) THEN
         IF (M%ZONE_MESH(M%ZONE_MESH(M%PRESSURE_ZONE(II,JJ,KK))%CONNECTED_ZONE_PARENT)%USE_FFT) THEN
   ...
   CASE(254)  ! PRESSURE ZONE PARENT
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (PRES_FLAG==ULMAT_FLAG) THEN
         GAS_PHASE_OUTPUT_RES = REAL(M%ZONE_MESH(M%PRESSURE_ZONE(II,JJ,KK))%CONNECTED_ZONE_PARENT,EB)
      ENDIF

   CASE(500)  ! PRESSURE MMS
      XHAT = M%XC(II) - UF_MMS*T
      ZHAT = M%ZC(KK) - WF_MMS*T
   ...
   CASE(502)  ! CHI_R
      ...
      JJJ = MAX(1,MIN(JJ,M%JBAR))
      KKK = MAX(1,MIN(KK,M%KBAR))
      GAS_PHASE_OUTPUT_RES = M%CHI_R(III,JJJ,KKK)
   CASE(504)  ! CFL 1
      IF (M%CELL(M%CELL_INDEX(II,JJ,KK))%SOLID) THEN
         GAS_PHASE_OUTPUT_RES = 0._EB
      ELSE
         UODX = MAXVAL(ABS(M%US(II-1:II,JJ,KK)))*M%RDX(II)
         VODY = MAXVAL(ABS(M%VS(II,JJ-1:JJ,KK)))*M%RDY(JJ)
         WODZ = MAXVAL(ABS(M%WS(II,JJ,KK-1:KK)))*M%RDZ(KK)
         UVW = UODX + VODY + WODZ  + ABS(M%DS(II,JJ,KK)) ! CFL_VELOCITY_NORM=1
         GAS_PHASE_OUTPUT_RES = DT*UVW
      ENDIF
   CASE(505)  ! CFL 3
      IF (M%CELL(M%CELL_INDEX(II,JJ,KK))%SOLID) THEN
   ...
   CASE(508)  ! IDEAL GAS PRESSURE
      ZZ_GET(1:N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RCON)
      GAS_PHASE_OUTPUT_RES = M%RHO(II,JJ,KK)*RCON*M%TMP(II,JJ,KK)

   CASE(513)  ! DHDX
      GAS_PHASE_OUTPUT_RES = M%RDXN(II)*(M%HS(II+1,JJ,KK)-M%HS(II,JJ,KK))
   CASE(514)  ! DHDY
      GAS_PHASE_OUTPUT_RES = M%RDYN(JJ)*(M%HS(II,JJ+1,KK)-M%HS(II,JJ,KK))
   CASE(515)  ! DHDZ
      GAS_PHASE_OUTPUT_RES = M%RDZN(KK)*(M%HS(II,JJ,KK+1)-M%HS(II,JJ,KK))

   CASE(523)  ! ABSOLUTE PRESSURE
      GAS_PHASE_OUTPUT_RES  = M%PBAR(KK,M%PRESSURE_ZONE(II,JJ,KK)) + M%RHO(II,JJ,KK)*(M%H(II,JJ,KK)-M%KRES(II,JJ,KK))
   CASE(528)  ! ADVECTIVE MASS FLUX X
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = M%ADV_FX(II,JJ,KK,Z_INDEX)
      ELSEIF (Y_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES), M%ADV_FX(II,JJ,KK,1:N_TRACKED_SPECIES) )
      ENDIF
   CASE(529)  ! ADVECTIVE MASS FLUX Y
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = M%ADV_FY(II,JJ,KK,Z_INDEX)
      ELSEIF (Y_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES), M%ADV_FY(II,JJ,KK,1:N_TRACKED_SPECIES) )
      ENDIF
   CASE(530)  ! ADVECTIVE MASS FLUX Z
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = M%ADV_FZ(II,JJ,KK,Z_INDEX)
      ELSEIF (Y_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES), M%ADV_FZ(II,JJ,KK,1:N_TRACKED_SPECIES) )
      ENDIF
   CASE(531)  ! DIFFUSIVE MASS FLUX X
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = M%DIF_FX(II,JJ,KK,Z_INDEX)
      ELSEIF (Y_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES), M%DIF_FX(II,JJ,KK,1:N_TRACKED_SPECIES) )
      ENDIF
   CASE(532)  ! DIFFUSIVE MASS FLUX Y
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = M%DIF_FY(II,JJ,KK,Z_INDEX)
      ELSEIF (Y_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES), M%DIF_FY(II,JJ,KK,1:N_TRACKED_SPECIES) )
      ENDIF
   CASE(533)  ! DIFFUSIVE MASS FLUX Z
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = M%DIF_FZ(II,JJ,KK,Z_INDEX)
      ELSEIF (Y_INDEX>0) THEN
         GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES), M%DIF_FZ(II,JJ,KK,1:N_TRACKED_SPECIES) )
      ENDIF
   CASE(534)  ! TOTAL MASS FLUX X
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX<=0 .AND. Y_INDEX<=0) THEN
         GAS_PHASE_OUTPUT_RES = SUM(M%ADV_FX(II,JJ,KK,:) + M%DIF_FX(II,JJ,KK,:))
      ELSE
         IF (Z_INDEX>0) THEN
            GAS_PHASE_OUTPUT_RES = M%ADV_FX(II,JJ,KK,Z_INDEX) + M%DIF_FX(II,JJ,KK,Z_INDEX)
         ELSEIF (Y_INDEX>0) THEN
            GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                                   (M%ADV_FX(II,JJ,KK,1:N_TRACKED_SPECIES) + M%DIF_FX(II,JJ,KK,1:N_TRACKED_SPECIES)) )
         ENDIF
      ENDIF
   CASE(535)  ! TOTAL MASS FLUX Y
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX<=0 .AND. Y_INDEX<=0) THEN
         GAS_PHASE_OUTPUT_RES = SUM(M%ADV_FY(II,JJ,KK,:) + M%DIF_FY(II,JJ,KK,:))
      ELSE
         IF (Z_INDEX>0) THEN
            GAS_PHASE_OUTPUT_RES = M%ADV_FY(II,JJ,KK,Z_INDEX) + M%DIF_FY(II,JJ,KK,Z_INDEX)
         ELSEIF (Y_INDEX>0) THEN
            GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                                   (M%ADV_FY(II,JJ,KK,1:N_TRACKED_SPECIES) + M%DIF_FY(II,JJ,KK,1:N_TRACKED_SPECIES)) )
         ENDIF
      ENDIF
   CASE(536)  ! TOTAL MASS FLUX Z
      GAS_PHASE_OUTPUT_RES = 0._EB
      IF (Z_INDEX<=0 .AND. Y_INDEX<=0) THEN
         GAS_PHASE_OUTPUT_RES = SUM(M%ADV_FZ(II,JJ,KK,:) + M%DIF_FZ(II,JJ,KK,:))
      ELSE
         IF (Z_INDEX>0) THEN
            GAS_PHASE_OUTPUT_RES = M%ADV_FZ(II,JJ,KK,Z_INDEX) + M%DIF_FZ(II,JJ,KK,Z_INDEX)
         ELSEIF (Y_INDEX>0) THEN
            GAS_PHASE_OUTPUT_RES = DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                                   (M%ADV_FZ(II,JJ,KK,1:N_TRACKED_SPECIES) + M%DIF_FZ(II,JJ,KK,1:N_TRACKED_SPECIES)) )
         ENDIF
      ENDIF

   CASE(550) ! CUTCELL VELOCITY DIVERGENCE
      GAS_PHASE_OUTPUT_RES = M%CARTVELDIV(II,JJ,KK)
   CASE(551) ! CARTESIAN VELOCITY DIVERGENCE
      GAS_PHASE_OUTPUT_RES = M%CARTVELDIV(II,JJ,KK)

   CASE(552) ! U_LS
      GAS_PHASE_OUTPUT_RES = M%U_LS(II,JJ)
   CASE(553) ! V_LS
      GAS_PHASE_OUTPUT_RES = M%V_LS(II,JJ)
...
CC_IBM_IF: IF (CC_IBM) THEN
   ...
   IF (M%CCVAR(II,JJ,KK,CC_CGSC) == CC_SOLID .OR. M%CELL(M%CELL_INDEX(II,JJ,KK))%SOLID) EXIT CC_IBM_IF

   CCVAR_IF: IF (M%CCVAR(II,JJ,KK,CC_IDCC) > 0) THEN ! we have a cutcell
      ...
      CC_LOOP: DO JCC=JCC_LO,JCC_HI
         ...
         Y_SPECIES = 1._EB
         IF (Z_INDEX > 0) THEN
            Y_SPECIES = M%CUT_CELL(ICC)%ZZ(Z_INDEX,JCC)
            RCON = SPECIES_MIXTURE(Z_INDEX)%RCON
         ELSEIF (Y_INDEX > 0) THEN
            ZZ_GET(1:N_TRACKED_SPECIES) = M%CUT_CELL(ICC)%ZZ(1:N_TRACKED_SPECIES,JCC)
            RCON = SPECIES(Y_INDEX)%RCON
            CALL GET_MASS_FRACTION(ZZ_GET,Y_INDEX,Y_SPECIES)
         ENDIF
         IF (DRY .AND. H2O_INDEX > 0) THEN
            ...
            IF (Y_INDEX==H2O_INDEX) Y_SPECIES=0._EB
         ENDIF
         VC = VC + M%CUT_CELL(ICC)%VOLUME(JCC)
         IND_SELECT_2: SELECT CASE(IND)
            CASE DEFAULT
               EXIT CCVAR_IF ! GAS_PHASE_OUTPUT_RES is unchanged
            CASE(1)   ! DENSITY
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + M%CUT_CELL(ICC)%RHO(JCC)*Y_SPECIES * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(5)   ! TEMPERATURE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + (M%CUT_CELL(ICC)%TMP(JCC)-TMPM)    * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(9)   ! PRESSURE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + &
                  ( M%PBAR(KK,M%PRESSURE_ZONE(II,JJ,KK)) + &
                    M%CUT_CELL(ICC)%RHO(JCC)*(M%CUT_CELL(ICC)%H(JCC)-M%KRES(II,JJ,KK)) - M%P_0(KK) ) &
                  * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(10)  ! VELOCITY
               IF(II<1 .OR. II>M%IBAR .OR. JJ<1 .OR. JJ>M%JBAR .OR. KK<1 .OR. KK>M%KBAR) THEN
                  GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + SQRT(2._EB*M%KRES(II,JJ,KK)) * M%CUT_CELL(ICC)%VOLUME(JCC)
               ELSE
                  CALL CC_CUTCELL_VELOCITY( &
                     M,0._EB,ICC,JCC, &
                     VELOCITY_COMPONENT,ATOTV, &
                     RETURN_INTEGRALS=.FALSE.)
                  GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + SQRT(DOT_PRODUCT(VELOCITY_COMPONENT,VELOCITY_COMPONENT)) &
                                                                                            * M%CUT_CELL(ICC)%VOLUME(JCC)
               ENDIF
            CASE(11)  ! HRRPUV
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + M%CUT_CELL(ICC)%Q(JCC)*0.001       * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(12)  ! H
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + 0.5_EB*(M%CUT_CELL(ICC)%H(JCC)+M%CUT_CELL(ICC)%HS(JCC)) &
                                                                                            * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(14)  ! DIVERGENCE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + M%CUT_CELL(ICC)%D(JCC)             * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(15)  ! MIXING TIME
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + M%CUT_CELL(ICC)%MIX_TIME(JCC)      * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(19)  ! RADIATION LOSS
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + M%CUT_CELL(ICC)%QR(JCC)*0.001      * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(22)  ! HS
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + M%CUT_CELL(ICC)%HS(JCC)            * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(90)  ! MASS FRACTION
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + Y_SPECIES/(1._EB-Y_H2O)          * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(94)  ! VOLUME FRACTION
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + &
                                     RCON*Y_SPECIES/M%CUT_CELL(ICC)%RSUM(JCC)/(1._EB-R_Y_H2O/M%CUT_CELL(ICC)%RSUM(JCC)) &
                                                                                            * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(138) ! HRRPUV REAC
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + M%CUT_CELL(ICC)%Q_REAC(JCC,REAC_INDEX)*0.001_EB &
                                                                                            * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(191) ! SCALAR UNKNOWN NUMBER
               GAS_PHASE_OUTPUT_RES = REAL(M%CUT_CELL(ICC)%UNKZ(JCC),EB); RETURN

            CASE(523) ! ABSOLUTE PRESSURE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + &
                  ( M%PBAR(KK,M%PRESSURE_ZONE(II,JJ,KK)) + M%CUT_CELL(ICC)%RHO(JCC)*(M%CUT_CELL(ICC)%H(JCC)-M%KRES(II,JJ,KK)) ) &
                                                                                            * M%CUT_CELL(ICC)%VOLUME(JCC)
            CASE(550) ! CUTCELL VELOCITY DIVERGENCE
               GAS_PHASE_OUTPUT_CC = GAS_PHASE_OUTPUT_CC + M%CCVELDIV(II,JJ,KK)*M%CUT_CELL(ICC)%VOLUME(JCC)
   ...
   AXIS_IF: IF (AXIS>0) THEN
      FCVAR_IF: IF (M%FCVAR(II,JJ,KK,CC_IDCF,AXIS)>0) THEN
         ! face centered quantities
         GAS_PHASE_OUTPUT_CFA = 0._EB
         CFACE_AREA = 0._EB
         ICF=M%FCVAR(II,JJ,KK,CC_IDCF,AXIS)
         NFACE=M%CUT_FACE(ICF)%NFACE
         CFA_LOOP: DO JCF=1,NFACE
            CFACE_AREA = CFACE_AREA + M%CUT_FACE(ICF)%AREA(JCF)

            IND_SELECT_3: SELECT CASE(IND)
               CASE DEFAULT
                  EXIT FCVAR_IF ! GAS_PHASE_OUTPUT_RES is unchanged
               CASE( 2, 3, 4)  ! F_X, F_Y, F_Z
                  GAS_PHASE_OUTPUT_CFA = GAS_PHASE_OUTPUT_CFA + M%CUT_FACE(ICF)%FN(JCF) * M%CUT_FACE(ICF)%AREA(JCF)
               CASE(6)   ! U-VELOCITY
                  GAS_PHASE_OUTPUT_CFA = GAS_PHASE_OUTPUT_CFA + M%CUT_FACE(ICF)%VEL(JCF) * M%CUT_FACE(ICF)%AREA(JCF)
               CASE(7)   ! V-VELOCITY
                  GAS_PHASE_OUTPUT_CFA = GAS_PHASE_OUTPUT_CFA + M%CUT_FACE(ICF)%VEL(JCF) * M%CUT_FACE(ICF)%AREA(JCF)
               CASE(8)   ! W-VELOCITY
                  GAS_PHASE_OUTPUT_CFA = GAS_PHASE_OUTPUT_CFA + M%CUT_FACE(ICF)%VEL(JCF) * M%CUT_FACE(ICF)%AREA(JCF)
               CASE(192:194) ! F_X,F_Y,F_Z UNKNOWN NUMBER
                  GAS_PHASE_OUTPUT_RES = REAL(M%CUT_FACE(ICF)%UNKF(JCF),EB); RETURN
...
USE TURB_KERNELS, ONLY: TAU_WALL_IJ
...
REAL(EB),INTENT(IN) :: T
REAL(EB) :: Q_CON,RHOSUM,VOLSUM,ZZ_GET(1:MAX_SPECIES),Y_SPECIES,DEPTH,ASH_DEPTH,UN,H_S,RHO_D_DYDN,U_CELL,V_CELL,W_CELL,&
...
IF (PRESENT(OPT_WALL_INDEX)) THEN
   ...
   IF (WC%BC_INDEX>0) BC    => M%BOUNDARY_COORD(WC%BC_INDEX)
   IF (WC%B1_INDEX>0) B1    => M%BOUNDARY_PROP1(WC%B1_INDEX)
   IF (WC%B2_INDEX>0) B2    => M%BOUNDARY_PROP2(WC%B2_INDEX)
   IF (WC%BR_INDEX>0) BR    => M%BOUNDARY_RADIA(WC%BR_INDEX)

ELSEIF (PRESENT(OPT_LP_INDEX)) THEN

   LP => M%LAGRANGIAN_PARTICLE(OPT_LP_INDEX)
   IF (LP%OD_INDEX>0) ONE_D => M%BOUNDARY_ONE_D(LP%OD_INDEX)
   IF (LP%BC_INDEX>0) BC    => M%BOUNDARY_COORD(LP%BC_INDEX)
   IF (LP%B1_INDEX>0) B1    => M%BOUNDARY_PROP1(LP%B1_INDEX)
   IF (LP%B2_INDEX>0) B2    => M%BOUNDARY_PROP2(LP%B2_INDEX)
   IF (LP%BR_INDEX>0) BR    => M%BOUNDARY_RADIA(LP%BR_INDEX)
   SURF_INDEX = LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)%SURF_INDEX

ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN

   CFA => M%CFACE(OPT_CFACE_INDEX)
   SURF_INDEX = CFA%SURF_INDEX
   IF (CFA%OD_INDEX>0) ONE_D => M%BOUNDARY_ONE_D(CFA%OD_INDEX)
   IF (CFA%BC_INDEX>0) BC    => M%BOUNDARY_COORD(CFA%BC_INDEX)
   IF (CFA%B1_INDEX>0) B1    => M%BOUNDARY_PROP1(CFA%B1_INDEX)
   IF (CFA%B2_INDEX>0) B2    => M%BOUNDARY_PROP2(CFA%B2_INDEX)
   IF (CFA%BR_INDEX>0) BR    => M%BOUNDARY_RADIA(CFA%BR_INDEX)
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE( 3) ! NORMAL VELOCITY
      SELECT CASE(BC%IOR)
         CASE( 1) ; SOLID_PHASE_OUTPUT = -M%U(BC%IIG-1,BC%JJG,BC%KKG)
         CASE(-1) ; SOLID_PHASE_OUTPUT =  M%U(BC%IIG  ,BC%JJG,BC%KKG)
         CASE( 2) ; SOLID_PHASE_OUTPUT = -M%V(BC%IIG,BC%JJG-1,BC%KKG)
         CASE(-2) ; SOLID_PHASE_OUTPUT =  M%V(BC%IIG,BC%JJG  ,BC%KKG)
         CASE( 3) ; SOLID_PHASE_OUTPUT = -M%W(BC%IIG,BC%JJG,BC%KKG-1)
         CASE(-3) ; SOLID_PHASE_OUTPUT =  M%W(BC%IIG,BC%JJG,BC%KKG  )
      END SELECT
      IF(PRESENT(OPT_CFACE_INDEX)) THEN
         IND1 = CFA%CUT_FACE_IND1
         IND2 = CFA%CUT_FACE_IND2
         SOLID_PHASE_OUTPUT = M%CUT_FACE(IND1)%VEL(IND2)
      ELSEIF (ICF>0) THEN
         SOLID_PHASE_OUTPUT_CTF = 0._EB
         CUT_FACE_AREA = 0._EB
         NFACE=M%CUT_FACE(ICF)%NFACE
         DO JCF=1,NFACE
            CUT_FACE_AREA = CUT_FACE_AREA + M%CUT_FACE(ICF)%AREA(JCF)
            SOLID_PHASE_OUTPUT_CTF = SOLID_PHASE_OUTPUT_CTF &
                                   - SIGN(1._EB,REAL(BC%IOR,EB))*M%CUT_FACE(ICF)%VEL(JCF)*M%CUT_FACE(ICF)%AREA(JCF)
         ENDDO
   ...
   CASE(11) ! PRESSURE COEFFICIENT
      IF (PRESENT(OPT_WALL_INDEX)) THEN
         ...
         JJG = BC%JJG
         KKG = BC%KKG
         PRESS = M%RHO(IIG,JJG,KKG)*(M%H(IIG,JJG,KKG)-M%KRES(IIG,JJG,KKG))
      ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
         IND1 = CFA%CUT_FACE_IND1
         IND2 = CFA%CUT_FACE_IND2
   ...
   CASE(13) ! GAUGE HEAT FLUX
      IF (PY%HEAT_TRANSFER_COEFFICIENT>=0._EB) THEN
         Q_CON = PY%HEAT_TRANSFER_COEFFICIENT*(M%TMP(BC%IIG,BC%JJG,BC%KKG)-PY%GAUGE_TEMPERATURE)
      ELSE
         IF (SF%BLOWING) THEN
            IF (OPT_WALL_INDEX > 0) THEN
               HTC=HEAT_TRANSFER_COEFFICIENT(M,T,M%TMP(BC%IIG,BC%JJG,BC%KKG)-B1%TMP_F,SF,WALL_INDEX_IN=OPT_WALL_INDEX,&
                                             SKIP_BLOWING=.TRUE.)
            ELSEIF (OPT_CFACE_INDEX > 0) THEN
               HTC=HEAT_TRANSFER_COEFFICIENT(M,T,M%TMP(BC%IIG,BC%JJG,BC%KKG)-B1%TMP_F,SF,CFACE_INDEX_IN=OPT_CFACE_INDEX,&
                                             SKIP_BLOWING=.TRUE.)
            ELSE
               HTC=HEAT_TRANSFER_COEFFICIENT(M,T,M%TMP(BC%IIG,BC%JJG,BC%KKG)-B1%TMP_F,SF,PARTICLE_INDEX_IN=OPT_LP_INDEX,&
                                             SKIP_BLOWING=.TRUE.)
            ENDIF
            Q_CON = HTC*(M%TMP(BC%IIG,BC%JJG,BC%KKG)-PY%GAUGE_TEMPERATURE)
         ELSE
            Q_CON = B1%HEAT_TRANS_COEF*(M%TMP(BC%IIG,BC%JJG,BC%KKG)-PY%GAUGE_TEMPERATURE)
         ENDIF
      ENDIF
   ...
   CASE(23) ! ADIABATIC SURFACE TEMPERATURE (Ferrari's Method for Solving the Quartic)
      ...
      IF (B1%HEAT_TRANS_COEF+H_W_EFF>1.E-5_EB .AND. ABS(B1%Q_RAD_IN-AAA*M%TMP(BC%IIG,BC%JJG,BC%KKG)**4)>5.E-3_EB) THEN
         AAA = B1%EMISSIVITY*SIGMA
         BBB = B1%HEAT_TRANS_COEF + H_W_EFF
         CCC = -B1%Q_RAD_IN - B1%HEAT_TRANS_COEF*M%TMP(BC%IIG,BC%JJG,BC%KKG) - H_W_EFF*LTMP
   ...
   CASE(38) ! WALL VISCOSITY
      IF (PRESENT(OPT_WALL_INDEX)) THEN
         SOLID_PHASE_OUTPUT = M%MU(BC%IIG,BC%JJG,BC%KKG)
      ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
   ...
   CASE(51)  ! ENTHALPY FLUX WALL
      ...
      SOLID_PHASE_OUTPUT = (-B1%RHO_F*H_S*B1%U_NORMAL &
                            -2._EB*B1%K_G*(M%TMP(BC%IIG,BC%JJG,BC%KKG)-B1%TMP_F)*B1%RDN)*0.001_EB

   CASE(60)  ! MASS FLUX WALL
      IF (PRESENT(OPT_WALL_INDEX)) THEN
         SELECT CASE(BC%IOR)
            CASE(-1) ; UN = -M%U(BC%IIG  ,BC%JJG,BC%KKG)
            CASE( 1) ; UN =  M%U(BC%IIG-1,BC%JJG,BC%KKG)
            CASE(-2) ; UN = -M%V(BC%IIG,BC%JJG  ,BC%KKG)
            CASE( 2) ; UN =  M%V(BC%IIG,BC%JJG-1,BC%KKG)
            CASE(-3) ; UN = -M%W(BC%IIG,BC%JJG,BC%KKG  )
            CASE( 3) ; UN =  M%W(BC%IIG,BC%JJG,BC%KKG-1)
         END SELECT
      ELSE
         UN = -B1%U_NORMAL
      ENDIF
   ...
   CASE(64) ! TOTAL MASS FLUX WALL
      ...
      IF (STORE_SPECIES_FLUX .AND. PRESENT(OPT_WALL_INDEX)) THEN ! Case of External walls or Obsts.
         ...
         IF (Z_INDEX>0) THEN
            SELECT CASE(BC%IOR)
            CASE(-1); SOLID_PHASE_OUTPUT=-(M%ADV_FX(IIG  ,JJG  ,KKG  ,Z_INDEX)+M%DIF_FX(IIG  ,JJG  ,KKG  ,Z_INDEX))
            CASE( 1); SOLID_PHASE_OUTPUT= (M%ADV_FX(IIG-1,JJG  ,KKG  ,Z_INDEX)+M%DIF_FX(IIG-1,JJG  ,KKG  ,Z_INDEX))
            CASE(-2); SOLID_PHASE_OUTPUT=-(M%ADV_FY(IIG  ,JJG  ,KKG  ,Z_INDEX)+M%DIF_FY(IIG  ,JJG  ,KKG  ,Z_INDEX))
            CASE( 2); SOLID_PHASE_OUTPUT= (M%ADV_FY(IIG  ,JJG-1,KKG  ,Z_INDEX)+M%DIF_FY(IIG  ,JJG-1,KKG  ,Z_INDEX))
            CASE(-3); SOLID_PHASE_OUTPUT=-(M%ADV_FZ(IIG  ,JJG  ,KKG  ,Z_INDEX)+M%DIF_FZ(IIG  ,JJG  ,KKG  ,Z_INDEX))
            CASE( 3); SOLID_PHASE_OUTPUT= (M%ADV_FZ(IIG  ,JJG  ,KKG-1,Z_INDEX)+M%DIF_FZ(IIG  ,JJG  ,KKG-1,Z_INDEX))
            END SELECT
         ELSEIF (Y_INDEX>0) THEN
            SELECT CASE(BC%IOR)
            CASE(-1); SOLID_PHASE_OUTPUT=-1._EB*DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                      (M%ADV_FX(IIG  ,JJG  ,KKG  ,1:N_TRACKED_SPECIES)+M%DIF_FX(IIG  ,JJG  ,KKG  ,1:N_TRACKED_SPECIES)) )
            CASE( 1); SOLID_PHASE_OUTPUT= 1._EB*DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                      (M%ADV_FX(IIG-1,JJG  ,KKG  ,1:N_TRACKED_SPECIES)+M%DIF_FX(IIG-1,JJG  ,KKG  ,1:N_TRACKED_SPECIES)) )
            CASE(-2); SOLID_PHASE_OUTPUT=-1._EB*DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                      (M%ADV_FY(IIG  ,JJG  ,KKG  ,1:N_TRACKED_SPECIES)+M%DIF_FY(IIG  ,JJG  ,KKG  ,1:N_TRACKED_SPECIES)) )
            CASE( 2); SOLID_PHASE_OUTPUT= 1._EB*DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                      (M%ADV_FY(IIG  ,JJG-1,KKG  ,1:N_TRACKED_SPECIES)+M%DIF_FY(IIG  ,JJG-1,KKG  ,1:N_TRACKED_SPECIES)) )
            CASE(-3); SOLID_PHASE_OUTPUT=-1._EB*DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                      (M%ADV_FZ(IIG  ,JJG  ,KKG  ,1:N_TRACKED_SPECIES)+M%DIF_FZ(IIG  ,JJG  ,KKG  ,1:N_TRACKED_SPECIES)) )
            CASE( 3); SOLID_PHASE_OUTPUT= 1._EB*DOT_PRODUCT( Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),&
                      (M%ADV_FZ(IIG  ,JJG  ,KKG-1,1:N_TRACKED_SPECIES)+M%DIF_FZ(IIG  ,JJG  ,KKG-1,1:N_TRACKED_SPECIES)) )
            END SELECT
         ENDIF
      ELSE
         UN  = -B1%U_NORMAL
   ...
   CASE(65) ! WALL PRESSURE (takes optional FORCE_DIRECTION vector)
      IF (PRESENT(OPT_WALL_INDEX)) THEN
         ...
         PR1 = M%RHO(IIG,JJG,KKG)*(M%H(IIG,JJG,KKG)-M%KRES(IIG,JJG,KKG))
         PR2 = PR1
         SELECT CASE(BC%IOR)
            CASE( 1)
               NVEC=(/ 1._EB,0._EB,0._EB/)
               Z1 = 0.5_EB*M%DX(IIG)
               Z2 = M%DX(IIG)+0.5_EB*M%DX(IIG+1)
               IC2 = M%CELL_INDEX(IIG+1,JJG,KKG)
               IF (.NOT.M%CELL(IC2)%SOLID) PR2 = M%RHO(IIG,JJG,KKG)*(M%H(IIG+1,JJG,KKG)-M%KRES(IIG+1,JJG,KKG))
            CASE(-1)
               NVEC=(/-1._EB,0._EB,0._EB/)
               Z1 = 0.5_EB*M%DX(IIG)
               Z2 = M%DX(IIG)+0.5_EB*M%DX(IIG-1)
            ...
            CASE( 2)
               ...
               Z1 = 0.5_EB*M%DY(JJG)
               Z2 = M%DY(JJG)+0.5_EB*M%DY(JJG+1)
               IC2 = M%CELL_INDEX(IIG,JJG+1,KKG)
               IF (.NOT.M%CELL(IC2)%SOLID) PR2 = M%RHO(IIG,JJG,KKG)*(M%H(IIG,JJG+1,KKG)-M%KRES(IIG,JJG+1,KKG))
            CASE(-2)
               NVEC=(/0._EB,-1._EB,0._EB/)
               Z1 = 0.5_EB*M%DY(JJG)
               Z2 = M%DY(JJG)+0.5_EB*M%DY(JJG-1)
            ...
            CASE( 3)
               ...
               Z1 = 0.5_EB*M%DZ(KKG)
               Z2 = M%DZ(KKG)+0.5_EB*M%DZ(KKG+1)
               IC2 = M%CELL_INDEX(IIG,JJG,KKG+1)
               IF (.NOT.M%CELL(IC2)%SOLID) PR2 = M%RHO(IIG,JJG,KKG)*(M%H(IIG,JJG,KKG+1)-M%KRES(IIG,JJG,KKG+1))
            CASE(-3)
               NVEC=(/0._EB,0._EB,-1._EB/)
               Z1 = 0.5_EB*M%DZ(KKG)
               Z2 = M%DZ(KKG)+0.5_EB*M%DZ(KKG-1)
   ...
   CASE(66) ! VISCOUS STRESS WALL (takes optional FORCE_DIRECTION vector)
      IF (PRESENT(OPT_WALL_INDEX)) THEN
         ...
         JJG = BC%JJG
         KKG = BC%KKG
         U_CELL = 0.5_EB*(M%U(IIG-1,JJG,KKG)+M%U(IIG,JJG,KKG))
         V_CELL = 0.5_EB*(M%V(IIG,JJG-1,KKG)+M%V(IIG,JJG,KKG))
         W_CELL = 0.5_EB*(M%W(IIG,JJG,KKG-1)+M%W(IIG,JJG,KKG))
         MU_WALL = M%MU_DNS(IIG,JJG,KKG)
      ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
         NVEC = BC%NVEC
         ...
         IND1 = CFA%CUT_FACE_IND1
         IND2 = CFA%CUT_FACE_IND2
         CALL GET_UVWGAS_CFACE(M,U_CELL,V_CELL,W_CELL,IND1,IND2,M%U,M%V,M%W,PREDFCT=1._EB)
         CALL GET_MUDNS_CFACE(MU_WALL,IND1,IND2)
         ICC = M%CUT_FACE(IND1)%CELL_LIST(2,LOW_IND,IND2)
         IIG = M%CUT_CELL(ICC)%IJK(1)
         JJG = M%CUT_CELL(ICC)%IJK(2)
         KKG = M%CUT_CELL(ICC)%IJK(3)
      ENDIF
      ...
      IF (PRESENT(OPT_WALL_INDEX) .OR. PRESENT(OPT_CFACE_INDEX)) THEN
         ...
         ENDIF
         RHO_WALL = B1%RHO_F
         CALL TAU_WALL_IJ(TAU_IJ,SVEC,VEL_CELL,VEL_WALL,NVEC,DN,M%D(IIG,JJG,KKG),MU_WALL,RHO_WALL,SF%ROUGHNESS)
         DO I=1,3
   ...
   CASE(67) ! WALL PRESSURE TEST (takes optional FORCE_DIRECTION vector)
      IF (PRESENT(OPT_WALL_INDEX)) THEN
         ...
         JJG = BC%JJG
         KKG = BC%KKG
         PVEC = M%RHO(IIG,JJG,KKG)*M%H(IIG,JJG,KKG) * NVEC ! surface normal pressure force
      ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
         NVEC = BC%NVEC
         ! find cut-cell adjacent to CFACE
         IND1 = CFA%CUT_FACE_IND1
   ...
   CASE(71) ! EFFECTIVE HEAT TRANSFER COEFFICIENT
      DTMP = M%TMP(BC%IIG,BC%JJG,BC%KKG)-0.5_EB*(B1%TMP_F_OLD+B1%TMP_F)
   ...
   CASE(90:92) ! FIRE ARRIVAL TIME, FIRE RESIDENCE TIME, LS SPREAD RATE
      ...
      ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
         OUTPUT_INDEX = OPT_CFACE_INDEX-M%INTERNAL_CFACE_CELLS_LB+M%N_INTERNAL_WALL_CELLS+M%N_EXTERNAL_WALL_CELLS
      ENDIF
      SELECT CASE(INDX)
         CASE(90); SOLID_PHASE_OUTPUT = M%FIRE_ARRIVAL_TIME(OUTPUT_INDEX)
         CASE(91); SOLID_PHASE_OUTPUT = M%FIRE_RESIDENCE_TIME(OUTPUT_INDEX)
         CASE(92); SOLID_PHASE_OUTPUT = M%LS_SPREAD_RATE(OUTPUT_INDEX)
      END SELECT
...
REAL(EB) :: Y_H2O,ZZ_GET(1:MAX_SPECIES),MW,Y_SPECIES,RCON,H_DUCT,H_TMPA
LOGICAL :: DRY
...
SELECT CASE(IND)
   ...
   CASE(436)  ! PARTICLE PHASE
      PARTICLE_OUTPUT = M%BOUNDARY_COORD(LP%BC_INDEX)%IOR
   CASE(437)  ! PARTICLE TEMPERATURE
      PARTICLE_OUTPUT = M%BOUNDARY_PROP1(LP%B1_INDEX)%TMP_F - TMPM
   ...
   CASE(441)  ! PARTICLE X
      PARTICLE_OUTPUT = M%BOUNDARY_COORD(LP%BC_INDEX)%X
   CASE(442)  ! PARTICLE Y
      PARTICLE_OUTPUT = M%BOUNDARY_COORD(LP%BC_INDEX)%Y
   CASE(443)  ! PARTICLE Z
      PARTICLE_OUTPUT = M%BOUNDARY_COORD(LP%BC_INDEX)%Z
   CASE(444)  ! PARTICLE U
   ...
   CASE(450)  ! PARTICLE DRAG FORCE X
      BC=>M%BOUNDARY_COORD(LP%BC_INDEX)
      PARTICLE_OUTPUT = M%RHO(BC%IIG,BC%JJG,BC%KKG) / LP%RVC * LP%ACCEL_X
   CASE(451)  ! PARTICLE DRAG FORCE Y
      BC=>M%BOUNDARY_COORD(LP%BC_INDEX)
      PARTICLE_OUTPUT = M%RHO(BC%IIG,BC%JJG,BC%KKG) / LP%RVC * LP%ACCEL_Y
   CASE(452)  ! PARTICLE DRAG FORCE Z
      BC=>M%BOUNDARY_COORD(LP%BC_INDEX)
      PARTICLE_OUTPUT = M%RHO(BC%IIG,BC%JJG,BC%KKG) / LP%RVC * LP%ACCEL_Z
   ...
   CASE(455)  ! PARTICLE HEAT TRANSFER COEFFICIENT
      PARTICLE_OUTPUT = M%BOUNDARY_PROP1(LP%B1_INDEX)%HEAT_TRANS_COEF
   CASE(456)  ! PARTICLE RADIATIVE HEAT FLUX
      PARTICLE_OUTPUT = (M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_RAD_IN-M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_RAD_OUT)*0.001_EB
   CASE(457)  ! PARTICLE CONVECTIVE HEAT FLUX
      PARTICLE_OUTPUT = M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_CON_F*0.001_EB
   CASE(458)  ! PARTICLE TOTAL HEAT FLUX
      PARTICLE_OUTPUT = ( M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_RAD_IN-M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_RAD_OUT &
                        + M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_CON_F )*0.001_EB
   CASE(459)  ! PARTICLE POWER
      PARTICLE_OUTPUT = -( M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_RAD_IN-M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_RAD_OUT &
                        + M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_CON_F)*0.001_EB*M%BOUNDARY_PROP1(LP%B1_INDEX)%AREA*LP%PWT
...
REAL(EB), INTENT(IN) :: DT
INTEGER, INTENT(IN) :: NM
CALL UPDATE_HRR_TS(MESHES(NM),DT,NM)
Q_DOT = Q_DOT + Q_DOT_MESH(:,NM)
M_DOT = M_DOT + M_DOT_MESH(:,NM)
...
RECURSIVE SUBROUTINE UPDATE_HRR_TS(M,DT,NM)
...
INTEGER, POINTER, DIMENSION(:,:,:) :: CELL_INDEX
INTEGER, POINTER, DIMENSION(:,:,:,:) :: CCVAR
REAL(EB), POINTER, DIMENSION(:) :: DX, DY, DZ, RC
REAL(EB), POINTER, DIMENSION(:,:,:) :: Q, QR, TMP, RHO, U, V, W
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZ
REAL(EB), POINTER, DIMENSION(:) :: D_PBAR_DT, D_PBAR_DT_S
INTEGER, POINTER, DIMENSION(:,:,:) :: PRESSURE_ZONE, INTERPOLATED_MESH
TYPE(WALL_TYPE), POINTER, DIMENSION(:) :: WALL
TYPE(BOUNDARY_COORD_TYPE), POINTER, DIMENSION(:) :: BOUNDARY_COORD
TYPE(BOUNDARY_PROP1_TYPE), POINTER, DIMENSION(:) :: BOUNDARY_PROP1
TYPE(CFACE_TYPE), POINTER, DIMENSION(:) :: CFACE
...
TYPE(CC_CUTCELL_TYPE), POINTER, DIMENSION(:) :: CUT_CELL
TYPE(LAGRANGIAN_PARTICLE_TYPE), POINTER, DIMENSION(:) :: LAGRANGIAN_PARTICLE
INTEGER :: IBAR, JBAR, KBAR, N_EXTERNAL_WALL_CELLS, N_INTERNAL_WALL_CELLS, NLP
INTEGER :: INTERNAL_CFACE_CELLS_LB, N_INTERNAL_CFACE_CELLS
...
REAL(EB) :: VC,AREA_F,U_N,ZZ_GET(1:MAX_SPECIES),H_S,H_S_ALPHA,H_S_J_ALPHA,ENTHALPY_SUM_OLD
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         ...
         Q_DOT_MESH(1,NM) = Q_DOT_MESH(1,NM) + Q(I,J,K)*VC
         Q_DOT_MESH(3,NM) = Q_DOT_MESH(3,NM) + QR(I,J,K)*VC
         Q_DOT_MESH(7,NM) = Q_DOT_MESH(7,NM) + 0.5_EB*(D_PBAR_DT_S(PRESSURE_ZONE(I,J,K))+D_PBAR_DT(PRESSURE_ZONE(I,J,K)))*VC
...
IF (CC_IBM) CALL ADD_Q_DOT_CUTCELLS_TS(NM,Q_DOT_MESH(1,NM),Q_DOT_MESH(3,NM),Q_DOT_MESH(7,NM),ENTHALPY_SUM(NM))

IF (ICYC>0) THEN
   Q_DOT_MESH(9,NM) = Q_DOT_MESH(9,NM) + (ENTHALPY_SUM(NM)-ENTHALPY_SUM_OLD)/DT
ELSE
   Q_DOT_MESH(9,NM) = 0._EB
ENDIF
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   Q_DOT_MESH(2,NM) = Q_DOT_MESH(2,NM) + B1%Q_DOT_O2_PP*AREA_F
   Q_DOT_MESH(4,NM) = Q_DOT_MESH(4,NM) - U_N*B1%RHO_F*H_S*AREA_F
   Q_DOT_MESH(5,NM) = Q_DOT_MESH(5,NM) - B1%Q_CON_F*AREA_F
   Q_DOT_MESH(6,NM) = Q_DOT_MESH(6,NM) - H_S_J_ALPHA*AREA_F
ENDDO WALL_LOOP
...
IF (OXIDATION_REACTION) THEN
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      IF (TWO_D)       AREA_F = AREA_F/DY(BC%JJG)
      IF (CYLINDRICAL) AREA_F = AREA_F*2._EB*PI
      Q_DOT_MESH(2,NM) = Q_DOT_MESH(2,NM) + LP%PWT*B1%Q_DOT_O2_PP*AREA_F
   ENDDO PARTICLE_LOOP
ENDIF
...
WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (CYLINDRICAL) AREA_F = AREA_F*2._EB*PI
   DO N=1,N_TRACKED_SPECIES
      M_DOT_MESH(N,NM) = M_DOT_MESH(N,NM) + B1%M_DOT_G_PP_ADJUST(N)*AREA_F
   ENDDO
ENDDO WALL_LOOP2
...
CFACE_LOOP_2 : DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   IF (CYLINDRICAL) AREA_F = AREA_F*2._EB*PI
   DO N=1,N_TRACKED_SPECIES
      M_DOT_MESH(N,NM) = M_DOT_MESH(N,NM) + B1%M_DOT_G_PP_ADJUST(N)*AREA_F*B1%AREA_ADJUST
   ENDDO
ENDDO CFACE_LOOP_2

CONTAINS

SUBROUTINE ADD_Q_DOT_CUTCELLS_TS(NM,QCOMB,QRAD,QPRES,SP_ENTH)
...
END SUBROUTINE ADD_Q_DOT_CUTCELLS_TS

END SUBROUTINE UPDATE_HRR_TS
...
REAL(EB), INTENT(IN) :: DT
INTEGER, INTENT(IN) :: NM
CALL UPDATE_MASS_TS(MESHES(NM),DT,NM)
MASS_DT = MASS_DT + DT*MASS_INTEGRAL_MESH(:,NM)
END SUBROUTINE UPDATE_MASS


RECURSIVE SUBROUTINE UPDATE_MASS_TS(M,DT,NM)
...
REAL(EB) :: VC,Y_MF_INT(1:MAX_SPECIES),ZZ_GET(1:MAX_SPECIES),MASS_INTEGRAL(0:2*MAX_SPECIES)
INTEGER :: I,J,K,ICC,JCC,NCELL

MASS_INTEGRAL_MESH(:,NM) = 0._EB
...
MASS_INTEGRAL(0:N_SPECIES+N_TRACKED_SPECIES) = 0._EB
         ...
         IF (CC_IBM) THEN
            ...
            IF (CCVAR(I,J,K,CC_IDCC) > 0) THEN
               ...
               DO JCC=1,NCELL
                  ...
                  ZZ_GET(1:N_TRACKED_SPECIES) = CUT_CELL(ICC)%ZZ(1:N_TRACKED_SPECIES,JCC)
                  CALL GET_MASS_FRACTION_ALL(ZZ_GET(1:N_TRACKED_SPECIES),Y_MF_INT(1:N_SPECIES))
...
MASS_INTEGRAL_MESH(0:N_SPECIES+N_TRACKED_SPECIES,NM) = MASS_INTEGRAL(0:N_SPECIES+N_TRACKED_SPECIES)

END SUBROUTINE UPDATE_MASS_TS
...
IF (M%N_PATCH==0 .AND. M%N_INTERNAL_CFACE_CELLS==0) RETURN
IF (.NOT. M%BNDF_DUMP) RETURN
...
FILE_LOOP: DO NF=1,N_BNDF
   IF (M%N_PATCH == 0) CYCLE FILE_LOOP
   BF => BOUNDARY_FILE(NF)
   ...
   PATCH_LOOP: DO IP=1,M%N_PATCH
      CALL COMPUTE_BNDF_PATCH
      CALL WRITE_BNDF_PATCH
   ENDDO PATCH_LOOP

   IF (OUTPUT_QUANTITY(BF%INDEX)%NAME=='FIRE ARRIVAL TIME') &
      BOUND_MAX = MAX(REAL(T_BEGIN,FB),MAXVAL(REAL(M%FIRE_ARRIVAL_TIME,FB),MASK=M%FIRE_ARRIVAL_TIME<9.E5_EB))
...
M%PP  = REAL(OUTPUT_QUANTITY(-IND)%AMBIENT_VALUE,FB)
M%PPN = 0._FB
M%IBK = 0
...
DO K=PA%KG1,PA%KG2
   DO J=PA%JG1,PA%JG2
      DO I=PA%IG1,PA%IG2
         IC = M%CELL_INDEX(I,J,K)
         IW = M%CELL(IC)%WALL_INDEX(-PA%IOR) ; IF (IW==0) CYCLE
         SELECT CASE(ABS(PA%IOR))
            CASE(1) ; L=J ; N=K
            CASE(2) ; L=I ; N=K
            CASE(3) ; L=I ; N=J
         END SELECT
         IF (M%WALL(IW)%BOUNDARY_TYPE/=NULL_BOUNDARY .AND. &
             M%WALL(IW)%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY .AND. .NOT.M%CELL(IC)%SOLID) THEN
            M%IBK(L,N) = 1
            M%PP(L,N)  = REAL(SOLID_PHASE_OUTPUT(IND,T,NM,BF%Y_INDEX,BF%Z_INDEX,BF%PART_INDEX,OPT_WALL_INDEX=IW,&
...
IF (BNDF_COUNTER(NM)>0 .AND. BF%TIME_INTEGRAL_INDEX>0) THEN
   DO N=N1,N2
      DO L=L1,L2
         NC = NC + 1
         M%BNDF_TIME_INTEGRAL(NC,BF%TIME_INTEGRAL_INDEX) = M%BNDF_TIME_INTEGRAL(NC,BF%TIME_INTEGRAL_INDEX) + &
            M%PP(L,N)*REAL(BNDF_CLOCK(BNDF_COUNTER(NM))-BNDF_CLOCK(BNDF_COUNTER(NM)-1),FB)
         M%PP(L,N) = M%BNDF_TIME_INTEGRAL(NC,BF%TIME_INTEGRAL_INDEX)
      ENDDO
   ENDDO
ENDIF
...
IF (.NOT.BF%CELL_CENTERED) THEN
   ...
   IF (OUTPUT_QUANTITY(BF%INDEX)%NAME=='FIRE ARRIVAL TIME') THEN
      WHERE(M%PP>9.E5_FB) M%IBK=0
   ENDIF

   DO N=N1-1,N2
      DO L=L1-1,L2
         IF (M%IBK(L,N)==1)     M%PPN(L,N) = M%PPN(L,N) + M%PP(L,N)
         IF (M%IBK(L+1,N)==1)   M%PPN(L,N) = M%PPN(L,N) + M%PP(L+1,N)
         IF (M%IBK(L,N+1)==1)   M%PPN(L,N) = M%PPN(L,N) + M%PP(L,N+1)
         IF (M%IBK(L+1,N+1)==1) M%PPN(L,N) = M%PPN(L,N) + M%PP(L+1,N+1)
         ISUM = M%IBK(L,N)+M%IBK(L,N+1)+M%IBK(L+1,N)+M%IBK(L+1,N+1)
         IF (ISUM>0) THEN
            M%PPN(L,N) = M%PPN(L,N)/REAL(ISUM,FB)
         ELSE
            M%PPN(L,N) = REAL(SOLID_PHASE_OUTPUT(IND,T,NM,BF%Y_INDEX,BF%Z_INDEX,BF%PART_INDEX,OPT_WALL_INDEX=0,&
                                               OPT_BNDF_INDEX=NF),FB)
         ENDIF
      ENDDO
   ENDDO
...
IF (.NOT.BF%CELL_CENTERED) THEN
   IF (BF%DEBUG .EQ. 0) THEN
      WRITE(LU_BNDF(NF,NM)) ((M%PPN(L,N),L=L1-1,L2),N=N1-1,N2)
      DO L = L1-1, L2
      DO N = N1-1, N2
         BOUND_MIN = MIN(M%PPN(L,N),BOUND_MIN)
         BOUND_MAX = MAX(M%PPN(L,N),BOUND_MAX)
      ENDDO
      ENDDO
   ELSE
...
ELSE
   IF (BF%DEBUG .EQ. 0) THEN
      WRITE(LU_BNDF(NF,NM)) ((M%PP(L,N),L=L1,L2+1),N=N1,N2+1)
      DO L = L1, L2+1
      DO N = N1, N2+1
         BOUND_MIN = MIN(M%PP(L,N),BOUND_MIN)
         BOUND_MAX = MAX(M%PP(L,N),BOUND_MAX)
      ENDDO
      ENDDO
   ELSE
      NBF_DEBUG = (2+L2-L1)*(2+N2-N1)
...
DO IP=1,M%NLP
   ...
   M%WORK4(BC%IIG,BC%JJG,BC%KKG) = M%WORK4(BC%IIG,BC%JJG,BC%KKG) + DROPMASS*LP%U*LP%RVC
   M%WORK5(BC%IIG,BC%JJG,BC%KKG) = M%WORK5(BC%IIG,BC%JJG,BC%KKG) + DROPMASS*LP%V*LP%RVC
   M%WORK6(BC%IIG,BC%JJG,BC%KKG) = M%WORK6(BC%IIG,BC%JJG,BC%KKG) + DROPMASS*LP%W*LP%RVC
ENDDO
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (M%CELL(IC)%SOLID .OR. M%CELL(IC)%EXTERIOR) THEN
      M%WORK4(BC%II,BC%JJ,BC%KK) = M%WORK4(BC%IIG,BC%JJG,BC%KKG)
      M%WORK5(BC%II,BC%JJ,BC%KK) = M%WORK5(BC%IIG,BC%JJG,BC%KKG)
      M%WORK6(BC%II,BC%JJ,BC%KK) = M%WORK6(BC%IIG,BC%JJG,BC%KKG)
   ENDIF
ENDDO
...
DO K=KMIN,KMAX
   DO J=JMIN,JMAX
      DO I=IMIN,IMAX
         WRITE(LU_UVW,TCFORM) M%U(I,J,K),M%V(I,J,K),M%W(I,J,K)
      ENDDO
   ENDDO
ENDDO
...
WRITE(TCFORM,'(A,I0,2A)') "(",5,INTG_LIST,")"
WRITE(LU_TMP,TCFORM) 1,M%IBAR,1,M%JBAR,1,M%KBAR

DO K=1,M%KBAR
   DO J=1,M%JBAR
      DO I=1,M%IBAR
         WRITE(LU_TMP,'(G0)') M%TMP(I,J,K)
      ENDDO
   ENDDO
ENDDO
...
SUBROUTINE DUMP_SPEC(FN_SPEC,NM)
...
WRITE(TCFORM,'(A,I0,2A)') "(",5,INTG_LIST,")"
WRITE(LU_SPEC,TCFORM) 1,M%IBAR,1,M%JBAR,1,M%KBAR
WRITE(TCFORM,'(A,I0,4A)') "(",N_TRACKED_SPECIES-1,"(G0,'",SEPARATOR,"'),G0",")"

DO K=1,M%KBAR
   DO J=1,M%JBAR
      DO I=1,M%IBAR
         WRITE(LU_SPEC,TCFORM) ( M%ZZ(I,J,K,N), N=1,N_TRACKED_SPECIES )
      ENDDO
   ENDDO
ENDDO
