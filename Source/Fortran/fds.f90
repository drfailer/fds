...
SUBROUTINE PROCESS_EXTERNAL_VARIABLES() BIND(C, NAME = "processExternalVariables")
  ...
  ENDIF
END SUBROUTINE PROCESS_EXTERNAL_VARIABLES


SUBROUTINE PREDICTOR_FINIT_DIFFERENCE() BIND(C, NAME = "predictorFinitDifference")
...
END SUBROUTINE PREDICTOR_FINIT_DIFFERENCE


SUBROUTINE EXCHANGE_VALUES() BIND(C, NAME = "exchangeValues")
...
SUBROUTINE COMPUTE_DENSITY() BIND(C, NAME = "computeDensity")
...
SUBROUTINE COMPUTE_VELOCITY() BIND(C, NAME = "computeVelocity")
   ! Calculate convective and diffusive terms of the velocity equation.
   COMPUTE_VELOCITY_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   ENDDO COMPUTE_VELOCITY_LOOP
END SUBROUTINE COMPUTE_VELOCITY


SUBROUTINE HVAC_SOLVER() BIND(C, NAME = "hvacSolver")
   ! HVAC solver
   IF (HVAC_SOLVE) THEN
   ...
   ENDIF
END SUBROUTINE HVAC_SOLVER


! TODO: this function does too many things
SUBROUTINE COMPUTE_WALL_BC() BIND(C, NAME = "computeWallBC")
...
END SUBROUTINE COMPUTE_WALL_BC


SUBROUTINE EXCHANGE_MESH_DIVERGENCE() BIND(C, NAME = "exchangeMeshDivergence")
...
END SUBROUTINE EXCHANGE_MESH_DIVERGENCE


SUBROUTINE UPDATE_GLOBAL_PRESSURE() BIND(C, NAME = "updateGlobalPressure")
...
END SUBROUTINE UPDATE_GLOBAL_PRESSURE


SUBROUTINE COMPUTE_DIVERGENCE_PHASE2() BIND(C, NAME = "computeDivergencePhase2")
...
END SUBROUTINE COMPUTE_DIVERGENCE_PHASE2


SUBROUTINE SOLVE_PRESSURE() BIND(C, NAME = "solvePressure")
...
SUBROUTINE PREDICT_VELOCITY() BIND(C, NAME = "predictVelocity")
...
INTEGER FUNCTION VERIFY_INSTABILITY() BIND(C, NAME = "verifyInstability") RESULT(EXIT_TS_LOOP)
   EXIT_TS_LOOP = 0
   ...
   IF (STOP_STATUS==INSTABILITY_STOP) THEN
      ...
      EXIT_TS_LOOP = 1
   ENDIF
END FUNCTION VERIFY_INSTABILITY


SUBROUTINE UPDATE_TIME_STEP_INDEX() BIND(C, NAME = "updateTimeStepIndex")
   ...
   ENDIF
END SUBROUTINE UPDATE_TIME_STEP_INDEX


! TODO: this function does too many things
INTEGER FUNCTION TIME_STEP_REDUCED() BIND(C, NAME = "timeStepReduced") RESULT(RES)
    RES = 0
    ...
    IF (ANY(CHANGE_TIME_STEP_INDEX==-1)) THEN
        ...
        FIRST_PASS = .FALSE.
    ENDIF
END FUNCTION TIME_STEP_REDUCED


SUBROUTINE START_PREDICTOR() BIND(C, NAME = "startPredictor")
    PREDICTOR = .TRUE.
    CORRECTOR = .FALSE.

    CALL PROCESS_EXTERNAL_VARIABLES
    CALL PREDICTOR_FINIT_DIFFERENCE

    ! Estimate quantities at next time step, and decrease/increase time step if
    ! necessary based on CFL condition

    FIRST_PASS = .TRUE.
END SUBROUTINE START_PREDICTOR


SUBROUTINE FINISH_PREDICTOR() BIND(C, NAME = "finishPredictor")
    ...
    DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
       CALL MATCH_VELOCITY(NM)
    ENDDO
END SUBROUTINE FINISH_PREDICTOR


SUBROUTINE APPLY_VELOCITY_BC() BIND(C, NAME = "applyVelocityBC")
...
SUBROUTINE RUN_PREDICTOR() BIND(C, NAME = "runPredictor")
    CALL START_PREDICTOR

    CHANGE_TIME_STEP_LOOP: DO
       CALL COMPUTE_DENSITY
       CALL EXCHANGE_VALUES
       CALL COMPUTE_VELOCITY
       CALL HVAC_SOLVER
       CALL COMPUTE_WALL_BC
       CALL EXCHANGE_MESH_DIVERGENCE
       CALL UPDATE_GLOBAL_PRESSURE
       CALL COMPUTE_DIVERGENCE_PHASE2
       CALL SOLVE_PRESSURE
       CALL PREDICT_VELOCITY
       IF (VERIFY_INSTABILITY() == 1) THEN
         EXIT CHANGE_TIME_STEP_LOOP
       ENDIF
       CALL UPDATE_TIME_STEP_INDEX
       IF (TIME_STEP_REDUCED() == 0) THEN  ! If the time step was reduced, CYCLE CHANGE_TIME_STEP_LOOP
          EXIT CHANGE_TIME_STEP_LOOP
       ENDIF
    ENDDO CHANGE_TIME_STEP_LOOP

    CALL FINISH_PREDICTOR
