...
USE GLOBAL_CONSTANTS, ONLY: GVEC,NULL_BOUNDARY,OPEN_BOUNDARY,INTERPOLATED_BOUNDARY,&
...
SUBROUTINE CALC_AGGLOMERATION(M,DT,NM)
...
! Local aliases (shadow module-level MESH_POINTERS for thread safety)
INTEGER :: IBAR, JBAR, KBAR
INTEGER, POINTER, DIMENSION(:,:,:) :: CELL_INDEX, PRESSURE_ZONE
TYPE(CELL_TYPE), POINTER, DIMENSION(:) :: CELL
REAL(EB), POINTER, DIMENSION(:,:,:) :: RHO, TMP, MU, U, V, W
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZ
REAL(EB), POINTER, DIMENSION(:) :: RDX, RDY, RDZ
REAL(EB), POINTER, DIMENSION(:,:) :: PBAR

IBAR = M%IBAR; JBAR = M%JBAR; KBAR = M%KBAR
CELL_INDEX => M%CELL_INDEX; CELL => M%CELL; PRESSURE_ZONE => M%PRESSURE_ZONE
ZZ => M%ZZ; RHO => M%RHO; TMP => M%TMP; MU => M%MU
U => M%U; V => M%V; W => M%W
RDX => M%RDX; RDY => M%RDY; RDZ => M%RDZ; PBAR => M%PBAR
ZZ_GET = 0._EB
...
RECURSIVE SUBROUTINE SOOT_SURFACE_OXIDATION_TS(DT,NM,M)
! Thread-safe per-mesh version of SOOT_SURFACE_OXIDATION.
! Uses M parameter instead of POINT_TO_MESH. No module-level pointer aliases.
USE GLOBAL_CONSTANTS, ONLY : R0,SOLID_BOUNDARY,MW_O2,O2_INDEX,SOOT_INDEX,NU_SOOT_OX,ZZ_MIN_GLOBAL
USE PHYSICAL_FUNCTIONS, ONLY: GET_MASS_FRACTION, GET_MOLECULAR_WEIGHT, GET_SPECIFIC_GAS_CONSTANT,&
                              GET_SPECIFIC_HEAT,GET_AVERAGE_SPECIFIC_HEAT
INTEGER,INTENT(IN) :: NM
...
TYPE(SPECIES_MIXTURE_TYPE), POINTER :: SM
TYPE(SPECIES_TYPE), POINTER :: SS
TYPE(WALL_TYPE), POINTER :: WC_L
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: B1_L
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC_L
TYPE(CFACE_TYPE), POINTER :: CFA_L

SS => SPECIES(SOOT_INDEX)

WALL_CELL_LOOP: DO IW = 1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   WC_L=>M%WALL(IW)
   B1_L=>M%BOUNDARY_PROP1(WC_L%B1_INDEX)
   BC_L=>M%BOUNDARY_COORD(WC_L%BC_INDEX)
   IF (WC_L%BOUNDARY_TYPE/=SOLID_BOUNDARY) CYCLE WALL_CELL_LOOP
   IF (B1_L%AWM_AEROSOL(SS%AWM_INDEX)<ZZ_MIN_GLOBAL) CYCLE WALL_CELL_LOOP
   M_SOOT = B1_L%AWM_AEROSOL(SS%AWM_INDEX)*B1_L%AREA
   IIG = BC_L%IIG
   JJG = BC_L%JJG
   KKG = BC_L%KKG
   RHO_G = M%RHO(IIG,JJG,KKG)
   TMP_G = M%TMP(IIG,JJG,KKG)
   ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,M%ZZ(IIG,JJG,KKG,1:N_TRACKED_SPECIES) &
      -M%M_DOT_PPP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)*DT/RHO_G)
   CALL GET_MASS_FRACTION(ZZ_GET,O2_INDEX,Y_O2)
   IF (Y_O2 < ZZ_MIN_GLOBAL) CYCLE
   CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW)
   X_O2 = Y_O2*MW/MW_O2
   DZZ = 0._EB
   DMDT = A*M_SOOT*X_O2*EXP(E/B1_L%TMP_F)
   VOL = M%DX(IIG)*M%RC(IIG)*M%DY(JJG)*M%DZ(KKG)
   DM = MIN(M_SOOT,DMDT*DT,-Y_O2*RHO_G*VOL/MINVAL(NU_SOOT_OX))

   DZZ = NU_SOOT_OX *  DM / VOL
   M%Q(IIG,JJG,KKG) = M%Q(IIG,JJG,KKG)-SUM(SPECIES_MIXTURE%H_F*DZZ)/DT

   DM = DM / B1_L%AREA
   IF (SS%AGGLOMERATING) THEN
      M_SOOT = M_SOOT / B1_L%AREA
      NS2 = FINDLOC(AGGLOMERATION_SPEC_INDEX,VALUE=SOOT_INDEX,DIM=1)
      DO NS=AGGLOMERATION_SMIX_INDEX(NS2),AGGLOMERATION_SMIX_INDEX(NS2)+N_PARTICLE_BINS(NS2)-1
         SM => SPECIES_MIXTURE(NS)
         IF (SM%AWM_INDEX > 0) &
            B1_L%AWM_AEROSOL(SM%AWM_INDEX) = B1_L%AWM_AEROSOL(SM%AWM_INDEX) - DM * B1_L%AWM_AEROSOL(SM%AWM_INDEX)/M_SOOT
      ENDDO
   ENDIF
   B1_L%AWM_AEROSOL(SS%AWM_INDEX) = B1_L%AWM_AEROSOL(SS%AWM_INDEX) - DM
   ...
   DO NS=1,N_TRACKED_SPECIES
      ...
      ZZ_GET(NS)=1._EB
      M_DOT_PPP_SINGLE = DZZ(NS)/DT
      MW_RATIO = SPECIES_MIXTURE(NS)%RCON/M%RSUM(IIG,JJG,KKG)
      IF (DZZ(NS)<0._EB) THEN
         M%D_SOURCE(IIG,JJG,KKG) = M%D_SOURCE(IIG,JJG,KKG) + M_DOT_PPP_SINGLE*MW_RATIO/RHO_G
      ELSE
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR,TMP_G)
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR2,B1_L%TMP_F)
         DELTA_H_G = CPBAR2*B1_L%TMP_F-CPBAR*TMP_G
         M%D_SOURCE(IIG,JJG,KKG) = M%D_SOURCE(IIG,JJG,KKG) + M_DOT_PPP_SINGLE*(MW_RATIO + DELTA_H_G/H_G)/RHO_G
         M%M_DOT_PPP(IIG,JJG,KKG,NS) = M%M_DOT_PPP(IIG,JJG,KKG,NS) + M_DOT_PPP_SINGLE
      ENDIF
   ENDDO
...
CFACE_LOOP: DO ICF=M%INTERNAL_CFACE_CELLS_LB+1,M%INTERNAL_CFACE_CELLS_LB+M%N_INTERNAL_CFACE_CELLS
   CFA_L=>M%CFACE(ICF)
   B1_L=>M%BOUNDARY_PROP1(CFA_L%B1_INDEX)
   BC_L=>M%BOUNDARY_COORD(CFA_L%BC_INDEX)
   IF (CFA_L%BOUNDARY_TYPE/=SOLID_BOUNDARY) CYCLE CFACE_LOOP
   IF (B1_L%AWM_AEROSOL(SS%AWM_INDEX)<ZZ_MIN_GLOBAL) CYCLE CFACE_LOOP
   M_SOOT = B1_L%AWM_AEROSOL(SS%AWM_INDEX)*B1_L%AREA
   IIG = BC_L%IIG
   JJG = BC_L%JJG
   KKG = BC_L%KKG
   RHO_G = B1_L%RHO_G
   TMP_G = B1_L%TMP_G
   ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,B1_L%ZZ_G(1:N_TRACKED_SPECIES) &
      -M%M_DOT_PPP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)*DT/RHO_G)
   CALL GET_MASS_FRACTION(ZZ_GET,O2_INDEX,Y_O2)
   IF (Y_O2 < ZZ_MIN_GLOBAL) CYCLE
   CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW)
   ...
   DO NS=1,N_TRACKED_SPECIES
      ...
      ZZ_GET(NS)=1._EB
      M_DOT_PPP_SINGLE = DZZ(NS)/DT
      MW_RATIO = SPECIES_MIXTURE(NS)%RCON/CFA_L%RSUM_G
      IF (DZZ(NS)<0._EB) THEN
         M%D_SOURCE(IIG,JJG,KKG) = M%D_SOURCE(IIG,JJG,KKG) + M_DOT_PPP_SINGLE*MW_RATIO/RHO_G
      ELSE
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR,TMP_G)
...
END SUBROUTINE SOOT_SURFACE_OXIDATION_TS
