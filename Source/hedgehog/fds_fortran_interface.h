...
void fds_cc_velocity_bc_ts(double t, int nm, int estimated, int do_ibedges);
void fds_cc_velocity_cutfaces_ts(int nm, int apply_to_estimated);
...
void fds_set_pressure_iterations(int val);
int fds_get_total_pressure_iterations();
void fds_set_total_pressure_iterations(int val);
void fds_set_iterate_baroclinic_term(int flag);
int fds_get_baroclinic();
...
// Full-slab flux push/pull (for push/buffer/pull exchange pipeline)
void fds_flux_push_slab(int nm, int nom, double *buf, int bufsize);
void fds_flux_pull_slab(int nm, int nom, const double *buf, int bufsize);
...
int fds_get_n_mpi_processes();             // number of MPI processes
