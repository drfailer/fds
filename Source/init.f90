...
OBST_LOOP_2: DO OBST_INDEX=1,M%N_OBST
   ...
   IF (ANY(SURFACE(OB%SURF_INDEX(:))%HT_DIM>1) .AND. (OB%I1==OB%I2 .OR. OB%J1==OB%J2 .OR. OB%K1==OB%K2)) THEN
      ...
      IF (OB%I1==OB%I2 .AND. ABS(OB%X2-OB%X1)>TWENTY_EPSILON_EB .AND. OB%UNDIVIDED_INPUT_LENGTH(1)<0.5_EB*M%DX(OB%I1)) THEN
         DO K=OB%K1+1,OB%K2
            IF (OB%J1>0) THEN
      ...
      IF (OB%J1==OB%J2 .AND. ABS(OB%Y2-OB%Y1)>TWENTY_EPSILON_EB .AND. OB%UNDIVIDED_INPUT_LENGTH(2)<0.5_EB*M%DY(OB%J1)) THEN
         DO K=OB%K1+1,OB%K2
            IF (OB%I1>0) THEN
               M%N_THIN_WALL_CELLS = M%N_THIN_WALL_CELLS + 1
      ...
      IF (OB%K1==OB%K2 .AND. ABS(OB%Z2-OB%Z1)>TWENTY_EPSILON_EB .AND. OB%UNDIVIDED_INPUT_LENGTH(3)<0.5_EB*M%DZ(OB%K1)) THEN
         DO I=OB%I1+1,OB%I2
            IF (OB%J1>0) THEN
               M%N_THIN_WALL_CELLS = M%N_THIN_WALL_CELLS + 1
...
ALLOCATE( M%U(-1:IBP1, 0:JBP1, 0:KBP1),STAT=IZERO)     ; CALL ChkMemErr('INIT','U',IZERO) ; M%U=0._EB
ALLOCATE( M%V( 0:IBP1,-1:JBP1, 0:KBP1),STAT=IZERO)     ; CALL ChkMemErr('INIT','V',IZERO) ; M%V=0._EB
ALLOCATE( M%W( 0:IBP1, 0:JBP1,-1:KBP1),STAT=IZERO)     ; CALL ChkMemErr('INIT','W',IZERO) ; M%W=0._EB
ALLOCATE(M%US(-1:IBP1, 0:JBP1, 0:KBP1),STAT=IZERO)     ; CALL ChkMemErr('INIT','US',IZERO) ; M%US=0._EB
ALLOCATE(M%VS( 0:IBP1,-1:JBP1, 0:KBP1),STAT=IZERO)     ; CALL ChkMemErr('INIT','VS',IZERO) ; M%VS=0._EB
ALLOCATE(M%WS( 0:IBP1, 0:JBP1,-1:KBP1),STAT=IZERO)     ; CALL ChkMemErr('INIT','WS',IZERO) ; M%WS=0._EB
ALLOCATE(M%WORK_U(-1:IBP1, 0:JBP1, 0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK_U',IZERO) ; M%WORK_U=0._EB
ALLOCATE(M%WORK_V( 0:IBP1,-1:JBP1, 0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK_V',IZERO) ; M%WORK_V=0._EB
ALLOCATE(M%WORK_W( 0:IBP1, 0:JBP1,-1:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK_W',IZERO) ; M%WORK_W=0._EB
...
ALLOCATE(M%WORK1_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK1_B',IZERO)
ALLOCATE(M%WORK2_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK2_B',IZERO)
ALLOCATE(M%WORK3_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK3_B',IZERO)
ALLOCATE(M%WORK4_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK4_B',IZERO)
ALLOCATE(M%WORK5_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK5_B',IZERO)
ALLOCATE(M%WORK6_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK6_B',IZERO)
ALLOCATE(M%WORK7_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK7_B',IZERO)
ALLOCATE(M%WORK8_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK8_B',IZERO)
ALLOCATE(M%WORK9_B(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','WORK9_B',IZERO)
...
ALLOCATE(M%SWORK1_B(0:IBP1,0:JBP1,0:KBP1,N_LOWER_SCALARS:N_TOTAL_SCALARS),STAT=IZERO) ; CALL ChkMemErr('INIT','SWORK1_B',IZERO)
ALLOCATE(M%SWORK2_B(0:IBP1,0:JBP1,0:KBP1,N_LOWER_SCALARS:N_TOTAL_SCALARS),STAT=IZERO) ; CALL ChkMemErr('INIT','SWORK2_B',IZERO)
ALLOCATE(M%SWORK3_B(0:IBP1,0:JBP1,0:KBP1,N_LOWER_SCALARS:N_TOTAL_SCALARS),STAT=IZERO) ; CALL ChkMemErr('INIT','SWORK3_B',IZERO)
M%IWORK1=0
M%SWORK1=0._EB
M%SWORK2=0._EB
M%SWORK3=0._EB
...
ALLOCATE(  M%D_SUM_LOC(N_ZONE),STAT=IZERO)       ; CALL ChkMemErr('INIT','D_SUM_LOC',IZERO)   ; M%D_SUM_LOC = 0._EB
ALLOCATE(  M%P_SUM_LOC(N_ZONE),STAT=IZERO)       ; CALL ChkMemErr('INIT','P_SUM_LOC',IZERO)   ; M%P_SUM_LOC = 0._EB
ALLOCATE(  M%U_SUM_LOC(N_ZONE),STAT=IZERO)       ; CALL ChkMemErr('INIT','U_SUM_LOC',IZERO)   ; M%U_SUM_LOC = 0._EB
...
WALL_LOOP_0: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.SOLID_CELL) THEN
      IF ( (ABS(B1%U_NORMAL_0)>TWENTY_EPSILON_EB .OR. ANY(SF%LEAK_PATH>=0)) .AND. WC%OBST_INDEX>0 ) THEN
...
IF (ONE_D%LAYER_THICKNESS(1)<TWENTY_EPSILON_EB) THEN
...
LAYER_LOOP: DO NL=1,ONE_D%N_LAYERS
   ...
   DO NN=1,ONE_D%N_MATL
      IF (ONE_D%MATL_COMP(NN)%MASS_FRACTION(NL)>TWENTY_EPSILON_EB) &
...
IF (NWP_MAX > MAX_WALL_CELLS_DIM) CALL SHUTDOWN('ERROR: NWP_MAX exceeds MAX_WALL_CELLS_DIM')
...
SELECT_POISSON_SOLVER: SELECT CASE(M%IPS)
   ...
   CASE (0:1) SELECT_POISSON_SOLVER
      ...
      IF (TWO_D .AND. CYLINDRICAL) THEN
         IF (ABS(XS)<=TWENTY_EPSILON_EB .AND. LBC==FISHPAK_BC_DIRICHLET_DIRICHLET) LBC = 5
         IF (ABS(XS)<=TWENTY_EPSILON_EB .AND. LBC==FISHPAK_BC_DIRICHLET_NEUMANN)   LBC = 6
         IF (ABS(XS)<=TWENTY_EPSILON_EB .AND. LBC==FISHPAK_BC_NEUMANN_NEUMANN)     LBC = 6
         IF (ABS(XS)<=TWENTY_EPSILON_EB .AND. LBC==FISHPAK_BC_NEUMANN_DIRICHLET)   LBC = 5
...
ALLOCATE(Q_DOT_MESH(N_Q_DOT,NMESHES),STAT=IZERO) ; CALL ChkMemErr('INIT','Q_DOT_MESH',IZERO) ; Q_DOT_MESH=0._EB
ALLOCATE(M_DOT_MESH(N_TRACKED_SPECIES,NMESHES),STAT=IZERO) ; CALL ChkMemErr('INIT','M_DOT_MESH',IZERO) ; M_DOT_MESH=0._EB
ALLOCATE(MASS_INTEGRAL_MESH(0:N_SPECIES+N_TRACKED_SPECIES,NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','MASS_INTEGRAL_MESH',IZERO) ; MASS_INTEGRAL_MESH=0._EB
...
IF (WC%OBST_INDEX>0) THEN
   OB=>M%OBSTRUCTION(WC%OBST_INDEX)
   IF (OB%FDS_AREA(ABS(BC%IOR))>TWENTY_EPSILON_EB) B1%AREA_ADJUST = &
      SF%AREA_MULTIPLIER*OB%INPUT_AREA(ABS(BC%IOR))/OB%FDS_AREA(ABS(BC%IOR))
   IF (B1%AREA_ADJUST<=TWENTY_EPSILON_EB) B1%AREA_ADJUST = 1._EB
...
IF (WC%OBST_INDEX>0 .AND. ABS(SF%VOLUME_FLOW)>=TWENTY_EPSILON_EB) THEN
   OB=>M%OBSTRUCTION(WC%OBST_INDEX)
   IF (OB%FDS_AREA(ABS(BC%IOR))>TWENTY_EPSILON_EB) &
      B1%U_NORMAL_0 = SF%VOLUME_FLOW*(OB%INPUT_AREA(ABS(BC%IOR))/OB%UNDIVIDED_INPUT_AREA(ABS(BC%IOR))) / OB%FDS_AREA(ABS(BC%IOR))
ENDIF
IF (WC%OBST_INDEX>0 .AND. ABS(SF%MASS_FLUX_TOTAL)>=TWENTY_EPSILON_EB) THEN
   OB=>M%OBSTRUCTION(WC%OBST_INDEX)
...
PROCESS_VENT: IF (WC%VENT_INDEX>0) THEN
   ...
   ENDIF
   IF (ABS(SF%MASS_FLUX_TOTAL)>TWENTY_EPSILON_EB) B1%U_NORMAL_0 = SF%MASS_FLUX_TOTAL/RHOA*B1%AREA_ADJUST

   IF (SF%CONVERT_VOLUME_TO_MASS) THEN
      IF (ABS(B1%U_NORMAL_0)>TWENTY_EPSILON_EB) THEN
         ZZ_GET=0._EB
...
! Set cross-mesh flags for WALL_BC decomposition (INTERPOLATED_BC and BACK_MESH)

DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   WC => M%WALL(IW)

   ! Mark cells with INTERPOLATED_BOUNDARY (requires OMESH access)
   IF (WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY) THEN
      WC%HAS_INTERPOLATED_BC = .TRUE.
   ENDIF

   ! Mark cells with BACK_MESH coupling (thin walls spanning meshes)
   IF (WC%OD_INDEX > 0) THEN
      ONE_D => M%BOUNDARY_ONE_D(WC%OD_INDEX)
      IF (ONE_D%BACK_MESH > 0) THEN
         WC%HAS_BACK_MESH = .TRUE.
      ENDIF
   ENDIF
ENDDO
...
FIND_BACK_WALL_CELL: DO  ! Look for the back wall cell; that is, the wall cell on the other side of the obstruction
   ...
   IF (SF%VARIABLE_THICKNESS .OR. SF%HT_DIM>1) THEN
      ...
      IF (THICKNESS<TWENTY_EPSILON_EB .AND. OBST_INDEX>0) THEN
         SELECT CASE(ABS(BC%IOR))
