...
PUBLIC COMBUSTION_MODEL, CHECK_REACTION, GET_FLAME_TEMPERATURE, CALC_FCENT, &
       CONDENSATION_EVAPORATION_KERNEL, COMBUSTION_BC_KERNEL
...
SUBROUTINE FIRE_FORWARD_EULER(ZZ_OUT,ZZ_IN,ZZ_0,ZETA_OUT,ZETA_IN,DT_LOC,TMP_IN,RHO_HAT,CELL_MASS,TAU_MIX,&
                              Q_REAC_LOC,TOTAL_MIXED_MASS,NO_REACTIONS,DZ_F0,MOLPCM3)
...
REAL(EB), INTENT(INOUT) :: DZ_F0(:), MOLPCM3
...
IF (ANY(REACTION%FAST_CHEMISTRY)) THEN
   DO PTY = 1,MAX_PRIORITY
      CALL REACTION_RATE(DZZ,ZZ_HAT,DT_LOC,RHO_HAT,TMP_IN,INFINITELY_FAST,Q_REAC_OUT,NO_REACTIONS,DZ_F0,MOLPCM3,PRIORITY=PTY)
      ZZ_HAT = ZZ_HAT + DZZ
...
IF (.NOT.ALL(REACTION%FAST_CHEMISTRY)) THEN
   CALL REACTION_RATE(DZZ,ZZ_HAT,DT_LOC,RHO_HAT,TMP_IN,FINITE_RATE,Q_REAC_OUT,NO_REACTIONS,DZ_F0,MOLPCM3)
   ZZ_HAT = ZZ_HAT + DZZ
   Q_REAC_LOC = Q_REAC_LOC + Q_REAC_OUT*TOTAL_MIXED_MASS
ENDIF
...
SUBROUTINE FIRE_RK2(ZZ_OUT,ZZ_IN,ZZ_0,ZETA_OUT,ZETA_IN,DT_SUB,N_INC,TMP_IN,RHO_HAT,CELL_MASS,TAU_MIX,&
                    Q_REAC_OUT,TOTAL_MIXED_MASS_OUT,NO_REACTIONS,DZ_F0,MOLPCM3)
...
DO N=1,N_INC
   CALL FIRE_FORWARD_EULER(ZZ_TMP_1,ZZ_TMP_0,ZZ_0,ZETA_TMP_1,ZETA_TMP_0,DT_LOC,TMP_IN,RHO_HAT,CELL_MASS,TAU_MIX,&
                           Q_REAC_1,TOTAL_MIXED_MASS_1,NO_REACTIONS,DZ_F0,MOLPCM3)

   CALL FIRE_FORWARD_EULER(ZZ_TMP_2,ZZ_TMP_1,ZZ_0,ZETA_TMP_2,ZETA_TMP_1,DT_LOC,TMP_IN,RHO_HAT,CELL_MASS,TAU_MIX,&
                           Q_REAC_2,TOTAL_MIXED_MASS_2,NO_REACTIONS,DZ_F0,MOLPCM3)
...
SUBROUTINE REACTION_RATE(DZZ,ZZ_OLD,DT_SUB,RHO_0,TMP_0,KINETICS,Q_REAC_OUT,NO_REACTIONS,DZ_F0,MOLPCM3,PRIORITY)
...
REAL(EB) :: DZ_F,YY_PRIMITIVE(1:N_SPECIES),MW,DT_TMP(1:N_TRACKED_SPECIES),DT_MIN,DT_LOC,&
            ...
            K_INF,K_0,P_RI,FCENT,C_I,RRTMP0
...
REAL(EB) :: A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            DZ_F0_LOC(1:N_REACTIONS),MOLPCM3_LOC
   ...
   INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE_SOLVER)
      ...
      CASE (EXPLICIT_EULER) ! Simple chemistry
         ...
         CALL FIRE_FORWARD_EULER(ZZ_MIXED_NEW,ZZ_MIXED,ZZ_0,ZETA,ZETA_0,DT_SUB,TMP_IN,RHO_HAT,&
                                 CELL_MASS,TAU_MIX,Q_REAC_SUB,TOTAL_MIXED_MASS,NO_REACTIONS,DZ_F0_LOC,MOLPCM3_LOC)
         ZETA_0 = ZETA
...
!> \brief Condensation/evaporation kernel (thread-safe).
!> \param M Mesh data structure
!> \param DT Current time step (s)
!> \param NM Current mesh number

SUBROUTINE CONDENSATION_EVAPORATION_KERNEL(M,DT,NM,WORK_BRANCH)
USE MATH_FUNCTIONS, ONLY : INTERPOLATE1D_UNIFORM
USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_HEAT, GET_MASS_FRACTION_ALL, &
   GET_VISCOSITY, GET_MOLECULAR_WEIGHT,RELATIVE_HUMIDITY
...
REAL(EB):: Y_GAS, DHOR, H_V_B, H_V_A, H_V, H_V_N, MW_RATIO, MW_GAS, &
           ZZ_GET(1:N_TRACKED_SPECIES),&
           X_CLOUD, Y_CLOUD, CP, TMP_N, Y_N, Y_1, Y_2, X_GUESS, &
           Y_GUESS, Y_ALL(1:N_SPECIES), P_RATIO, Y_COND, &
           T_BOIL_EFF, RHO_G, TMP_G, TMP_W, Y_NN, D_AIR, &
           H_MASS,N_PART,X_WALL,Y_WALL, M_WALL, M_WALL2, RVC, &
           B_NUMBER, &
           MU_AIR, SC_AIR, TMP_N2, M_DOT_VAL, M_VAP, Y_PLUS, &
           Y_S_PLUS, GAMMA, RHOCBAR, MCBAR, H_L_1, H_L_2
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: WK1,WK2
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: SK1

! Select WORK scratch arrays based on branch
WORK_BRANCH_VAL = 1
IF (PRESENT(WORK_BRANCH)) WORK_BRANCH_VAL = WORK_BRANCH
IF (WORK_BRANCH_VAL==2) THEN
   WK1=>M%WORK1_B; WK2=>M%WORK2_B; SK1=>M%SWORK1_B
ELSE
   WK1=>M%WORK1; WK2=>M%WORK2; SK1=>M%SWORK1
ENDIF

ZZ_INTERIM=> SK1
ZZ_INTERIM(0:M%IBP1,0:M%JBP1,0:M%KBP1,1:) = &
   M%ZZ(0:M%IBP1,0:M%JBP1,0:M%KBP1,1:)
RHO_INTERIM => WK1
RHO_INTERIM(0:M%IBP1,0:M%JBP1,0:M%KBP1) = &
   M%RHO(0:M%IBP1,0:M%JBP1,0:M%KBP1)
TMP_INTERIM => WK2
TMP_INTERIM(0:M%IBP1,0:M%JBP1,0:M%KBP1) = &
   M%TMP(0:M%IBP1,0:M%JBP1,0:M%KBP1)
...
SPEC_LOOP: DO NS = 1, N_TRACKED_SPECIES
   ...
   SS => SPECIES(Y_INDEX)
   CALL INTERPOLATE1D_UNIFORM(LBOUND(SS%H_V,1),SS%H_V, &
      SS%TMP_V,H_V_B)
   ...
   DO K = 1, M%KBAR
      DO J = 1, M%JBAR
         ILOOP: DO I = 1, M%IBAR
            IF (M%CELL(M%CELL_INDEX(I,J,K))%SOLID) CYCLE ILOOP
            IF (CC_IBM) THEN
               IF (M%CCVAR(I,J,K,CC_CGSC) /= CC_GASPHASE) &
                  CYCLE ILOOP
            ENDIF

            ZZ_GET(1:N_TRACKED_SPECIES) = &
               ZZ_INTERIM(I,J,K,1:N_TRACKED_SPECIES)
            IF (ZZ_GET(NS) < ZZ_MIN_GLOBAL .AND. &
               ZZ_GET(Z_COND_INDEX) < ZZ_MIN_GLOBAL) CYCLE ILOOP
            DHOR = H_V_B*SS%MW/R0

            P_RATIO = M%PBAR(0,M%PRESSURE_ZONE(I,J,K))/P_STP

            TMP_G = M%TMP(I,J,K)
            RHO_G = M%RHO(I,J,K)
            ! Boiling temperature at current background pressure
            T_BOIL_EFF = MAX(0._EB, &
               DHOR*SS%TMP_V/(DHOR-SS%TMP_V*LOG(P_RATIO) &
               +TWENTY_EPSILON_EB))

            CALL INTERPOLATE1D_UNIFORM(LBOUND(SS%H_V,1), &
               SS%H_V,TMP_G,H_V)
            CALL INTERPOLATE1D_UNIFORM(LBOUND(SS%H_V,1), &
               SS%H_V,T_BOIL_EFF,H_V_B)
            ...
            ! Determine the ratio of molecular weights
            MW_GAS = 0._EB
            ...
            X_CLOUD = MIN(1._EB, &
               EXP(DHOR*(1._EB/T_BOIL_EFF-1._EB/TMP_G)))
            Y_CLOUD = X_CLOUD/ &
               (MW_RATIO + (1._EB-MW_RATIO)*X_CLOUD)
            RVC = M%RDX(I)*M%RRN(I)*M%RDY(J)*M%RDZ(K)
            IF (Y_GAS > Y_CLOUD) THEN
               IF (ZZ_INTERIM(I,J,K,NS) < ZZ_MIN_GLOBAL) &
                  CYCLE ILOOP
               Y_1 = MAX(Y_CLOUD, &
                  Y_GAS-ZZ_INTERIM(I,J,K,NS))
               Y_2 = Y_GAS
               Y_N = 0.5_EB*(Y_1+Y_2)
            ELSE
            ...
            EVAP_LOOP: DO
               ...
               IF (ABS(TMP_N - TMP_N2)<1.E-2_EB) &
                  EXIT EVAP_LOOP
               CALL INTERPOLATE1D_UNIFORM( &
                  LBOUND(SS%H_V,1),SS%H_V,TMP_N,H_V_N)
               H_V_A = 0.5_EB*(H_V_N+H_V_B)
               X_GUESS = MIN(1._EB, &
                  EXP(H_V_A*SS%MW/R0* &
                  (1._EB/T_BOIL_EFF-1._EB/TMP_N)))
               Y_GUESS = X_GUESS/ &
                  (MW_RATIO + (1._EB-MW_RATIO)*X_GUESS)
               Y_NN = Y_N
               IF (Y_GUESS >= Y_N) THEN
                  Y_1 = Y_N
                  Y_N = 0.5_EB*(Y_N+Y_2)
               ELSE
                  ...
                  Y_N = 0.5_EB*(Y_N+Y_1)
               ENDIF
               IF (ABS(Y_NN-Y_N)/Y_N < 1.E-4_EB) &
                  EXIT EVAP_LOOP
            ENDDO EVAP_LOOP

            ! Limit based on evaporation rate
            D_AIR = D_Z(NINT(TMP_G),NS)
            H_MASS = 2._EB*D_AIR/SM%MEAN_DIAMETER
            N_PART = Y_COND * RHO_G / &
               (FOTHPI*SS%DENSITY_LIQUID* &
               (0.5_EB*SM%MEAN_DIAMETER)**3)
            N_PART = MAX(NUCLEATION_SITES,N_PART)
            B_NUMBER = LOG(1._EB + &
               ABS(Y_CLOUD - Y_GAS) / &
               MAX(DY_MIN_BLOWING, (1._EB - Y_CLOUD)))
            Y_1 = ZZ_INTERIM(I,J,K,NS)
            IF (Y_GAS < Y_CLOUD) THEN
               ZZ_INTERIM(I,J,K,NS) = &
                  ZZ_INTERIM(I,J,K,NS) + &
                  MIN(Y_N-Y_GAS, &
                  H_MASS*N_PART*4._EB*PI* &
                  (0.5_EB*SM%MEAN_DIAMETER)**2* &
                  B_NUMBER*DT/RHO_G)
            ELSE
               ZZ_INTERIM(I,J,K,NS) = &
                  ZZ_INTERIM(I,J,K,NS) - &
            ...
            ZZ_INTERIM(I,J,K,Z_COND_INDEX) = &
               ZZ_INTERIM(I,J,K,Z_COND_INDEX)-(Y_2-Y_1)
            TMP_INTERIM(I,J,K) = &
               TMP_INTERIM(I,J,K) - (Y_2-Y_1)*H_V/CP
            M%M_DOT_PPP(I,J,K,NS) = &
               M%M_DOT_PPP(I,J,K,NS) + &
               RHO_G*(Y_2-Y_1)/DT
            M%M_DOT_PPP(I,J,K,Z_COND_INDEX) = &
               M%M_DOT_PPP(I,J,K,Z_COND_INDEX) - &
               RHO_G*(Y_2-Y_1)/DT
            M%D_SOURCE(I,J,K) = M%D_SOURCE(I,J,K) - &
               (Y_2-Y_1)*H_V/(CP*TMP_G*DT)
         ENDDO ILOOP
      ENDDO
   ENDDO
   ...
   WALL_LOOP: DO IW=1, &
      M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
      WC=>M%WALL(IW)
      B1 => M%BOUNDARY_PROP1(WC%B1_INDEX)
      IF (WC%BOUNDARY_TYPE /= SOLID_BOUNDARY .OR. &
         B1%NODE_INDEX > 0) CYCLE WALL_LOOP
      B2 => M%BOUNDARY_PROP2(WC%B2_INDEX)
      BC => M%BOUNDARY_COORD(WC%BC_INDEX)
      I = BC%IIG
      J = BC%JJG
      K = BC%KKG
      IF (B1%AWM_AEROSOL(SS%AWM_INDEX) < &
         TWENTY_EPSILON_EB .AND. &
         ZZ_INTERIM(I,J,K,NS) < ZZ_MIN_GLOBAL) &
         CYCLE WALL_LOOP
      ...
      CALL INTERPOLATE1D_UNIFORM(LBOUND(SS%H_V,1), &
         SS%H_V,TMP_G,H_V)
      CALL INTERPOLATE1D_UNIFORM(LBOUND(SS%H_L,1), &
         SS%H_L,TMP_G,H_L_1)
      ...
      ! Compute equilibrium vapor mass fraction
      X_WALL = MIN(1._EB, &
         EXP(DHOR*(1._EB/T_BOIL_EFF-1._EB/TMP_W)))
      Y_WALL = X_WALL/ &
         (MW_RATIO + (1._EB-MW_RATIO)*X_WALL)
      ...
      ELSE
         IF (B1%AWM_AEROSOL(SS%AWM_INDEX) < &
            TWENTY_EPSILON_EB) CYCLE WALL_LOOP
         Y_1 = Y_GAS
         Y_2 = MIN(Y_WALL, &
            (Y_GAS*RHO_G+M_WALL*RVC) / &
            (RHO_G+M_WALL*RVC))
         Y_N = 0.5_EB*(Y_1+Y_2)
      ENDIF
      ...
      Y_PLUS = RHO_G*MAX(0.0001_EB,B2%U_TAU)* &
         0.5_EB/(B1%RDN*MU_AIR)
      GAMMA = 0.01_EB*(SC_AIR*Y_PLUS)**4/ &
         (1._EB+5._EB*SC_AIR**3*Y_PLUS)
      Y_S_PLUS = SC_AIR*Y_PLUS*EXP(-GAMMA) + &
         (2.12_EB*LOG(Y_PLUS) + &
         (3.85_EB*SC_AIR**ONTH-1.3_EB)**2 + &
         2.12_EB*LOG(SC_AIR)) * EXP(-1._EB/GAMMA)
      ! Find equilibrium
      IF (SURFACE(WC%SURF_INDEX)%THERMAL_BC_INDEX== &
         THERMALLY_THICK) THEN
         ONE_D=>M%BOUNDARY_ONE_D(WC%OD_INDEX)
         SF=>SURFACE(WC%SURF_INDEX)
         RHOCBAR = 0._EB
         DO NMAT=1,SF%N_MATL
            IF (ONE_D%MATL_COMP(NMAT)%RHO(1)<= &
               TWENTY_EPSILON_EB) CYCLE
            RHOCBAR = RHOCBAR + &
               ONE_D%MATL_COMP(NMAT)%RHO(1)* &
               MATERIAL(SF%MATL_INDEX(NMAT))%C_S(ITMP)
         ENDDO
         ...
         EVAP_LOOP_2: DO
            ...
            TMP_N = TMP_W + &
               ((Y_GAS-Y_N)*RHO_G/RVC)* &
               (H_V+H_L_1-H_L_2)/MCBAR
            IF (ABS(TMP_N2- TMP_N)<1.E-2_EB) &
               EXIT EVAP_LOOP_2
            CALL INTERPOLATE1D_UNIFORM( &
               LBOUND(SS%H_V,1),SS%H_V,TMP_N,H_V_N)
            CALL INTERPOLATE1D_UNIFORM( &
               LBOUND(SS%H_L,1),SS%H_L,TMP_N,H_L_2)
            H_V_A = 0.5_EB*(H_V_N+H_V_B)
            X_GUESS = MIN(1._EB, &
               EXP(H_V_A*SS%MW/R0* &
               (1._EB/T_BOIL_EFF-1._EB/TMP_N)))
      ...
      IF (Y_GAS > Y_WALL) THEN
         M_DOT_VAL = RHO_G*MAX(0.0001_EB,B2%U_TAU)* &
            (Y_GAS-Y_WALL)/Y_S_PLUS
         M_VAP = -MIN((Y_GAS-Y_N)*RHO_G/ &
            (RVC*(1._EB-Y_N)),M_DOT_VAL*B1%AREA*DT)
      ELSE
         M_DOT_VAL = RHO_G*MAX(0.0001_EB,B2%U_TAU)* &
            (Y_WALL-Y_GAS)/Y_S_PLUS
         M_VAP = MIN((Y_N-Y_GAS)*RHO_G/ &
            (RVC*(1._EB-Y_N)),M_DOT_VAL*B1%AREA*DT)
      ENDIF

      ZZ_INTERIM(I,J,K,NS) = &
         ZZ_INTERIM(I,J,K,NS)+M_VAP*RVC
      RHO_INTERIM(I,J,K) = SUM(ZZ_INTERIM(I,J,K,:))
      ZZ_INTERIM(I,J,K,:) = &
         ZZ_INTERIM(I,J,K,:)/RHO_INTERIM(I,J,K)
      M_WALL2 = M_WALL - M_VAP
      CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW_GAS)
      M%D_SOURCE(I,J,K) = M%D_SOURCE(I,J,K) + &
         M_VAP*MW_GAS/SS%MW*RVC/(RHO_G*DT)
      M%M_DOT_PPP(I,J,K,NS) = &
         M%M_DOT_PPP(I,J,K,NS) + M_VAP*RVC/DT
      ZZ_INTERIM(I,J,K,:) = ZZ_INTERIM(I,J,K,:)/ &
         SUM(ZZ_INTERIM(I,J,K,:))
      IF (SM2%AWM_INDEX > 0) &
         B1%AWM_AEROSOL(SM2%AWM_INDEX) = M_WALL2/B1%AREA
      IF (SS%AWM_INDEX > 0) &
         B1%AWM_AEROSOL(SS%AWM_INDEX) = M_WALL2/B1%AREA
      B1%Q_CONDENSE = &
         -M_VAP * (H_V+H_L_1-H_L_2)/(B1%AREA*DT)
   ENDDO WALL_LOOP
ENDDO SPEC_LOOP

END SUBROUTINE CONDENSATION_EVAPORATION_KERNEL

!> \brief Thread-safe kernel version of COMBUSTION_BC.
!> Fills ghost cells of Q from neighboring mesh OMESH data.
!> \param M Mesh data structure
!> \param NM Mesh number

SUBROUTINE COMBUSTION_BC_KERNEL(M,NM)
   ...
   DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
      DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
         DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
            Q_OTHER = Q_OTHER + M%OMESH(NOM)%Q(IIO,JJO,KKO)
         ENDDO
      ENDDO
   ENDDO
   N_INT_CELLS = (EWC%IIO_MAX-EWC%IIO_MIN+1) * (EWC%JJO_MAX-EWC%JJO_MIN+1) * (EWC%KKO_MAX-EWC%KKO_MIN+1)
   M%Q(BC%II,BC%JJ,BC%KK) = Q_OTHER/REAL(N_INT_CELLS,EB)
ENDDO WALL_LOOP

END SUBROUTINE COMBUSTION_BC_KERNEL
